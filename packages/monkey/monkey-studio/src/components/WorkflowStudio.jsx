  ...
  loading: () => (
    <div className="absolute inset-0 flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        <div className="w-12 h-12 border-4 border-white/5 border-t-[#4262ff] rounded-full animate-spin" />
...
function WorkflowCard({ workflow, onClick, activeTab, onRename, onDelete }) {
  ...
  return (
    <div
      ...
      className="group relative aspect-[3/4] rounded-lg overflow-hidden cursor-pointer border border-white/5 bg-[#0a0a0a] transition-all hover:border-[#4262ff]/30 hover:scale-[1.02] shadow-2xl"
    >
      {workflow.thumbnail ? (
        <img
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-blue-500/10 to-[#ffd02f]/10 flex items-center justify-center">
          <svg
            width="40"
            height="40"
            viewBox="0 0 24 24"
          ...
          {showOptions && (
            <div className="absolute top-10 right-0 w-32 bg-[#111] border border-white/10 rounded-lg shadow-2xl py-1 animate-in fade-in zoom-in duration-200">
              <button
                ...
                className="w-full px-4 py-2 text-left text-[11px] font-bold text-white/70 hover:text-[#4262ff] hover:bg-white/5 transition-colors flex items-center gap-2"
              >
      ...
      <div className="absolute inset-x-0 bottom-0 p-4">
        <div className="text-[10px] font-bold text-[#4262ff] uppercase tracking-wider mb-1 opacity-80">
          {workflow.category || "General"}
        </div>
        <h3 className="text-sm font-bold text-white truncate group-hover:text-[#4262ff] transition-colors">
...
export default function WorkflowStudio({
  ...
  // Catch-all routes (/studio/[brandSlug]/[[...tab]], /motion-monkey/[[...tab]]) expose the
  ...
  if (loading && !selectedWorkflow) {
    return (
      <div className="h-full flex items-center justify-center">
        <div className="animate-spin text-[#4262ff] text-3xl">◌</div>
      </div>
    );
  }
        ...
        {isHeaderVisible ? (
          <div className="flex-shrink-0 h-14 border-b border-white/5 flex items-center justify-between px-6 bg-black/40 z-30">
                  ...
                  <button
                    ...
                    className={`px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-md transition-all ${
                      activeSubTab === "playground"
                        ? "bg-[#4262ff] text-black shadow-[0_0_15px_rgba(66, 98, 255,0.2)]"
            ...
            <div className="flex items-center gap-3">
              <span className="text-[11px] font-black text-[#4262ff] uppercase tracking-widest">
            ...
            <div className="flex bg-white/5 p-1 rounded-lg">
               <button
                 ...
                 className={`px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-md transition-all ${
                   activeSubTab === "playground" ? "bg-[#4262ff] text-black" : "text-white/40"
                 }`}
               >
                 Play
               </button>
               <button
                 ...
                 type="button"
                 className={`px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-md transition-all ${
                   activeSubTab === "builder" ? "bg-[#4262ff] text-black" : "text-white/40"
                 }`}
               >
                 Builder
               </button>
                ...
                <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
                  <form onSubmit={handleRun} className="space-y-6">
                              ...
                              <div key={key} className="space-y-2">
                                ...
                                {prop.type === "string" && !prop.enum ? (
                                  <textarea
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-[#4262ff]/50 transition-colors min-h-[80px] resize-none"
                                    placeholder={
                                  ...
                                  <select
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-[#4262ff]/50 transition-colors"
                                  >
                                    {prop.enum.map((opt) => (
                                      <option
                                        key={opt}
                    ...
                    <button
                      ...
                      className="w-full py-4 bg-[#4262ff] text-black text-xs font-black uppercase tracking-[0.2em] rounded-xl hover:bg-white transition-all transform hover:scale-[1.02] active:scale-95 disabled:opacity-50 disabled:grayscale shadow-[0_0_30px_rgba(66, 98, 255,0.15)] flex items-center justify-center gap-3 mt-8"
                    >
                      {isExecuting ? (
                        <>
                ...
                {isExecuting && (
                  <div className="flex flex-col items-center gap-6 animate-fade-in">
                    <div className="relative">
                      <div className="w-24 h-24 border-[3px] border-white/5 border-t-[#4262ff] rounded-full animate-spin shadow-[0_0_40px_rgba(66, 98, 255,0.1)]" />
                      <div className="absolute inset-0 flex items-center justify-center text-[#4262ff]">
                        <svg
                          width="32"
                          height="32"
                          viewBox="0 0 24 24"
                    ...
                    <div className="text-center space-y-2">
                      <div className="text-[10px] font-black text-[#4262ff] uppercase tracking-[0.3em] animate-pulse">
                        Running Pipeline
                      </div>
                  ...
                  <div className="w-full max-w-4xl space-y-8 animate-fade-in-up">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {result.outputs?.map((out, idx) => (
                        <div
                          key={idx}
                          className="group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-[#4262ff]/30 transition-all shadow-2xl"
                        >
                          {out.type === "image_url" ? (
                            <img
                              src={out.value}
                          ...
                          <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 to-transparent translate-y-full group-hover:translate-y-0 transition-transform">
                            <div className="flex items-center justify-between">
                              <span className="text-[10px] font-black text-[#4262ff] uppercase tracking-widest">
                                {out.id}
                              </span>
                              <a
                                href={out.value}
                                target="_blank"
                                rel="noreferrer"
                                className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center hover:bg-[#4262ff] hover:text-black transition-colors"
                              >
                                <svg
                                  width="14"
                                  height="14"
      ...
      <div className="max-w-7xl mx-auto w-full">
        <div className="flex flex-col gap-6 mb-12">
          <div className="flex items-end justify-between">
            ...
            <button
              ...
              className="px-6 py-3 bg-[#4262ff] text-black text-xs font-black uppercase tracking-widest rounded-lg hover:bg-white transition-all transform hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(66, 98, 255,0.3)] flex items-center gap-2"
            >
              <svg
                width="14"
                height="14"
        ...
        {loading ? (
          <div className="py-20 flex items-center justify-center">
            <div className="w-10 h-10 border-4 border-white/5 border-t-[#4262ff] rounded-full animate-spin" />
          </div>
        ) : (
          ...
          >
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-[#4262ff] uppercase tracking-widest">Workflow Name</label>
                <input
                  autoFocus
                  type="text"
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-[#4262ff]/50 transition-colors"
                />
              </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  type="submit"
                  className="flex-1 bg-[#4262ff] text-black px-4 py-3 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-white transition-all transform hover:scale-105 active:scale-95"
                >
                  Save Name
                </button>
              </div>
