...
describe("diff viewer file tree utilities", () => {
  ...
  test("collapses unary directory chains while flattening", () => {
    const rows = flattenFileTree(
      buildFileTree([{ file: "packages/clisisCoder/src/cli/app.ts" }, { file: "packages/clisisCoder/src/server/server.ts" }]),
    )

    expect(rows.map((row) => `${"  ".repeat(row.depth)}${row.kind}:${row.name}`)).toEqual([
      "directory:packages/clisisCoder/src",
      "  directory:cli",
      "    file:app.ts",
      "  directory:server",
      "    file:server.ts",
  ...
  test("does not collapse a directory into a file row", () => {
    const rows = flattenFileTree(buildFileTree([{ file: "packages/clisisCoder/src/app.ts" }]))
  ...
  test("stops collapsing at branches", () => {
    const rows = flattenFileTree(
      buildFileTree([
        { file: "packages/clisisCoder/src/cli/app.ts" },
        { file: "packages/clisisCoder/src/server/server.ts" },
  ...
  test("collapses expanded unary children under the first visible directory id", () => {
    ...
    expect(flattenFileTree(tree, new Set()).map((row) => row.name)).toEqual(["packages/clisisCoder/src"])
    expect(flattenFileTree(tree, new Set([packages.id])).map((row) => row.name)).toEqual([
      "packages/clisisCoder/src",
      "cli",
      "server",
    ])
  })
  ...
  test("moves collapsed chain selection to first visible child", () => {
    ...
    const packages = rows.find((row) => row.kind === "directory" && row.name === "packages/clisisCoder/src")!
  ...
  test("moves file and collapsed directory selection to visible parent", () => {
    ...
    const root = rows.find((row) => row.kind === "directory" && row.name === "packages/clisisCoder/src")!
    const cli = rows.find((row) => row.kind === "directory" && row.name === "cli")!
