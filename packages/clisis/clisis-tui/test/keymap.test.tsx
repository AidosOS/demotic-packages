...
import { getclisisCoderModeStack, CLISIS_CODER_BASE_MODE, clisisCoderKeymapProvider, registerclisisCoderKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerclisisCoderKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when clisisCoder mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getclisisCoderModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getclisisCoderModeStack(keymap).push("autocomplete")
