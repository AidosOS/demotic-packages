...
import {
  ...
  CLISIS_CODER_BASE_MODE,
  clisisCoderKeymapProvider,
  registerclisisCoderKeymap,
  useBindings,
  useclisisCoderKeymap,
} from "./keymap"
  ...
  const output = yield* Effect.scoped(
    Effect.gen(function* () {
      const renderer = yield* Effect.acquireRelease(
        Effect.tryPromise(() =>
          createCliRenderer({
            ...
            useMouse: !Flag.CLISIS_CODER_DISABLE_MOUSE && input.config.mouse,
            consoleOptions: {
      ...
      yield* Effect.acquireRelease(
        Effect.sync(() => registerclisisCoderKeymap(keymap, renderer, input.config)),
            ...
            <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
              ...
              >
                ...
                >
                  <TuiStartupProvider
                    value={{
                      initialRoute: process.env.CLISIS_CODER_ROUTE ? JSON.parse(process.env.CLISIS_CODER_ROUTE) : undefined,
                      skipInitialLoading: Boolean(process.env.CLISIS_CODER_FAST_BOOT),
                    }}
                  >
                    <ClipboardProvider>
                      <EpilogueProvider set={(value) => (epilogue.value = value)}>
                        <clisisCoderKeymapProvider keymap={keymap}>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.CLISIS_CODER_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.CLISIS_CODER_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("clisisCoder")
      return
    }
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.CLISIS_CODER_EXPERIMENTAL_WORKSPACES,
      ...
      },
      {
        ...
        title: "Open docs",
        run: () => {
          open("https://clisiscoder.ai/docs").catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to clisisCoder v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={
        !Flag.CLISIS_CODER_EXPERIMENTAL_DISABLE_COPY_ON_SELECT
          ? () => Selection.copy(renderer, toast, clipboard)
          : undefined
      }
    >
      <Show when={Flag.CLISIS_CODER_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
