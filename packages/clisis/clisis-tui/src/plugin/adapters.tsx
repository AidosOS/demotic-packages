import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@demotic/clisis-plugin/tui"
...
import type { useclisisCoderKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useclisisCoderKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getclisisCoderModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getclisisCoderModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
