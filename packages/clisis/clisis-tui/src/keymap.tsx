...
export const CLISIS_CODER_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const CLISIS_CODER_MODE_KEY = "clisis-coder.mode"

export const clisisCoderKeymapProvider = KeymapProvider
export const useclisisCoderKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type clisisCoderModeStack = ReturnType<typeof createClisisCoderModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, clisisCoderModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createClisisCoderModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(CLISIS_CODER_MODE_KEY, CLISIS_CODER_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(CLISIS_CODER_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(CLISIS_CODER_MODE_KEY, stack.at(-1)?.mode ?? CLISIS_CODER_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? CLISIS_CODER_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(CLISIS_CODER_MODE_KEY, undefined)
...
export function useclisisCoderModeStack() {
  return getclisisCoderModeStack(useclisisCoderKeymap())
}

export function getclisisCoderModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("clisisCoder mode stack is not registered for this keymap")
  return value
}
...
export function registerclisisCoderKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createClisisCoderModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = useclisisCoderKeymap()
