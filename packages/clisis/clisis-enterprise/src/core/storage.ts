...
import { lazy } from "@demotic/clisis-core/util/lazy"
...
export namespace Storage {
  ...
  function s3(): Adapter {
    const bucket = process.env.CLISIS_CODER_STORAGE_BUCKET!
    const region = process.env.CLISIS_CODER_STORAGE_REGION || "us-east-1"
    const client = new AwsClient({
      region,
      accessKeyId: process.env.CLISIS_CODER_STORAGE_ACCESS_KEY_ID!,
      secretAccessKey: process.env.CLISIS_CODER_STORAGE_SECRET_ACCESS_KEY!,
    })
    return createAdapter(client, `https://s3.${region}.amazonaws.com`, bucket)
  }

  function r2() {
    const accountId = process.env.CLISIS_CODER_STORAGE_ACCOUNT_ID!
    const client = new AwsClient({
      accessKeyId: process.env.CLISIS_CODER_STORAGE_ACCESS_KEY_ID!,
      secretAccessKey: process.env.CLISIS_CODER_STORAGE_SECRET_ACCESS_KEY!,
    })
    return createAdapter(client, `https://${accountId}.r2.cloudflarestorage.com`, process.env.CLISIS_CODER_STORAGE_BUCKET!)
  }

  const adapter = lazy(() => {
    const type = process.env.CLISIS_CODER_STORAGE_ADAPTER
