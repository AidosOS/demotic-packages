...
import { MenuV2 } from "@demotic/clisis-ui/v2/menu-v2"
...
import { wslclisisCoderAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.clisisCoderChecks[item.config.distro]
          const clisisCoderAction = () => wslclisisCoderAction(check())
          const busy = () => wsl.data?.job?.kind === "install-clisisCoder" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={clisisCoderAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installClisisCoder(item.config.distro))}
                    >
