...
import { Spinner } from "@demotic/clisis-ui/spinner"
import { showToast } from "@demotic/clisis-ui/toast"
import { createEffect, createMemo, For, Match, onCleanup, Show, Switch } from "solid-js"
import { createStore } from "solid-js/store"
import { useLanguage } from "@/context/language"
import { usePlatform } from "@/context/platform"
import { useWslServers } from "./context"
import { enterWslclisisCoderStep } from "./settings-model"

type WslServerStep = "wsl" | "distro" | "clisisCoder"

const STEPS: WslServerStep[] = ["wsl", "distro", "clisisCoder"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const clisisCoderCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.clisisCoderChecks[distro] ?? null
  })
  ...
  const installingClisisCoder = createMemo(() => {
    const job = current()?.job
    return job?.kind === "install-clisisCoder" && job.distro === selectedDistro()
  })
  ...
  const clisisCoderReady = createMemo(() => {
    const check = clisisCoderCheck()
    return !!check?.resolvedPath && !check.error
  })
  const allReady = createMemo(() => wslReady() && distroReady() && clisisCoderReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-clisisCoder"
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.clisisCoderChecks[distro]) {
      return { key: `probe-clisisCoder:${distro}`, run: () => api.probeClisisCoder(distro) }
    }
    return null
  })
  ...
  const clisisCoderMessage = createMemo(() => {
    const state = current()
    if (!state) return language.t("wsl.onboarding.checkingClisisCoder")
    const distro = selectedDistro()
    if (state.job?.kind === "install-clisisCoder") {
      return distro
        ? language.t("wsl.onboarding.updatingclisisCoderIn", { distro })
        : language.t("wsl.onboarding.updatingClisisCoder")
    }
    if (state.job?.kind === "probe-clisisCoder") {
      return distro
        ? language.t("wsl.onboarding.checkingclisisCoderIn", { distro })
        : language.t("wsl.onboarding.checkingClisisCoder")
    }
    if (clisisCoderCheck()?.error) return clisisCoderCheck()!.error
    if (clisisCoderCheck()?.matchesDesktop === false) {
      return distro
        ? language.t("wsl.onboarding.updateclisisCoderIn", { distro })
        : language.t("wsl.onboarding.updateClisisCoder")
    }
    if (clisisCoderReady()) {
      return distro
        ? language.t("wsl.onboarding.clisisCoderReadyIn", { distro })
        : language.t("wsl.onboarding.clisisCoderReady")
    }
    return distro
      ? language.t("wsl.onboarding.installclisisCoderIn", { distro })
  ...
  const openclisisCoderStep = () => {
    const distro = selectedDistro()
    if (!distro) return
    void run(() => enterWslclisisCoderStep(distro, api.probeClisisCoder, (step) => setStore("step", step)))
  }
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.clisis-coder"),
        state:
          active === step
            ? "current"
            : step === "wsl"
              ...
              : step === "distro"
                ? distroReady()
                  ...
                  : index > activeIndex
                    ? "locked"
                    : "warning"
                : clisisCoderCheck()?.matchesDesktop === false
                  ? "warning"
                  : clisisCoderReady()
                    ? "done"
                    : index > activeIndex
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "clisisCoder"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.clisis-coder")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.probeClisisCoder(distro))}
                      >
                        {language.t("wsl.onboarding.refresh")}
                      </Button>
                    </Show>
                    <Show when={!clisisCoderReady() || clisisCoderCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.installClisisCoder(distro))}
                      >
                        <Show when={installingClisisCoder()}>
                          <Spinner class="size-4 shrink-0" />
                        </Show>
                        {clisisCoderCheck()?.resolvedPath
                          ? language.t("wsl.onboarding.updateClisisCoder")
                          : language.t("wsl.onboarding.installClisisCoder")}
                      </Button>
                    </Show>
                  </div>
                </div>
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{clisisCoderMessage()}</div>
                <Show when={clisisCoderCheck()?.matchesDesktop === false ? clisisCoderCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "clisisCoder" && allReady() && selectedDistro()}>
