...
export type WslclisisCoderCheck = {
  distro: string
...
export type WslJob =
  ...
  | { kind: "probe-clisisCoder"; distro: string; startedAt: number }
  | { kind: "install-clisisCoder"; distro: string; startedAt: number }
...
export type WslServersPlatform = {
  ...
  probeClisisCoder(name: string): Promise<void>
  installClisisCoder(name: string): Promise<void>
