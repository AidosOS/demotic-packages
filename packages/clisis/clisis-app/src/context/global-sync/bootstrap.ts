import type {
  ...
  Session,
  Todo,
} from "@demotic/clisis-sdk/v2/client"
import { showToast } from "@/utils/toast"
import { getFilename } from "@demotic/clisis-core/util/path"
import { retry } from "@demotic/clisis-core/util/retry"
...
import { NormalizedProviderListResponse } from "@demotic/clisis-ui/context"
...
export const loadGlobalConfigQuery = (scope: ServerScope, sdk: clisisCoderClient) =>
  queryOptions({
    queryKey: [scope, "config"],
    queryFn: () => retry(() => sdk.global.config.get().then((x) => x.data!)),
  })

export const loadProjectsQuery = (scope: ServerScope, sdk: clisisCoderClient) =>
  queryOptions({
    ...
    queryFn: () =>
      retry(() =>
        sdk.project.list().then((x) => {
          return (x.data ?? [])
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("ClisisCoder-test"))
            .slice()
...
export const loadProvidersQuery = (scope: ServerScope, directory: string | null, sdk: clisisCoderClient) =>
  queryOptions({
    queryKey: [scope, directory, "providers"],
    queryFn: () => retry(() => sdk.provider.list().then((x) => normalizeProviderList(x.data!))),
  })

export const loadAgentsQuery = (scope: ServerScope, directory: string | null, sdk: clisisCoderClient) =>
  queryOptions({
    queryKey: [scope, directory, "agents"],
    queryFn: () => retry(() => sdk.app.agents().then((x) => normalizeAgentList(x.data))),
  })

export const loadPathQuery = (scope: ServerScope, directory: string | null, sdk: clisisCoderClient) =>
  queryOptions<Path>({
