  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://ClisisCoder.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://ClisisCoder.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://ClisisCoder.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://ClisisCoder.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://ClisisCoder.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://ClisisCoder.example.com"),
      ),
    ).toEqual({
