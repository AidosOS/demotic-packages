import type { Config, clisisCoderClient, Path, Project, ProviderAuthResponse, Todo } from "@demotic/clisis-sdk/v2/client"
...
import { createSimpleContext, NormalizedProviderListResponse } from "@demotic/clisis-ui/context"
...
export const loadMcpQuery = (scope: ServerScope, directory: string, sdk: clisisCoderClient) =>
  queryOptions({
    queryKey: [scope, directory, "mcp"] as const,
    queryFn: () => sdk.mcp.status().then((r) => r.data ?? {}),
  })

export const loadLspQuery = (scope: ServerScope, directory: string, sdk: clisisCoderClient) =>
  queryOptions({
...
function makeQueryOptionsApi(
  ...
  sdkFor: (dir: PathKey) => clisisCoderClient,
) {
  return {
...
export function createServerSyncContextInner(_serverSDK?: ServerSDK) {
  ...
  const sdkCache = new Map<string, clisisCoderClient>()
