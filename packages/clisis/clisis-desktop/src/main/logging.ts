...
export async function exportDebugLogs() {
  ...
  const output = join(app.getPath("downloads"), `ClisisCoder-debug-${stamp()}.zip`)
  try {
...
function serverLogRoots() {
  ...
  return [...new Set([join(xdgData, "clisisCoder", "log"), join(app.getPath("userData"), "clisisCoder", "log")])]
}
