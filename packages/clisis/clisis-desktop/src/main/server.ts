...
const SIDECAR_SERVICE_NAME = "clisisCoder server"
...
export function preferAppEnv(userDataPath: string) {
  ...
  Object.assign(process.env, {
    ...
    CLISIS_CODER_EXPERIMENTAL_ICON_DISCOVERY: "true",
    CLISIS_CODER_EXPERIMENTAL_FILEWATCHER: "true",
...
export async function checkHealth(url: string, password?: string | null): Promise<boolean> {
  ...
  if (password) {
    const auth = Buffer.from(`clisisCoder:${password}`).toString("base64")
...
function createSidecarEnv(): Record<string, string> {
  ...
  if (!app.isPackaged) env.CLISIS_CODER_DISABLE_CHANNEL_DB = "1"
  return env
}
