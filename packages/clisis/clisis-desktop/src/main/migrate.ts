...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.clisis-coder.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `clisis-coder.settings.dat` is special: it maps to the `clisis-coder.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"clisis-coder.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // ClisisCoder.settings.dat â†’ the electron settings store ("clisis-coder.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "clisis-coder.global.dat").
  const storeName = filename === "clisis-coder.settings.dat" ? "clisis-coder.settings" : filename
