...
import { type WslCommandLine, resolveWslClisisCoder, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const clisisCoder = await resolveWslClisisCoder(distro)
  if (!clisisCoder) throw new Error(`clisisCoder is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "clisisCoder"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export CLISIS_CODER_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export CLISIS_CODER_CLIENT=desktop",
    `export CLISIS_CODER_SERVER_USERNAME=${shellEscape(username)}`,
    `export CLISIS_CODER_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(clisisCoder)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
