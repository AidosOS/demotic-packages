...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-probe-clisisCoder", (_event: IpcMainInvokeEvent, name: string) =>
    controller.probeClisisCoder(requireWslIpcString("distro", name)),
  )
  ipcMain.handle("wsl-servers-install-clisisCoder", (_event: IpcMainInvokeEvent, name: string) =>
    controller.installClisisCoder(requireWslIpcString("distro", name)),
  )
