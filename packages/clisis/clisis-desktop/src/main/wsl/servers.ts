...
import { expectclisisCoderVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
import { clearWslDistroState, wslServerIdToRestart } from "./policy"
import {
  installWslDistro,
  installWslClisisCoder,
...
export function createWslServersController(appVersion: string, spawnSidecar: SpawnSidecar, logger?: ControllerLogger) {
  ...
  const setclisisCoderCheck = (distro: string, check: WslclisisCoderCheck) => {
    setState({
      clisisCoderChecks: {
        ...state.clisisCoderChecks,
        [distro]: check,
      },
    })
  }

  const refreshclisisCoderCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await resolveWslClisisCoder(distro, opts)
    const version = resolved ? await readWslCommandVersion(resolved, distro, opts) : null
    setclisisCoderCheck(distro, clisisCoderCheck(distro, resolved, version, appVersion))
  }
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      void refreshclisisCoderCheck(item.config.distro).catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl clisisCoder check failed", { id, distro: item.config.distro, message })
      })
  ...
  return {
    ...
    async probeClisisCoder(name: string) {
      await runJob({ kind: "probe-clisisCoder", distro: name, startedAt: Date.now() }, async (abort) => {
        await refreshclisisCoderCheck(name, { signal: abort.signal })
      })
    },

    async installClisisCoder(name: string) {
      await runJob({ kind: "install-clisisCoder", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslClisisCoder(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "clisisCoder installation failed")
        }
        await refreshclisisCoderCheck(name, { signal: abort.signal })
        expectclisisCoderVersion(state.clisisCoderChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.clisisCoderChecks, distro) : {}),
      })
    },
...
function clisisCoderCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "clisisCoder is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "clisisCoder is installed but could not run",
    }
  }
  return {
    distro,
