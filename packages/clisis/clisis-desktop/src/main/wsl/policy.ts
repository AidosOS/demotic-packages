import type { WslDistroProbe, WslclisisCoderCheck, WslServerItem } from "../../preload/types"
...
export function clearWslDistroState(
  ...
  clisisCoderChecks: Record<string, WslclisisCoderCheck>,
  distro: string,
) {
  const nextDistroProbes = { ...distroProbes }
  const nextclisisCoderChecks = { ...clisisCoderChecks }
  delete nextDistroProbes[distro]
  delete nextclisisCoderChecks[distro]
  return { distroProbes: nextDistroProbes, clisisCoderChecks: nextclisisCoderChecks }
}
