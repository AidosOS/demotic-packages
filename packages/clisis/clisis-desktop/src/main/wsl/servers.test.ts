...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectclisisCoderVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectclisisCoderVersion("1.14.35", "1.16.2")).toThrow(
    "clisisCoder update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating clisisCoder", () => {
  expect(
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.clisis-coder/bin/clisisCoder",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, clisisCoderChecks: {} })
})
