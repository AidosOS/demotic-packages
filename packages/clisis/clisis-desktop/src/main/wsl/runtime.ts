...
export async function installWslClisisCoder(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
      ["bash", "-lc", `curl -fsSL https://clisiscoder.ai/install | bash -s -- --version ${shellEscape(version)}`],
      distro,
    ),
    withTimeout(opts, DEFAULT_WSL_INSTALL_TIMEOUT_MS),
    DEFAULT_WSL_INSTALL_TIMEOUT_MS,
...
export async function resolveWslClisisCoder(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        'if [ -x "$HOME/.clisis-coder/bin/clisisCoder" ]; then printf "%s\\n" "$HOME/.clisis-coder/bin/clisisCoder"; fi',
        distro,
        opts,
      )
    ).stdout,
