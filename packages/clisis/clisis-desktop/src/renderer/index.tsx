    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.CLISIS_CODER_CHANNEL === "prod" &&
...
const deepLinkEvent = "clisisCoder:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__CLISIS_CODER__ ??= {}
  const pending = window.__CLISIS_CODER__.deepLinks ?? []
  window.__CLISIS_CODER__.deepLinks = [...pending, ...urls]
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://clisiscoder.ai/favicon-96x96-v3.png",
      })
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("clisis-coder.global.dat").getItem("language")
