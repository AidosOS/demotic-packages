...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      CLISIS_CODER_CLI_NAME: `'${binary}'`,
      CLISIS_CODER_MODELS_DEV: modelsData,
      CLISIS_CODER_CHANNEL: `'${Script.channel}'`,
      CLISIS_CODER_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
  ...
  await Bun.write(
    ...
    JSON.stringify(
      {
        name: `@demotic/clisis-${name}`,
        version: Script.version,
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/AidosOS/demotic-clisis-coder.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
