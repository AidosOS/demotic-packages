...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createClisisCoderClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@demotic/clisis-cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createClisisCoderClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createClisisCoderClient({ baseUrl: connection.url, headers: connection.headers })
    })
