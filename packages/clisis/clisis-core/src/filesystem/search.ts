...
export class Service extends Context.Service<Service, Interface>()("@demotic/clisis-Search") {}
...
export const layer: Layer.Layer<Service, never, FSUtil.Service | Ripgrep.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const acquire = Effect.fn("Search.acquire")(function* (cwd: string) {
      // The clisisCoder test runtime owns an isolated XDG tree that Windows must
      // remove before process exit, so use ripgrep instead of native FFF there.
      if (process.env.CLISIS_CODER_TEST_HOME) return undefined
      ...
      return yield* Effect.gen(function* () {
        ...
        const made = yield* fffSync("create picker", () =>
          Fff.create({
            ...
            // home directory, if the user specifically opened clisisCoder at the
    ...
    // in 99% of use cases user that is opened clisisCoder at certain directory will
