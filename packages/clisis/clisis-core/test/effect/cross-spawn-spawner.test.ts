...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "ClisisCoder-core-test-"))
  return {
    path: dir,
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["CLISIS_CODER_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { CLISIS_CODER_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("CLISIS_CODER_TEST_SHELL=ok")
      }),
    )
