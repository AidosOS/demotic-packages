...
import { AbsolutePath, Location, Model, clisisCoder, Session, Tool } from "@demotic/clisis-core/public"
import { tmpdir } from "./fixture/tmpdir"
import { testEffect } from "./lib/effect"

const it = testEffect(ClisisCoder.layer)

describe("public native clisisCoder API", () => {
  it.effect("exposes only the intentional Session capabilities", () =>
    Effect.gen(function* () {
      const clisisCoder = yield* ClisisCoder.Service

      expect(Object.keys(clisisCoder).sort()).toEqual(["sessions", "tools"])

      expect(Object.keys(ClisisCoder.sessions).sort()).toEqual([
        "context",
        "create",
        "events",
        "get",
      ...
      expect(yield* ClisisCoder.sessions.list()).toBeArray()
      yield* ClisisCoder.tools.register({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          yield* ClisisCoder.sessions.create({
            id: sessionID,
            location: Location.Ref.make({ directory: AbsolutePath.make(tmp.path) }),
          })

          yield* ClisisCoder.sessions.switchModel({ sessionID, model })

          expect((yield* ClisisCoder.sessions.get(sessionID)).model).toEqual(model)
        }),
      ),
    ),
  )
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap(([available, disabled]) =>
        Effect.gen(function* () {
          ...
          yield* ClisisCoder.sessions.switchModel({ sessionID: availableID, model: ref({ variant: "default" }) })
          const disabledError = yield* ClisisCoder.sessions
            .switchModel({ sessionID: disabledID, model: ref() })
            .pipe(Effect.flip)
          const missingError = yield* ClisisCoder.sessions
          ...
          expect((yield* ClisisCoder.sessions.get(availableID)).model).toEqual(ref({ variant: "default" }))
          expect((yield* ClisisCoder.sessions.get(disabledID)).model).toBeUndefined()
        }),
      ),
    ),
  )
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          yield* ClisisCoder.sessions.switchModel({ sessionID, model: selected })

          const error = yield* ClisisCoder.sessions
            .switchModel({ sessionID, model: ref({ variant: "unknown" }) })
            .pipe(Effect.flip)

          expect(error).toBeInstanceOf(Session.VariantUnavailableError)
          expect((yield* ClisisCoder.sessions.get(sessionID)).model).toEqual(selected)
        }),
      ),
    ),
  )
