...
import { ConfigReference } from "@demotic/clisis-core/config/reference"
...
import { ProjectReference } from "@demotic/clisis-core/project-reference"
import { Repository } from "@demotic/clisis-core/repository"
import { RepositoryCache } from "@demotic/clisis-core/repository-cache"
...
describe("ProjectReference", () => {
  it.live("uses the broad experimental flag unless references are explicitly configured", () =>
    withEnv(
      { CLISIS_CODER_EXPERIMENTAL: "true", CLISIS_CODER_EXPERIMENTAL_REFERENCES: undefined },
      Effect.sync(() => {
        expect(Flag.CLISIS_CODER_EXPERIMENTAL_REFERENCES).toBe(true)
      }),
    ).pipe(
      Effect.flatMap(() =>
        withEnv(
          { CLISIS_CODER_EXPERIMENTAL: "true", CLISIS_CODER_EXPERIMENTAL_REFERENCES: "false" },
          Effect.sync(() => {
            expect(Flag.CLISIS_CODER_EXPERIMENTAL_REFERENCES).toBe(false)
          }),
        ),
      ),
    ),
...
function withReferences<A, E, R>(body: Effect.Effect<A, E, R>) {
  return withEnv({ CLISIS_CODER_EXPERIMENTAL_REFERENCES: "true" }, body)
}

function withoutReferences<A, E, R>(body: Effect.Effect<A, E, R>) {
  return withEnv({ CLISIS_CODER_EXPERIMENTAL: undefined, CLISIS_CODER_EXPERIMENTAL_REFERENCES: undefined }, body)
}
