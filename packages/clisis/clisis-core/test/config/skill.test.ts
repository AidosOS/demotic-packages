...
import { ConfigSkillPlugin } from "@demotic/clisis-core/config/plugin/skill"
    ...
    Effect.gen(function* () {
      ...
      yield* ConfigSkillPlugin.Plugin.effect.pipe(
        Effect.provideService(
          ...
          Config.Service.of({
            entries: () =>
              Effect.succeed([
                new Config.Directory({ type: "directory", path: AbsolutePath.make("/repo/.clisis-coder") }),
                new Config.Document({
                  type: "document",
                  info: decode({
      ...
      expect(sources).toEqual([
        new SkillV2.DirectorySource({
          type: "directory",
          path: AbsolutePath.make(path.join("/repo/.clisis-coder", "skill")),
        }),
        new SkillV2.DirectorySource({
          type: "directory",
          path: AbsolutePath.make(path.join("/repo/.clisis-coder", "skills")),
        }),
