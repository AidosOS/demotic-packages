...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@demotic/clisis-acme")).toBe("@demotic/clisis-acme")
    expect(Npm.sanitize("@demotic/clisis-acme@1.0.0")).toBe("@demotic/clisis-acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/clisisCoder/acme.git"
    const expected = win ? "acme@git+https_//github.com/clisisCoder/acme.git" : spec
