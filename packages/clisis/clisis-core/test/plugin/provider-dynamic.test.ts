...
import { AISDK } from "@demotic/clisis-core/aisdk"
...
import { DynamicProviderPlugin } from "@demotic/clisis-core/plugin/provider/dynamic"
...
function tempEntrypoint(source: string) {
  return Effect.acquireRelease(
    Effect.promise(async () => {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "ClisisCoder-provider-dynamic-"))
