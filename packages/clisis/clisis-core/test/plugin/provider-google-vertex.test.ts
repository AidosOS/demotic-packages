...
import { GoogleVertexPlugin } from "@demotic/clisis-core/plugin/provider/google-vertex"
  ...
  it.effect("ignores OpenAI-compatible providers that are not Google Vertex", () =>
    Effect.gen(function* () {
      ...
      yield* transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.clisisCoder, (provider) => {
          provider.api = {
            type: "aisdk",
            package: "@ai-sdk/openai-compatible",
            url: "https://clisiscoder.ai/zen/v1",
          }
        }),
      )

      const provider = yield* catalog.provider.get(ProviderV2.ID.clisisCoder)
