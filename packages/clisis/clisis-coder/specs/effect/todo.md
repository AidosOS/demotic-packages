...
Short roadmap for Effect cleanup in `packages/ClisisCoder`.
...
- `ERR` Typed errors â€” replace legacy `NamedError.create(...)` and
...
- `RENDER` User-visible error rendering â€” preserve structured typed-error
  details at CLI, HTTP, and tool boundaries.
  Shrinks: opaque `Error: Name` rendering.
- `HTTP` HTTP route cleanup â€” make route errors explicit instead of
...
- `TEST` Effect test migration â€” use `testEffect`, `it.live`, and
  `it.instance` with explicit layers.
  Shrinks: Promise-style tests, sleeps, mutable global test flags.
- `RF` RuntimeFlags / Flag deletion â€” move mutable
...
- `GLOBAL` Global paths / import side effects â€” make global path state
...
- `INST` Instance context â€” keep project context explicit through Effect refs
  and bridge boundaries.
- `BRIDGE` Promise/callback interop â€” keep bridge helpers, but reduce
  legacy ALS coupling.
  Shrinks: ad hoc Promise/callback re-entry code.
- `PROC` AppProcess migration â€” prefer `AppProcess.Service` over raw
  process wrappers.
  Shrinks: direct spawn callsites and legacy process helpers.
- `FS` FSUtil migration â€” prefer `FSUtil.Service` over raw
  filesystem APIs.
  Shrinks: direct `fs` / `Bun.file` service callsites where inappropriate.
- `RT` Runtime/facade cleanup â€” remove service-local `makeRuntime`
...
- `OA` OpenAPI compatibility â€” tighten source schemas instead of
...
- HTTP error middleware still guesses status codes from error names â€”
...
- [ ] `ERR-4` Sweep remaining `NamedError.create(...)` and
      `Effect.die(...)` callsites for expected failures â€” re-run `git
...
- [ ] Sweep lingering `Flag.*` reads â€” many CLI/TUI/config/observability
...
- `Global.make()` still reads mutable `Flag.ClisisCoder_CONFIG_DIR`.
...
- `PROC` / `FS` â€” continue AppProcess and FSUtil migrations as
  focused PRs when touching relevant files.
- `RT` â€” remove service-local runtime facades only when they are not an
  intentional boundary.
- `OA` â€” shrink [`public.ts`](../../src/server/routes/instance/httpapi/public.ts)
  by tightening source schemas one workaround at a time.
- `fetch` â†’ `HttpClient` â€” migrate raw fetch callsites when the caller is
  already effectful or being effectified.
- `Tools` â€” remaining tool cleanup is narrow: `webfetch` HTML extraction
