...
Practical reference for the current tool-migration state in `packages/ClisisCoder`.
...
Tool tests should use the existing Effect helpers in `packages/ClisisCoder/test/lib/effect.ts`:
...
- [x] `read.ts` â€” streams through `FSUtil.Service.stream` with `Stream.splitLines`; the legacy Node stream / `readline` helper is gone
- [ ] `bash.ts` â€” already uses Effect child-process primitives; only keep tracking shell-specific platform bridges and parser/loading details as they come up
- [ ] `webfetch.ts` â€” already uses `HttpClient`; remaining work is limited to smaller boundary helpers like HTML text extraction
- [ ] `file/ripgrep.ts` â€” adjacent to tool migration; still has raw fs/process usage that affects `grep.ts` and file-search routes
- [x] `patch/index.ts` â€” apply path now returns `Effect` over `FSUtil.Service`; the parser and chunk replacer stay pure
