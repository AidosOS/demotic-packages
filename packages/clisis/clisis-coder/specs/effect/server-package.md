...
Practical reference for a future `packages/server` split after the ClisisCoder
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/ClisisCoder`.
...
Until enough shared service code lives outside `packages/ClisisCoder`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/ClisisCoder`

It should not import `packages/ClisisCoder` services while `packages/ClisisCoder`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on ClisisCoder-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/ClisisCoder` implementation details.
