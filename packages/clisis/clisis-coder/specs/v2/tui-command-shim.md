...
Delete `packages/ClisisCoder/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/ClisisCoder/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/ClisisCoder/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/ClisisCoder`
- TUI plugin loader tests in `packages/ClisisCoder` if runtime plugin API wiring changed
