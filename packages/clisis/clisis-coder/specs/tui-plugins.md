...
- Author package entrypoint is `@demotic/clisis-plugin/tui`.
...
```json
{
  "$schema": "https://clisiscoder.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/ClisisCoder-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
    ...
    "sound": true,
    "volume": 0.4,
    "sound_pack": "clisis-coder.default",
    "sounds": {
...
- Import types from `@demotic/clisis-plugin/tui`.
- `@demotic/clisis-plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@demotic/clisis-plugin/tui"
...
- The value is a semver range checked against the running ClisisCoder version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.ClisisCoder` is absent, no check is performed (backward compatible).
...
- `clisis-coder plugin <module>` and TUI install both run install â†’ manifest read â†’ config patch.
- Alias: `clisis-coder plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.ClisisCoder` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.ClisisCoder`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.ClisisCoder`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.ClisisCoder` dir or `ClisisCoder_CONFIG_DIR`, ClisisCoder installs `@demotic/clisis-plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@demotic/clisis-plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Clisis Coder's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Clisis Coder registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@demotic/clisis-plugin/tui`.
...
- `message` is required; `title` defaults to `"ClisisCoder"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `ClisisCoder.default`.
...
- Local plugins persist installed themes under the local `.ClisisCoder/themes` area near the plugin config source.
...
- `--pure` / `CLISIS_CODER_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.ClisisCoder/plugins/tui-smoke.tsx`
- Local vim plugin: `.ClisisCoder/plugins/tui-vim.tsx`
- Local smoke config: `.ClisisCoder/tui.json`
- Local smoke theme: `.ClisisCoder/plugins/smoke-theme.json`
