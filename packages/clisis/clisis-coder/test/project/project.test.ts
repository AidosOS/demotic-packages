...
import { ProjectCopy } from "@demotic/clisis-core/project/copy"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const clisisCoderFile = path.join(tmp, ".git", "clisisCoder")
      expect(yield* Effect.promise(() => Bun.file(clisisCoderFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "clisisCoder")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "clisisCoder")
      const wrongCache = path.join(parentDir, ".git", "clisisCoder")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "clisisCoder")
      const cacheB = path.join(bareB, "clisisCoder")
      const wrongCache = path.join(parentDir, ".git", "clisisCoder")
