...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/clisiscoder.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/clisiscoder.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/clisiscoder.jsonc", dir: ".clisis-coder", suggestion: "clisisCoder" },
        expected:
          'Directory ".clisis-coder" in /tmp/clisiscoder.jsonc is not valid. Rename the directory to "clisisCoder" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/clisiscoder.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/clisiscoder.jsonc: schema mismatch\nâ†³ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.clisiscoder.demoticsuite.ai/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.clisiscoder.demoticsuite.ai/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `clisisCoder models` to list available models",
      "Or check your config (clisiscoder.json) provider/model names",
    ].join("\n")
