...
import { clisisCoderClient, type GlobalEvent } from "@demotic/clisis-sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<clisisCoderClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<clisisCoderClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<clisisCoderClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<clisisCoderClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<clisisCoderClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://ClisisCoder.test"),
...
function sdk(
  input: {
    ...
    subscribe?: clisisCoderClient["event"]["subscribe"]
    globalEvent?: clisisCoderClient["global"]["event"]
    promptAsync?: clisisCoderClient["session"]["promptAsync"]
    status?: clisisCoderClient["session"]["status"]
    messages?: clisisCoderClient["session"]["messages"]
    children?: clisisCoderClient["session"]["children"]
    permissions?: clisisCoderClient["permission"]["list"]
    questions?: clisisCoderClient["question"]["list"]
  } = {},
) {
  const client = new clisisCoderClient()

  const subscribe: clisisCoderClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: clisisCoderClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: clisisCoderClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: clisisCoderClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: clisisCoderClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: clisisCoderClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: clisisCoderClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: clisisCoderClient["question"]["list"] = input.questions ?? (() => ok([]))
