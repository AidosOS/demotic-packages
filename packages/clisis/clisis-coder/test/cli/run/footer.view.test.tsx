...
import type { QuestionRequest } from "@demotic/clisis-sdk/v2"
import { clisisCoderKeymapProvider, registerclisisCoderKeymap } from "@demotic/clisis-tui/keymap"
import {
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerclisisCoderKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerclisisCoderKeymap(keymap, renderer, tuiConfig)
  ...
  try {
    await app.renderOnce()
    expect(app.captureCharFrame()).toContain("interrupt â€¢ 1 agent ctrl+x down â€¢ ctrl+b background â€¢ 1 queued ctrl+x q")
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerclisisCoderKeymap(keymap, renderer, tuiConfig)
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "clisisCoder", modelID: "gpt-5" })
  ...
  try {
    ...
    expect(frame).toContain("Search")
    expect(frame).toContain("clisisCoder")
