...
// `CLISIS_CODER_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { cliIt } from "../../lib/cli-process"

describe("clisisCoder run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, clisisCoder }) =>
      Effect.gen(function* () {
        ...
        const result = yield* ClisisCoder.run("say hi")
        ClisisCoder.expectExit(result, 0)
  ...
  // (the prompt was accepted, then the upstream provider failed), clisisCoder
  ...
  cliIt.concurrent(
    ...
    ({ llm, clisisCoder }) =>
      Effect.gen(function* () {
        ...
        const result = yield* ClisisCoder.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, clisisCoder }) =>
      Effect.gen(function* () {
        ...
        const result = yield* ClisisCoder.run("say hi", { format: "json" })
        ClisisCoder.expectExit(result, 0)

        const events = ClisisCoder.parseJsonEvents(result.stdout)
