...
// terminal sizes. The default clisisCoder tui command is excluded â€”
// `clisisCoder --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `clisisCoder --help` prints today;
...
// top-level help on `--help` and exits 1; not a real clisisCoder command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("clisisCoder CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ clisisCoder }) =>
      Effect.gen(function* () {
        const topLevel = yield* ClisisCoder.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* ClisisCoder.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`clisisCoder ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`clisisCoder ${argv.join(" ")} --help`)
        }
