...
import { isZedTerminal, offsetToPosition, resolveZedDbPath, resolveZedSelection } from "@demotic/clisis-tui/editor-zed"
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.CLISIS_CODER_ZED_DB
  process.env.CLISIS_CODER_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.CLISIS_CODER_ZED_DB
    else process.env.CLISIS_CODER_ZED_DB = previous
    home.mockRestore()
  }
})
