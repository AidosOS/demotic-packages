...
describe("clisisCoder acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ clisisCoder }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ clisisCoder }))
        ...
        expect(initialized.agentInfo?.name).toBe("clisisCoder")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ clisisCoder }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ clisisCoder })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("ClisisCoder-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "ClisisCoder-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.CLISIS_CODER_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ clisisCoder }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("ClisisCoder-login")
