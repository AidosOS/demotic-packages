...
describe("clisisCoder acp lifecycle subprocess", () => {
  cliIt.live(
    "stdin EOF exits cleanly",
    ({ clisisCoder }) =>
      Effect.gen(function* () {
        const acp = yield* ClisisCoder.acp()
        acp.close()
  ...
  cliIt.live(
    ...
    ({ home, llm, clisisCoder }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient(
          { clisisCoder },
          { CLISIS_CODER_CONFIG_CONTENT: JSON.stringify(verifierConfig(llm.url)) },
        )
