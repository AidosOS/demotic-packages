...
describe("clisisCoder read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // CLISIS_CODER_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ clisisCoder }) =>
      Effect.gen(function* () {
        const r = yield* ClisisCoder.spawn(["mcp", "list"])
        ClisisCoder.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ clisisCoder }) =>
      Effect.gen(function* () {
        const r = yield* ClisisCoder.spawn(["providers", "list"])
        ClisisCoder.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ clisisCoder }) =>
      Effect.gen(function* () {
        const r = yield* ClisisCoder.spawn(["models"])
        ClisisCoder.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ clisisCoder }) =>
      Effect.gen(function* () {
        const r = yield* ClisisCoder.spawn(["agent", "list"])
        ClisisCoder.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh CLISIS_CODER_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ clisisCoder }) =>
      Effect.gen(function* () {
        const r = yield* ClisisCoder.spawn(["session", "list"])
        ClisisCoder.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ clisisCoder }) =>
      Effect.gen(function* () {
        const r = yield* ClisisCoder.spawn(["stats"])
        ClisisCoder.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under CLISIS_CODER_TEST_HOME / XDG_DATA_HOME.
  // Accept either form â€” both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ clisisCoder }) =>
      Effect.gen(function* () {
        const r = yield* ClisisCoder.spawn(["db", "path"])
        ClisisCoder.expectExit(r, 0, "db path")
