...
const { Npm } = await import("@demotic/clisis-core/npm")
...
function load(dir: string, flags?: Parameters<typeof RuntimeFlags.layer>[0]) {
  const source = path.join(dir, "clisiscoder.json")
  return Effect.gen(function* () {
  ...
  it.live("loads a file:// plugin function export", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(
          path.join(dir, "clisiscoder.json"),
  ...
  it.live("loads npm server plugin from package ./server export", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(path.join(dir, "clisiscoder.json"), JSON.stringify({ plugin: ["acme-plugin@1.0.0"] }, null, 2))
  ...
  it.live("rejects npm server export that resolves outside plugin directory", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(path.join(dir, "clisiscoder.json"), JSON.stringify({ plugin: ["acme-plugin"] }, null, 2))
    ...
    withTmp(
      async (dir) => {
        await Bun.write(
          ...
          JSON.stringify(
            {
              plugin: ["ClisisCoder-openai-codex-auth@1.0.0", "ClisisCoder-copilot-auth@1.0.0", "regular-plugin@1.0.0"],
            },
            null,
            2,
          ),
      ...
      (_tmp) =>
        Effect.gen(function* () {
          ...
          try {
            ...
            expect(pkgs).not.toContain("ClisisCoder-openai-codex-auth@1.0.0")
            expect(pkgs).not.toContain("ClisisCoder-copilot-auth@1.0.0")
          } finally {
            install.mockRestore()
          }
        }),
  ...
  it.live("continues loading plugins when plugin init throws", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(path.join(dir, "clisiscoder.json"), JSON.stringify({ plugin: [file, ok] }, null, 2))
  ...
  it.live("continues loading plugins when plugin import fails", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(path.join(dir, "clisiscoder.json"), JSON.stringify({ plugin: [missing, ok] }, null, 2))
...
`,
        ...
        await Bun.write(path.join(dir, "clisiscoder.json"), JSON.stringify({ plugin: [aSpec, bSpec] }, null, 2))
