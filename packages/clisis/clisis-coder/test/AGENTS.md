...
- `config?: Partial<Config.Info>` - Write an `Clisis Coder.json` config file
...
- Directories are created in the system temp folder with prefix `Clisis Coder-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("Clisis Coder-test-")
  }),
)
```
