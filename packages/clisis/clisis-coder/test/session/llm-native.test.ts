...
import { LLMEvent, ToolFailure } from "@demotic/clisis-llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@demotic/clisis-llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("clisisCoder"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("clisisCoder") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("clisisCoder") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, clisisCoder, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored clisisCoder auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("clisisCoder") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("clisisCoder"),
