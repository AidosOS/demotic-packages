...
describe("session.retry.retryable", () => {
  ...
  test("maps free limits to Go upsell action", () => {
    ...
    expect(SessionRetry.retryable(error, "clisisCoder")).toEqual({
      message: SessionRetry.GO_UPSELL_MESSAGE,
      action: {
        reason: "free_tier_limit",
        provider: "clisisCoder",
        title: "Free limit reached",
        message: "Subscribe to clisisCoder Go for reliable access to the best open-source models, starting at $5/month.",
        label: "subscribe",
  ...
  test("maps Go subscription limits to workspace PAYG upsell", () => {
    ...
    expect(SessionRetry.retryable(error, "ClisisCoder-go")).toEqual({
      message:
        "5 hour usage limit reached. It will reset in 5 hours 23 minutes. To continue using this model now, enable usage from your available balance - https://clisiscoder.ai/workspace/wrk_01K6XGM22R6FM8JVABE9XDQXGH/go",
      action: {
        reason: "account_rate_limit",
        provider: "ClisisCoder-go",
        ...
        label: "open settings",
        link: "https://clisiscoder.ai/workspace/wrk_01K6XGM22R6FM8JVABE9XDQXGH/go",
      },
    })
  })
  ...
  test("maps Go subscription limits without limit metadata", () => {
    ...
    expect(SessionRetry.retryable(error, "ClisisCoder-go")?.action?.message).toBe(
