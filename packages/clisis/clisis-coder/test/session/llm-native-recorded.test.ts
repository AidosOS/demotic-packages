...
import { HttpRecorder } from "@demotic/clisis-http-recorder"
import { HttpRecorderInternal } from "@demotic/clisis-http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@demotic/clisis-llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@demotic/clisis-llm/route"
import { Env } from "@/env"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.CLISIS_CODER_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["clisisCoder", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CLISIS_CODER_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("CLISIS_CODER_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["clisisCoder", "native", "oauth", "tool-loop"],
  ...
  {
    id: "ClisisCoder-proxy",
    name: "clisisCoder proxy",
    providerID: ProviderV2.ID.clisisCoder,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["clisisCoder", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.CLISIS_CODER_RECORD_CONSOLE_TOKEN && process.env.CLISIS_CODER_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.clisisCoder,
        name: "clisisCoder Zen",
        env: ["CLISIS_CODER_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.CLISIS_CODER_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.CLISIS_CODER_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.CLISIS_CODER_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["clisisCoder", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CLISIS_CODER_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("CLISIS_CODER_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("CLISIS_CODER_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set CLISIS_CODER_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the clisisCoder LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "clisiscoder.json"),
      JSON.stringify({ $schema: "https://clisiscoder.ai/config.json", ...scenario.config(model) }),
    ),
  )
