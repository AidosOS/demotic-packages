// Subprocess test harness for the clisisCoder CLI. Spawns the real binary against
...
//
// Configuration flows through clisisCoder's built-in test affordances:
//   - CLISIS_CODER_CONFIG_CONTENT      : provider config inline, no files to find
//   - CLISIS_CODER_TEST_HOME           : pins os.homedir() â†’ tmpdir
//   - CLISIS_CODER_DISABLE_PROJECT_CONFIG : skip walking up for clisiscoder.json
//   - CLISIS_CODER_PURE                : skip external plugin discovery + install
//   - CLISIS_CODER_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `clisis-coder.run` is fully wired. The shape supports adding more
// builders (`clisis-coder.serve(opts)`, `clisis-coder.acp(opts)`, `clisis-coder.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape â€” see the TODO at the bottom of clisisCoderCli.
...
const clisisCoderRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(clisisCoderRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    CLISIS_CODER_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    CLISIS_CODER_CONFIG_CONTENT: configJson,
    CLISIS_CODER_DISABLE_PROJECT_CONFIG: "1",
    CLISIS_CODER_PURE: "1",
    CLISIS_CODER_DISABLE_AUTOUPDATE: "1",
    CLISIS_CODER_DISABLE_AUTOCOMPACT: "1",
    CLISIS_CODER_DISABLE_MODELS_FETCH: "1",
    CLISIS_CODER_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `clisisCoder run`. New flags should
...
// `clisisCoder serve` is a long-lived process â€” it never exits on its own.
...
// `clisisCoder acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type clisisCoderCli = {
  // High-level: run a single prompt against the test model. Short-lived.
  readonly run: (message: string, opts?: RunOpts) => Effect.Effect<RunResult>
  // Spawn `clisisCoder serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `clisisCoder acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly clisisCoder: clisisCoderCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("clisis-coder.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("clisis-coder.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "clisisCoder server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `clisisCoder serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("clisis-coder.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const clisisCoder: clisisCoderCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, clisisCoder })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `clisis-coder.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "clisisCoder") {
...
// every call site. Use this for any test that needs the clisisCoder CLI fixture.
//
...
// resources (e.g. `clisis-coder.serve`) without an extra `Effect.scoped` wrapper â€”
