...
import { WorkspaceV2 } from "@demotic/clisis-core/workspace"
import { WorkspaceTable } from "@demotic/clisis-core/control-plane/workspace.sql"
...
const originalEnv = {
  CLISIS_CODER_AUTH_CONTENT: process.env.CLISIS_CODER_AUTH_CONTENT,
  CLISIS_CODER_EXPERIMENTAL_WORKSPACES: process.env.CLISIS_CODER_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.CLISIS_CODER_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@ClisisCoder.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.CLISIS_CODER_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=ClisisCoder-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.CLISIS_CODER_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.CLISIS_CODER_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.CLISIS_CODER_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=ClisisCoder-test")
