...
import { Search } from "@demotic/clisis-core/filesystem/search"
...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".clisis-coder")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .clisis-coder/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const clisisCoder = path.join(test.directory, ".clisis-coder")
      const tool = path.join(clisisCoder, "tool")
  ...
  it.instance("ignores non-tool exports in .clisis-coder/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".clisis-coder", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` â€” `oh-my-clisisCoder` was registering a tool
  ...
  it.instance("loads tools from .clisis-coder/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const clisisCoder = path.join(test.directory, ".clisis-coder")
      const tools = path.join(clisisCoder, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".clisis-coder", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const clisisCoder = path.join(test.directory, ".clisis-coder")
        const customTools = path.join(clisisCoder, "tools")
        const plugin = path.join(clisisCoder, "node_modules", "@clisis-coder", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(clisisCoder, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@demotic/clisis-plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".clisis-coder", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(clisisCoder, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@demotic/clisis-plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(clisisCoder, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(clisisCoder, "node_modules", "cowsay")
