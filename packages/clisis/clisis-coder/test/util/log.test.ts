...
it.live("local dev log is not truncated twice for the same run", () =>
  Effect.gen(function* () {
    const log = Global.Path.log
    const runID = process.env.CLISIS_CODER_RUN_ID
    const initialized = process.env.CLISIS_CODER_LOG_INITIALIZED_RUN_ID
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        Global.Path.log = log
        if (runID === undefined) delete process.env.CLISIS_CODER_RUN_ID
        else process.env.CLISIS_CODER_RUN_ID = runID
        if (initialized === undefined) delete process.env.CLISIS_CODER_LOG_INITIALIZED_RUN_ID
        else process.env.CLISIS_CODER_LOG_INITIALIZED_RUN_ID = initialized
      }),
    )

    const dir = yield* tmpdirScoped()
    Global.Path.log = dir
    process.env.CLISIS_CODER_RUN_ID = "run-1"
    delete process.env.CLISIS_CODER_LOG_INITIALIZED_RUN_ID
