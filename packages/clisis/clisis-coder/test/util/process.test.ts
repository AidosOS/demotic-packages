...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.CLISIS_CODER_TEST ?? "")'), {
      env: {
        CLISIS_CODER_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "CLISIS_CODER_TEST_SHELL"], {
      shell: true,
      env: {
        CLISIS_CODER_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("CLISIS_CODER_TEST_SHELL=ok")
  })
