...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "clisiscoder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "clisiscoder.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "clisiscoder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "clisiscoder.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["clisiscoder.json", "clisiscoder.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "clisiscoder.json"),
        path.join(project, "clisiscoder.jsonc"),
      ])
    })
  })
