...
import { EffectFlock } from "@demotic/clisis-core/util/effect-flock"
...
import { CrossSpawnSpawner } from "@demotic/clisis-core/cross-spawn-spawner"
...
import { ConfigPluginV1 } from "@demotic/clisis-core/v1/config/plugin"
...
function remoteConfigClient(input: {
  ...
  return HttpClient.make((request) => {
    if (request.url.includes(".well-known/clisisCoder")) {
...
const schemaConfig = (config: object) => ({ $schema: "https://clisiscoder.ai/config.json", ...config })
...
const managedConfigDir = process.env.CLISIS_CODER_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.CLISIS_CODER_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.CLISIS_CODER_CONSOLE_TOKEN
  else process.env.CLISIS_CODER_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})

const writeManagedSettingsEffect = (settings: object, filename?: string) =>
  FSUtil.use.writeWithDirs(path.join(managedConfigDir, filename ?? "clisiscoder.json"), JSON.stringify(settings))

async function writeConfig(dir: string, config: object, name = "clisiscoder.json") {
  await Filesystem.write(path.join(dir, name), JSON.stringify(config))
}

const writeConfigEffect = (dir: string, config: object, name = "clisiscoder.json") =>
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.all(
      [
        ...
        input.local ? writeConfigEffect(path.join(directory, ".clisis-coder"), schemaConfig(input.local)) : undefined,
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://clisiscoder.ai/config.json",
      snapshot: false,
    })
...
it.effect("creates global jsonc config with schema when no global configs exist", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const content = yield* FSUtil.use.readFileString(path.join(dir, "clisiscoder.jsonc"))
      expect(content).toContain('"$schema": "https://clisiscoder.ai/config.json"')
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
  ),
)

it.effect("does not create global config when CLISIS_CODER_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
    const custom = yield* tmpdirScoped()
    yield* withGlobalConfig({}, ({ dir }) =>
      withProcessEnv(
        "CLISIS_CODER_CONFIG_DIR",
        custom,
        Effect.gen(function* () {
          yield* Config.use.get().pipe(provideInstanceEffect(dir))

          expect(yield* FSUtil.use.existsSafe(path.join(dir, "clisiscoder.jsonc"))).toBe(false)
        }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
      ),
    )
  }),
...
it.instance("updates config and preserves empty shell sentinel", () =>
  Effect.gen(function* () {
    ...
    yield* writeConfigEffect(
      test.directory,
      { $schema: "https://clisiscoder.ai/config.json", shell: "bash" },
      "config.json",
    )
...
it.effect("updates global config and omits empty shell key in json", () =>
  withGlobalConfig({ config: { shell: "bash" } }, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const writtenConfig = yield* FSUtil.use.readJson(path.join(dir, "clisiscoder.json"))
...
it.effect("updates global config and omits empty shell key in jsonc", () =>
  withGlobalConfig({ config: { shell: "bash", model: "test/model" }, name: "clisiscoder.jsonc" }, ({ dir }) =>
    Effect.gen(function* () {
      yield* Config.use.updateGlobal({ shell: "" })

      const file = path.join(dir, "clisiscoder.jsonc")
...
it.instance("ignores legacy tui keys in clisisCoder config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
...
it.instance("loads JSONC config file", () =>
  Effect.gen(function* () {
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, "clisiscoder.jsonc"),
      `{
        // This is a comment
        "$schema": "https://clisiscoder.ai/config.json",
        "model": "test/model",
...
it.instance("preserves env variables when adding $schema to config", () =>
  withProcessEnv(
    ...
    Effect.gen(function* () {
      ...
      yield* FSUtil.use.writeWithDirs(
        path.join(test.directory, "clisiscoder.json"),
      ...
      const content = yield* FSUtil.use.readFileString(path.join(test.directory, "clisiscoder.json"))
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { clisisCoder: { options: { apiKey: "{env:CLISIS_CODER_CONSOLE_TOKEN}" } } },
        }),
      ),
...
accountTokenIt.instance("resolves env templates in account config with account token", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(config.provider?.["clisisCoder"]?.options?.apiKey).toBe("st_test_token")
  }),
)
...
it.instance("throws error for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(path.join(test.directory, "clisiscoder.json"), "{ invalid json }")
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
it.instance("loads config from .ClisisCoder directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".clisis-coder", "agent", "test.md"),
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".clisis-coder", "agent", "ordered.md"),
      `---
permission:
  bash: allow
  "*": deny
...
it.instance("loads agents from .clisis-coder/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".clisis-coder", "agents", "helper.md"),
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".clisis-coder", "agents", "nested", "child.md"),
      `---
model: test/model
mode: subagent
---
...
it.instance("loads commands from .clisis-coder/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".clisis-coder", "command", "hello.md"),
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".clisis-coder", "command", "nested", "child.md"),
      `---
...
it.instance("loads commands from .clisis-coder/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".clisis-coder", "commands", "hello.md"),
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".clisis-coder", "commands", "nested", "child.md"),
      `---
description: Nested command
---
Nested command template`,
...
it.effect("does not try to install dependencies in read-only CLISIS_CODER_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("CLISIS_CODER_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
)

it.effect("installs dependencies in writable CLISIS_CODER_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.effect("global config remains global when project config is disabled", () =>
  withConfigTree(
    ...
    withProcessEnv(
      "CLISIS_CODER_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const config = yield* Config.use.get()
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".clisis-coder", "agent", "helper.md"),
      `---
model: test/model
mode: subagent
---
...
// Note: preload.ts sets CLISIS_CODER_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
it.instance("managed jsonc settings override managed json settings", () =>
  Effect.gen(function* () {
    ...
    yield* writeManagedSettingsEffect({ model: "managed/jsonc" }, "clisiscoder.jsonc")
...
it.instance("local .ClisisCoder config can override MCP from project config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://clisiscoder.ai/config.json",
    ...
    yield* FSUtil.use.ensureDir(path.join(test.directory, ".clisis-coder"))
    yield* writeConfigEffect(
      path.join(test.directory, ".clisis-coder"),
      {
        $schema: "https://clisiscoder.ai/config.json",
        mcp: {
          docs: {
...
remoteProjectOverride.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const config = yield* Config.use.get()
      expect(remoteProjectOverride.seen.wellKnown).toBe("https://example.com/.well-known/clisisCoder")
...
trailingSlashWellKnown.it.instance("wellknown URL with trailing slash is normalized", () =>
  Effect.gen(function* () {
    ...
    expect(trailingSlashWellKnown.seen.wellKnown).toBe("https://example.com/.well-known/clisisCoder")
  }),
)
      ...
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            ...
            expect(fetchedUrl).toBe(`${server.url.origin}/.well-known/clisisCoder`)
            expect(config.mcp?.jira?.enabled).toBe(true)
          }),
        ),
      { git: true },
...
const templatedHeaderWellKnown = wellKnown({
  remoteConfig: {
    url: "https://config.example.com/clisiscoder.json",
...
templatedHeaderWellKnown.it.instance("wellknown remote_config supports templated env vars in headers", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(templatedHeaderWellKnown.seen.wellKnown).toBe("https://example.com/.well-known/clisisCoder")
    expect(templatedHeaderWellKnown.seen.remote).toBe("https://config.example.com/clisiscoder.json")
...
const remotePrecedenceWellKnown = wellKnown({
  ...
  remoteConfig: { url: "https://config.example.com/{env:TEST_TOKEN}/clisiscoder.json" },
  remote: {
...
remotePrecedenceWellKnown.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const config = yield* Config.use.get()
      expect(remotePrecedenceWellKnown.seen.remote).toBe("https://config.example.com/test-token/clisiscoder.json")
      expect(config.mcp?.confluence?.enabled).toBe(true)
    }),
)
...
const nullConfigWellKnown = wellKnown({
  wellKnown: {
    config: null,
    remote_config: { url: "https://config.example.com/clisiscoder.json" },
  },
  remote: {
    mcp: { confluence: { type: "remote", url: "https://confluence.example.com/mcp", enabled: true } },
  },
...
nullConfigWellKnown.it.instance("wellknown config null is treated as absent", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(nullConfigWellKnown.seen.remote).toBe("https://config.example.com/clisiscoder.json")
    expect(config.mcp?.confluence?.enabled).toBe(true)
  }),
)

const invalidRemoteWellKnown = wellKnown({
  remoteConfig: { url: "https://config.example.com/clisiscoder.json" },
...
invalidRemoteWellKnown.it.instance("wellknown remote_config rejects non-object config responses", () =>
  Effect.gen(function* () {
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(invalidRemoteWellKnown.seen.remote).toBe("https://config.example.com/clisiscoder.json")
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    const file = path.join(tmp.path, "clisiscoder.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-clisisCoder@2.4.3", file)).toBe("oh-my-clisisCoder@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-clisisCoder@2.4.3", "file:///project/.clisis-coder/plugin/oh-my-ClisisCoder.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.clisis-coder/plugin/demo.ts", "file:///project/.clisis-coder/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.clisis-coder/plugin/demo.ts"])
  })
  ...
  it.effect("loads auto-discovered local plugins as file urls", () =>
    withConfigTree(
      ...
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".clisis-coder", "plugin", "my-plugin.js"),
          "export default {}",
        )
...
describe("CLISIS_CODER_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
  ...
  it.instance("skips project .clisis-coder/ directories when flag is set", () =>
    withProcessEnv(
      "CLISIS_CODER_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".clisis-coder", "command", "test-cmd.md"),
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { CLISIS_CODER_CONFIG_DIR: undefined, CLISIS_CODER_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "CLISIS_CODER_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { CLISIS_CODER_DISABLE_PROJECT_CONFIG: "true", CLISIS_CODER_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed CLISIS_CODER_PERMISSION JSON used to crash
...
describe("CLISIS_CODER_PERMISSION env var", () => {
  it.instance("does not crash when CLISIS_CODER_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "CLISIS_CODER_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
        const config = yield* Config.use.get()
...
describe("CLISIS_CODER_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in CLISIS_CODER_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
        "CLISIS_CODER_CONFIG_CONTENT",
        JSON.stringify({
          $schema: "https://clisiscoder.ai/config.json",
  ...
  it.instance("substitutes {file:} tokens in CLISIS_CODER_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "clisisCoder Managed",
          PayloadIdentifier: "ai.clisis-coder.managed.test",
          PayloadType: "ai.clisis-coder.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    ConfigV1.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://clisiscoder.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://clisiscoder.ai/config.json")
})
