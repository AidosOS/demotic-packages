...
const globalConfigFiles = ["clisiscoder.json", "clisiscoder.jsonc", "tui.json", "tui.jsonc"].map((file) =>
  path.join(Global.Path.config, file),
)

const cleanState = Effect.gen(function* () {
  const fs = yield* FSUtil.Service
  delete process.env.CLISIS_CODER_CONFIG
  delete process.env.CLISIS_CODER_TUI_CONFIG
...
it.instance("keeps server and tui plugin merge semantics aligned", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const local = path.join(test.directory, ".clisis-coder")
      yield* fs.makeDirectory(local, { recursive: true })

      yield* fs.writeJson(path.join(Global.Path.config, "clisiscoder.json"), {
        plugin: [["shared-plugin@1.0.0", { source: "global" }], "global-only@1.0.0"],
      })
      yield* fs.writeJson(path.join(Global.Path.config, "tui.json"), {
        plugin: [["shared-plugin@1.0.0", { source: "global" }], "global-only@1.0.0"],
      })
      yield* fs.writeJson(path.join(local, "clisiscoder.json"), {
  ...
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(
        path.join(test.directory, ".clisis-coder", "tui.json"),
...
it.instance("migrates tui-specific keys from clisiscoder.json when tui.json does not exist", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      const source = path.join(test.directory, "clisiscoder.json")
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "clisiscoder.json.tui-migration.bak"))).toBe(true)
...
it.instance("migrates project legacy tui keys even when global tui.json already exists", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const test = yield* TestInstance
      yield* fs.writeJson(path.join(Global.Path.config, "tui.json"), { theme: "global" })
      yield* fs.writeJson(path.join(test.directory, "clisiscoder.json"), {
      ...
      const server = JSON.parse(yield* fs.readFileString(path.join(test.directory, "clisiscoder.json")))
      expect(server.theme).toBeUndefined()
      expect(server.tui).toBeUndefined()
    }),
  ),
...
it.instance("skips migration when clisiscoder.jsonc is syntactically invalid", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
  ...
  "username": "still-broken"
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "clisiscoder.jsonc.tui-migration.bak"))).toBe(false)
      const source = yield* fs.readFileString(path.join(test.directory, "clisiscoder.jsonc"))
...
it.instance("skips migration when tui.json already exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      yield* fs.writeJson(path.join(test.directory, "clisiscoder.json"), { theme: "legacy" })
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "clisiscoder.json.tui-migration.bak"))).toBe(false)
    }),
  ),
)
...
it.instance("migration backup preserves JSONC comments", () =>
  ...
  "tui": {
    ...
    "scroll_speed": 1.5
      ...
      const backup = yield* fs.readFileString(path.join(test.directory, "clisiscoder.jsonc.tui-migration.bak"))
...
it.instance("migrates legacy tui keys across multiple clisiscoder.json levels", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      ...
      yield* fs.writeJson(path.join(test.directory, "clisiscoder.json"), { theme: "root-theme" })
      yield* fs.writeJson(path.join(nested, "clisiscoder.json"), { theme: "nested-theme" })
...
it.instance("project config takes precedence over CLISIS_CODER_TUI_CONFIG (matches CLISIS_CODER_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("CLISIS_CODER_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from CLISIS_CODER_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
      ...
      yield* fs.writeJson(path.join(customDir, "clisiscoder.json"), { model: "test/model" })
      yield* fs.writeJson(path.join(customDir, "tui.json"), { theme: "should-not-load" })

      yield* withEnv(
        "CLISIS_CODER_CONFIG",
        path.join(customDir, "clisiscoder.json"),
        Effect.gen(function* () {
          const config = yield* getTuiConfig(test.directory)
          expect(config.theme).toBeUndefined()
        }),
...
it.instance("loads .clisis-coder/tui.json", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("silently skips malformed tui.json - load failures degrade to {}", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(path.join(test.directory, ".clisis-coder", "tui.json"), JSON.stringify({ theme: "fallback" }))
