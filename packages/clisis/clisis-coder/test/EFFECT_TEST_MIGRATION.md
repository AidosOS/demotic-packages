...
it.instance("instance-local behavior", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("ClisisCoder-test-")
  }),
)
...
- `it.effect(...)` â€” pure Effect behavior with `TestClock` and
  `TestConsole`.
- `it.instance(...)` â€” service behavior that needs one scoped ClisisCoder
  instance.
- `it.live(...)` â€” real time, filesystem mtimes, child processes, git,
...
9. Run the focused test file and `bun typecheck` from `packages/ClisisCoder`.
...
- `test/effect/instance-state.test.ts` â€” scoped directories, instance
  switching, disposal, and concurrency.
- `test/bus/bus-effect.test.ts` â€” `Deferred`, streams, scoped fibers.
- `test/agent/plugin-agent-regression.test.ts` â€” real service layers plus
  fake boundary layers.
- `test/account/service.test.ts` â€” service-level live tests, typed errors,
  fake HTTP clients.
...
```bash
git grep -n "Effect.runPromise\|ManagedRuntime\|Promise.withResolvers\|Bun.sleep\|withTestInstance" -- packages/ClisisCoder/test
```
