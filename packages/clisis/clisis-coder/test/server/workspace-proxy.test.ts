...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips ClisisCoder-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      const request = HttpServerRequest.fromWeb(
        new Request("http://localhost/test", {
          headers: {
            ...
            "x-clisis-coder-workspace": "ws_123",
            "x-custom": "preserved",
          },
        }),
      )
      ...
      expect(forwarded["x-clisis-coder-directory"]).toBeUndefined()
      expect(forwarded["x-clisis-coder-workspace"]).toBeUndefined()
