 ...
 *
 * The script intentionally isolates `CLISIS_CODER_DB` before importing modules that touch
...
void (await import("@demotic/clisis-core/util/log")).init({ print: false })
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "clisiscoder.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "clisiscoder.jsonc")).text(),
          )
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-clisis-coder-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-clisis-coder-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "clisisCoder", model: "test" })}`,
      headers: ctx.headers(),
    }))
    .json(200, array, "status"),
