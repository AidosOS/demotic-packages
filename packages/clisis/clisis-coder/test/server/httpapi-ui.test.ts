...
const testStateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      ...
      envPassword: process.env.CLISIS_CODER_SERVER_PASSWORD,
      envUsername: process.env.CLISIS_CODER_SERVER_USERNAME,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("CLISIS_CODER_SERVER_PASSWORD", original.envPassword)
        restoreEnv("CLISIS_CODER_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            CLISIS_CODER_SERVER_PASSWORD: input?.password,
            CLISIS_CODER_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  it.live("serves the web UI through the HTTP API app", () =>
    Effect.gen(function* () {
      ...
      const response = yield* uiApp({
        ...
        client: httpClient(
          new Response("<html>clisisCoder</html>", { headers: { "content-type": "text/html" } }),
          (request) => {
            proxiedUrl = request.url
          },
        ),
      ...
      expect(yield* responseText(response)).toBe("<html>clisisCoder</html>")
      expect(proxiedUrl).toBe("https://app.clisiscoder.demoticsuite.ai/")
    }),
  )
  ...
  it.live("strips upstream transfer encoding headers from proxied assets", () =>
    Effect.gen(function* () {
      ...
      expect(response.status).toBe(200)
      expect(proxiedUrl).toBe("https://app.clisiscoder.demoticsuite.ai/assets/app.js")
            ...
            Layer.succeed(
              ...
              HttpClient.make((request) =>
                Effect.succeed(
                  HttpClientResponse.fromWeb(
                    request,
                    new Response("<html>clisisCoder</html>", {
                      headers: {
  ...
  it.live("accepts auth token for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        username: "clisisCoder",
        disableEmbeddedWebUi: true,
        client: httpClient(new Response("<html>clisisCoder</html>", { headers: { "content-type": "text/html" } })),
      }).request(`/?auth_token=${btoa("clisisCoder:secret")}`)
  ...
  it.live("accepts basic auth for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("clisisCoder:secret")}` },
      })
  ...
  it.live("accepts basic auth passwords containing colons for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("clisisCoder:sec:ret")}` },
      })
  ...
  it.live("allows web UI preflight without auth", () =>
    Effect.gen(function* () {
      const response = yield* app({ password: "secret", username: "clisisCoder" }).request("/", {
        method: "OPTIONS",
        headers: {
