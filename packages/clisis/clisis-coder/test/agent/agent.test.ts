...
it.instance("plan agent denies edits except .clisis-coder/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".clisis-coder/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".clisis-coder", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.CLISIS_CODER_TEST_HOME
      process.env.CLISIS_CODER_TEST_HOME = test.directory
