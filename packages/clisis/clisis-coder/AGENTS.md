...
- Running `bun dev` from `packages/Clisis Coder` starts the live interactive TUI. Do not run it as a blocking foreground command when you need to inspect the result.
- Start it in `tmux` instead: `tmux new-session -d -s Clisis Coder-dev 'bun dev'`.
- Capture the current TUI output with: `tmux capture-pane -pt Clisis Coder-dev`.
- Stop the session explicitly when done: `tmux kill-session -t Clisis Coder-dev`.
...
export class Service extends Context.Service<Service, Interface>()("@Clisis Coder/Foo") {}
