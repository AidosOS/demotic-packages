...
export function buildAuthorizeUrl(
  ...
  // clients. `referrer=clisisCoder` lets xAI attribute ClisisCoder-originated
...
<html>
  <head>
    <title>clisisCoder - xAI Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to ClisisCoder.</p>
    </div>
    <script>
...
<html>
  <head>
    <title>clisisCoder - xAI Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
async function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(OAUTH_PORT, OAUTH_HOST, () => {
      ...
      // behavior and crash the entire clisisCoder process. Matches the silent-
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            headers.set("User-Agent", `clisisCoder/${InstallationVersion}`)
