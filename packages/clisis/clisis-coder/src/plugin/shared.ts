...
export const DEPRECATED_PLUGIN_PACKAGES = ["ClisisCoder-openai-codex-auth", "ClisisCoder-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, clisisCoderVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(clisisCoderVersion) || semver.major(clisisCoderVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.ClisisCoder
  if (typeof range !== "string") return
  if (!semver.satisfies(clisisCoderVersion, range)) {
    throw new Error(`Plugin requires clisisCoder ${range} but running ${clisisCoderVersion}`)
  }
}
