...
export type PatchDeps = {
  ...
  files: (dir: string, name: "clisisCoder" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".clisis-coder")
}

function patchName(kind: Kind): "clisisCoder" | "tui" {
  if (kind === "server") return "clisisCoder"
  return "tui"
}
