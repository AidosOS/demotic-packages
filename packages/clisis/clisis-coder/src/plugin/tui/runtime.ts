...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@demotic/clisis-plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import * as Log from "@demotic/clisis-core/util/log"
import { errorData, errorMessage } from "@demotic/clisis-tui/util/error"
...
import { hasTheme, upsertTheme } from "@demotic/clisis-tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@demotic/clisis-tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@demotic/clisis-core/v1/config/plugin"
import { createCommandShim } from "@demotic/clisis-tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@demotic/clisis-tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".clisis-coder"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".clisis-coder", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".clisis-coder", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.CLISIS_CODER_PURE ? [] : pluginOrigins
    if (Flag.CLISIS_CODER_PURE && pluginOrigins.length) {
