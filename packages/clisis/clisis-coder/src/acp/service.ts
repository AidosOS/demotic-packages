...
import type { Message, clisisCoderClient, SessionMessageResponse } from "@demotic/clisis-sdk/v2"
...
export const AuthMethodID = "ClisisCoder-login"
...
export class Service extends Context.Service<Service, Interface>()("@demotic/clisis-ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `clisisCoder auth login` in the terminal",
      name: "Login with clisisCoder",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: clisisCoderClient) {
...
function makeUsageService(sdk: clisisCoderClient) {
...
async function loadDirectorySnapshot(sdk: clisisCoderClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, clisisCoder provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const clisisCoderProvider = providers[ProviderV2.ID.make("clisisCoder")]
  const clisisCoderModel = clisisCoderProvider ? Provider.sort(Object.values(clisisCoderProvider.models))[0] : undefined
  if (clisisCoderProvider && clisisCoderModel) return { providerID: clisisCoderProvider.id, modelID: clisisCoderModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "clisisCoder service failure", service })
}
