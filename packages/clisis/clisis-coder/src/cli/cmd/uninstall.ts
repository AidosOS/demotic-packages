...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall clisisCoder and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall clisisCoder")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g clisis-coder",
      pnpm: "pnpm uninstall -g clisis-coder",
      bun: "bun remove -g clisis-coder",
      yarn: "yarn global remove clisis-coder",
      brew: "brew uninstall clisisCoder",
      choco: "choco uninstall clisisCoder",
      scoop: "scoop uninstall clisisCoder",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "clisisCoder", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".clisis-coder")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using clisisCoder!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# clisisCoder") || content.includes(".clisis-coder/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".clisis-coder/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".clisis-coder/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".clisis-coder"))
    ) {
      continue
    }
