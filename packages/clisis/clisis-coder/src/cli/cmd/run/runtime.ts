...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "clisis-coder.mode": input.resolveSession ? "local" : "attach",
      "clisis-coder.initial_input": !!input.initialInput,
      "clisis-coder.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "clisis-coder.directory": ctx.directory,
        "clisis-coder.resume": ctx.resume === true,
        "clisis-coder.agent.name": state.agent,
        "clisis-coder.model.provider": state.model?.providerID,
        "clisis-coder.model.id": state.model?.modelID,
        "clisis-coder.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "clisis-coder.model.provider": model.providerID,
            "clisis-coder.model.id": model.modelID,
            "clisis-coder.model.variant": state.activeVariant,
          })
          return {
      ...
      if (Flag.CLISIS_CODER_SHOW_TTFD) {
        footer.append({
          kind: "system",
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "clisis-coder.model.id": state.model?.modelID,
                "clisis-coder.model.variant": state.activeVariant,
                "clisis-coder.prompt.chars": prompt.text.length,
                "clisis-coder.prompt.parts": prompt.parts.length,
                "clisis-coder.prompt.include_files": includeFiles,
                "clisis-coder.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "clisis-coder.directory": input.directory,
      "clisis-coder.initial_input": !!input.initialInput,
      "clisis-coder.demo": input.demo,
    },
    async () => {
