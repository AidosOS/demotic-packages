...
import type { clisisCoderClient, PermissionRequest, QuestionRequest, ToolPart } from "@demotic/clisis-sdk/v2"
...
type PromptModel = Parameters<clisisCoderClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<clisisCoderClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<clisisCoderClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<clisisCoderClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<clisisCoderClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<clisisCoderClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<clisisCoderClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<clisisCoderClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<clisisCoderClient["question"]["reject"]>[0]
