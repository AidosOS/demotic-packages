...
import { clisisCoderKeymapProvider, formatKeyBindings } from "@demotic/clisis-tui/keymap"
...
export class RunFooter implements FooterApi {
  ...
  constructor(
    ...
    void render(
      () =>
        createComponent(clisisCoderKeymapProvider, {
    ...
    this.flushing = this.flushing
      .then(() =>
        withRunSpan(
          ...
          {
            "clisis-coder.footer.phase": phase,
    ...
    this.flushing = this.flushing
      .then(() =>
        withRunSpan(
          "RunFooter.flush",
          {
            "clisis-coder.batch.commits": batch.length,
            "clisis-coder.footer.phase": phase,
            "session.id": this.options.sessionID() || undefined,
          },
          async () => {
