...
import { registerclisisCoderKeymap } from "@demotic/clisis-tui/keymap"
...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "clisis-coder.agent.name": input.agent,
      "clisis-coder.directory": input.directory,
      "clisis-coder.first": input.first,
      "clisis-coder.model.provider": input.model?.providerID,
      "clisis-coder.model.id": input.model?.modelID,
      "clisis-coder.model.variant": input.variant,
    ...
    async () => {
      ...
      try {
        ...
        unregisterKeymap = registerclisisCoderKeymap(keymap, renderer, input.tuiConfig)
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "clisis-coder.show_exit": next.showExit,
