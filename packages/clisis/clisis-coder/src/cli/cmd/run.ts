...
//      clisisCoder server and runs interactive mode against it.
//
...
import { createClisisCoderClient, type clisisCoderClient, type ToolPart } from "@demotic/clisis-sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<clisisCoderClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run clisisCoder with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running clisisCoder server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to CLISIS_CODER_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to CLISIS_CODER_SERVER_USERNAME or 'clisisCoder')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const attachSDK = (dir?: string) => {
        return createClisisCoderClient({
          baseUrl: args.attach!,
          directory: dir,
      ...
      async function session(sdk: clisisCoderClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: clisisCoderClient, sessionID: string) {
      ...
      async function current(sdk: clisisCoderClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: clisisCoderClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: clisisCoderClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: clisisCoderClient) {
        ...
        async function loop(client: clisisCoderClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createClisisCoderClient({
        baseUrl: "http://ClisisCoder.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
