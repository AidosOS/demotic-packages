...
import type { LLMClientShape } from "@demotic/clisis-llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("clisisCoder"))
    return { type: "unsupported", reason: "provider is not openai, clisisCoder, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @demotic/clisis-llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains ClisisCoder-owned. The native runtime only adapts
      // the @demotic/clisis-llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
