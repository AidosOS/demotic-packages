...
import { clisisCoderHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for clisisCoder serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(clisisCoderHttpApi)
...
export const PublicApi = clisisCoderHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "clisisCoder",
    version: "1.0.0",
    description: "clisisCoder api",
