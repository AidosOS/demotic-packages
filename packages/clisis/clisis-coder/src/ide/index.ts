...
export function alreadyInstalled() {
  return process.env["CLISIS_CODER_CALLER"] === "vscode" || process.env["CLISIS_CODER_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.clisis-coder"], {
    nothrow: true,
  })
