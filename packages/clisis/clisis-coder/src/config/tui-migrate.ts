...
const TUI_SCHEMA_URL = "https://clisiscoder.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from clisiscoder.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const clisisCoder = await clisisCoderFiles(input)
  for (const file of clisisCoder) {
...
async function clisisCoderFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "clisisCoder"),
    ...(await Filesystem.findUp(["clisiscoder.json", "clisiscoder.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "clisisCoder"))
  }
  if (Flag.CLISIS_CODER_CONFIG) files.push(Flag.CLISIS_CODER_CONFIG)
