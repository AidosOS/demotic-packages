...
const MANAGED_PLIST_DOMAIN = "ai.clisis-coder.managed"

// Keys injected by macOS/MDM into the managed plist that are not clisisCoder config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/clisisCoder"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "clisisCoder")
    default:
      return "/etc/clisisCoder"
  }
}

export function managedConfigDir() {
  return process.env.CLISIS_CODER_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
