...
import { isRecord } from "@demotic/clisis-tui/util/record"
...
import { TuiKeybind } from "@demotic/clisis-tui/config/keybind"
...
import { TuiConfig } from "@demotic/clisis-tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@demotic/clisis-TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old clisiscoder.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.clisis-coder`
  // folders between cwd and home, and CLISIS_CODER_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.CLISIS_CODER_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit CLISIS_CODER_TUI_CONFIG override, if set.
  if (Flag.CLISIS_CODER_TUI_CONFIG) {
    const configFile = Flag.CLISIS_CODER_TUI_CONFIG
  ...
  // 4. `.clisis-coder` directories (and CLISIS_CODER_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".clisis-coder") || dir === Flag.CLISIS_CODER_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".clisis-coder") && dir !== Flag.CLISIS_CODER_CONFIG_DIR) continue
