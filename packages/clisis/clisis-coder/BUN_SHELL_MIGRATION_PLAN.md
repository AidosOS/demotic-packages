...
Replace runtime Bun shell template-tag usage in `packages/ClisisCoder/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/ClisisCoder/src` is removed except:
