...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve clisisCoder route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "clisisCoder")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "clisisCoder")).toBe("openai")
    expect(statProvider("big-pickle", "", "clisisCoder")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "clisisCoder"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep clisisCoder as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "clisisCoder"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "clisisCoder"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep clisisCoder or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "clisisCoder"), country: "US" })).toMatchObject([
