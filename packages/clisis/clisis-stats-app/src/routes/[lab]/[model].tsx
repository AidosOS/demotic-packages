...
import {
  ...
  type StatsModelData,
  type UsageRange,
} from "@demotic/clisis-stats-core/domain/home"
import { runtime } from "@demotic/clisis-stats-core/runtime"
...
const statsModelFallbackUrl = "https://stats.clisiscoder.demoticsuite.ai"
...
export default function StatsModel() {
  ...
  const modelDescription = createMemo(() =>
    stats()
      ? `${modelName()} usage, rank, token mix, cost, geo breakdown, and peer stats across ClisisCoder.`
      : `${modelName()} model facts, limits, and clisisCoder usage availability.`,
  )
  ...
  return (
    <main data-page="stats" data-theme={themePreference()}>
      ...
      <Meta property="og:site_name" content="clisisCoder" />
      ...
      <div data-slot="model-hero-grid">
        <div data-slot="model-hero-copy">
          ...
          <Show
            when={props.data}
            fallback={
              <p>Model facts from the shared model index. clisisCoder usage appears once this model has activity.</p>
            }
          >
            {(data) => (
              <p>
                Ranked #{data().rank} across recent clisisCoder token usage with {formatPercent(data().tokenShare)} of
                observed volume.
              </p>
            )}
          </Show>
...
function ModelCatalogCallout(props: { catalog: ModelCatalogEntry | null }) {
  return (
    <div data-component="model-rank-panel">
      ...
      <p>No clisisCoder usage in the current stats window.</p>
    </div>
  )
}
...
function ModelOverview(props: { data: StatsModelData | null }) {
  return (
    <section data-section="model-panel">
      ...
      <Show
        when={props.data}
        fallback={<ModelEmptyState title="No usage summary" description="This model has no clisisCoder usage rows yet." />}
      >
        {(data) => (
