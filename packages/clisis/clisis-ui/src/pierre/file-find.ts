...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("ClisisCoder-find")
  api.delete("ClisisCoder-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("ClisisCoder-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("ClisisCoder-find", new Highlight(...rest))
    return true
  }
