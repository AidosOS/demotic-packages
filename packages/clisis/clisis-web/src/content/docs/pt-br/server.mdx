---
title: Servidor
description: Interaja com o servidor Clisis Coder via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `Clisis Coder serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente Clisis Coder pode usar.
...
```bash
Clisis Coder serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `Clisis Coder.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
Clisis Coder serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `Clisis Coder_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `Clisis Coder`, ou defina `Clisis Coder_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `Clisis Coder serve` quanto ao `Clisis Coder web`.

```bash
Clisis Coder_SERVER_PASSWORD=your-password Clisis Coder serve
```
...
Quando você executa `Clisis Coder`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor Clisis Coder para interagir com o Clisis Coder programaticamente.
:::

Essa arquitetura permite que o Clisis Coder suporte múltiplos clientes e permite que você interaja com o Clisis Coder programaticamente.

Você pode executar `Clisis Coder serve` para iniciar um servidor autônomo. Se você tiver o TUI do Clisis Coder em execução, `Clisis Coder serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do Clisis Coder [IDE](/docs/ide).
...
O servidor Clisis Coder expõe as seguintes APIs.
