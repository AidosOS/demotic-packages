...
Com o Clisis Coder, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o Clisis Coder usa nosso próprio tema `Clisis Coder`.
...
O Clisis Coder vem com vários temas embutidos.
...
- Querem que o Clisis Coder corresponda à aparência do seu terminal
...
O Clisis Coder suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/Clisis Coder/themes/*.json` ou `$XDG_CONFIG_HOME/Clisis Coder/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.Clisis Coder/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.Clisis Coder/themes/*.json`
