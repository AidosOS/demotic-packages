...
Existem dois tipos de agentes no Clisis Coder; agentes primários e subagentes.
...
Clisis Coder vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Clisis Coder vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Clisis Coder vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Clisis Coder, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `Clisis Coder.json`:
...
- Global: `~/.config/Clisis Coder/agents/`
- Por projeto: `.Clisis Coder/agents/`
...
Se nenhuma temperatura for especificada, o Clisis Coder usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do Clisis Coder quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do Clisis Coder usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Clisis Coder Zen](/docs/zen), você usaria `Clisis Coder/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `Clisis Coder models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/AidosOS/demotic-clisis-coder).
:::
