...
O Clisis Coder Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Clisis Coder. Você assina o Clisis Coder Go e
...
O Clisis Coder Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Clisis Coder Go funciona como qualquer outro provedor no Clisis Coder.

1. Você entra no **<a href={console}>Clisis Coder Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Clisis Coder Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Clisis Coder Go.
:::
...
O Clisis Coder Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Clisis Coder
usa o formato `Clisis Coder-go/<model-id>`. Por exemplo, para o Kimi K2.6, você usaria
`Clisis Coder-go/kimi-k2.6` na sua configuração.
...
Criamos o Clisis Coder Go para:
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Clisis Coder também.
