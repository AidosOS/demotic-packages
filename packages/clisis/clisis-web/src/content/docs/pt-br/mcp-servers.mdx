...
Você pode adicionar ferramentas externas ao Clisis Coder usando o _Model Context Protocol_, ou MCP. O Clisis Coder suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do Clisis Coder](https://Clisis Coder.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/Clisis Coder`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O Clisis Coder lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o Clisis Coder irá:
...
Se o servidor requer autenticação, o Clisis Coder solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `Clisis Coder mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o Clisis Coder armazenará os tokens de forma segura em `~/.local/share/Clisis Coder/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no Clisis Coder, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do Clisis Coder como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o Clisis Coder à sua conta do Sentry.
