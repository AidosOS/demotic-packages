...
As habilidades do agente permitem que o Clisis Coder descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O Clisis Coder pesquisa nesses locais:

- Configuração do projeto: `.Clisis Coder/skills/<name>/SKILL.md`
- Configuração global: `~/.config/Clisis Coder/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o Clisis Coder sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.Clisis Coder/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/Clisis Coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.Clisis Coder/skills/git-release/SKILL.md` assim:
...
O Clisis Coder lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `Clisis Coder.json`:
...
**Para agentes embutidos** (em `Clisis Coder.json`):
