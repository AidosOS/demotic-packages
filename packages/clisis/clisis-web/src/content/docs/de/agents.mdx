...
Es gibt zwei Arten von Agenten in Clisis Coder; Primäragenten und Subagenten.
...
Clisis Coder verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `Clisis Coder.json`-Konfigurationsdatei:
...
- Pro Projekt: `.Clisis Coder/agents/`
...
Wenn keine Temperatur angegeben ist, verwendet Clisis Coder modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Clisis Coder-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Clisis Coder-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Clisis Coder Zen](/docs/zen) verwenden, würden Sie `Clisis Coder/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `Clisis Coder models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/AidosOS/demotic-clisis-coder).
:::
