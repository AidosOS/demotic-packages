---
title: Konfiguration
description: Verwenden Sie die Konfiguration Clisis Coder JSON.
---

Sie können Clisis Coder mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

Clisis Coder unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
Wenn Ihre globale Konfiguration beispielsweise `theme: "Clisis Coder"` und `autoupdate: true` und Ihre Projektkonfiguration `model: "anthropic/claude-sonnet-4-5"` festlegt, umfasst die endgültige Konfiguration alle drei Einstellungen.
...
1. **Remote-Konfiguration** (von `.well-known/Clisis Coder`) – Organisationsstandards
2. **Globale Konfiguration** (`~/.config/Clisis Coder/Clisis Coder.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`Clisis Coder_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`Clisis Coder.json` im Projekt) – projektspezifische Einstellungen
5. **`.Clisis Coder` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`Clisis Coder_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.Clisis Coder` und `~/.config/Clisis Coder` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Organisationen können die Standardkonfiguration über den Endpunkt `.well-known/Clisis Coder` bereitstellen. Dieser wird automatisch abgerufen, wenn Sie sich bei einem Anbieter authentifizieren, der dies unterstützt.
...
Platzieren Sie Ihre globale Clisis Coder-Konfiguration in `~/.config/Clisis Coder/Clisis Coder.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `Clisis Coder.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.

:::tip
Platzieren Sie die projektspezifische Konfiguration im Stammverzeichnis Ihres Projekts.
:::

Wenn Clisis Coder gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
Geben Sie mithilfe der Umgebungsvariablen `Clisis Coder_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `Clisis Coder_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
Umgebungsvariable. Dieses Verzeichnis wird nach Agenten, Befehlen,
Modi und Plugins genau wie das Standardverzeichnis `.Clisis Coder` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.Clisis Coder` geladen, sodass es deren Einstellungen **überschreiben** kann.

---

## Schema

Die Konfigurationsdatei verfügt über ein Schema, das in [**`Clisis Coder.ai/config.json`**](https://Clisis Coder.ai/config.json) definiert ist.
...
Sie können Servereinstellungen für die Befehle `Clisis Coder serve` und `Clisis Coder web` über die Option `server` konfigurieren.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres Clisis Coder-Servers erkannt werden.
- `mdnsDomain` – Benutzerdefinierter Domänenname für den mDNS-Dienst. Standardmäßig ist `Clisis Coder.local`. Nützlich für die Ausführung mehrerer Instanzen im selben Netzwerk.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer Clisis Coder-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht Clisis Coder, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können das Thema, das Sie in Ihrer Clisis Coder-Konfiguration verwenden möchten, über die Option `theme` konfigurieren.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/Clisis Coder/agents/` oder `.Clisis Coder/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift Clisis Coder mit einer Warnung auf `"build"` zurück.

Diese Einstellung gilt für alle Schnittstellen: TUI, CLI (`Clisis Coder run`), Desktop-App und GitHub Aktion.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/Clisis Coder/commands/` oder `.Clisis Coder/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Clisis Coder lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
Clisis Coder erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert Clisis Coder mit benutzerdefinierten Tools, Hooks und Integrationen.

Platzieren Sie Ihre Plugin-Dateien in `.Clisis Coder/plugins/` oder `~/.config/Clisis Coder/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
...
Dies ist nützlich, wenn Sie Clisis Coder darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.
