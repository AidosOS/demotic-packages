...
Clisis Coder Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Clisis Coder. Du abonnierst Clisis Coder Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
Clisis Coder zu verwenden.
...
Clisis Coder Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Clisis Coder Go funktioniert wie jeder andere Provider in Clisis Coder.

1. Du meldest dich bei **<a href={console}>Clisis Coder Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Clisis Coder Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Clisis Coder Go abonnieren.
:::
...
Clisis Coder Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Clisis Coder Config verwendet das Format `Clisis Coder-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `Clisis Coder-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben Clisis Coder Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Clisis Coder zu nutzen.
