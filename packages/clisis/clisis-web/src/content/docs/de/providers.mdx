---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Clisis Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Clisis Coder verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Clisis Coder-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Clisis Coder anpassen
config.
...
Clisis Coder Zen ist eine Liste von Modellen, die vom Clisis Coder-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Clisis Coder funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Clisis Coder Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Clisis Coder aus und gehen Sie zu [Clisis Coder.ai/auth](https://Clisis Coder.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in Clisis Coder und ist völlig optional.

---

## Clisis Coder Go

Clisis Coder Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Clisis Coder-Team getestet und verifiziert wurden, dass sie gut mit Clisis Coder funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Clisis Coder Go` und gehen Sie zu [Clisis Coder.ai/auth](https://Clisis Coder.ai/zen).
...
So verwenden Sie Amazon Bedrock mit Clisis Coder:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Clisis Coder ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `Clisis Coder.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Clisis Coder wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können Clisis Coder so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Clisis Coder ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Clisis Coder-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `Clisis Coder`, Bereiche: `api`
...
um das Token im Clisis Coder-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Clisis Coder verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Clisis Coder
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`Clisis Coder.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [Clisis Coder-gitlab-auth](https://www.npmjs.com/package/Clisis Coder-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `Clisis Coder.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Clisis Coder:
...
So verwenden Sie Google Vertex AI mit Clisis Coder:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Clisis Coder ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Clisis Coder konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Clisis Coder-Konversation als separate Sitzung aufgeführt.
...
Sie können Clisis Coder für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Clisis Coder über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Clisis Coder über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Clisis Coder konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/Clisis Coder).
:::
...
So verwenden Sie Ollama Cloud mit Clisis Coder:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Clisis Coder.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Clisis Coder verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Clisis Coder Zen ist eine Liste getesteter und verifizierter Modelle, die vom Clisis Coder-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Clisis Coder Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Clisis Coder Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Clisis Coder API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Clisis Coder-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Clisis Coder-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Clisis Coder:
...
Sie können Modelle auch über Ihre Clisis Coder-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Clisis Coder verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `Clisis Coder.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Clisis Coder erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `Clisis Coder auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Clisis Coder-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Clisis Coder-Konfiguration übereinstimmt.
