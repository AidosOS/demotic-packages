...
Agent Skills erlauben Clisis Coder, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Clisis Coder sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Clisis Coder vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.Clisis Coder/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/Clisis Coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.Clisis Coder/skills/git-release/SKILL.md` zum Beispiel so:
...
Clisis Coder listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `Clisis Coder.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `Clisis Coder.json`):
