---
title: Server
description: Nutze den Clisis Coder-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `Clisis Coder serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein Clisis Coder-Client nutzen kann.
...
Setze `Clisis Coder_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `Clisis Coder`, kann aber mit `Clisis Coder_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `Clisis Coder serve` und `Clisis Coder web`.
...
Wenn du `Clisis Coder` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den Clisis Coder-Server, um Clisis Coder programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `Clisis Coder serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `Clisis Coder serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Clisis Coder-[IDE](/docs/ide)-Plugins.
...
Der Clisis Coder-Server stellt folgende APIs bereit.
