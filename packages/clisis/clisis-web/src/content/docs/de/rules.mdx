---
title: Regeln
description: Lege eigene Anweisungen fuer Clisis Coder fest.
---

Du kannst Clisis Coder ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in Clisis Coder den Befehl `/init` aus.
...
So kann Clisis Coder sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht Clisis Coder sie zu erweitern.
...
Clisis Coder kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/Clisis Coder/AGENTS.md` speichern.
Diese gelten in allen Clisis Coder-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Clisis Coder dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/Clisis Coder/AGENTS.md` existiert)
...
Beim Start sucht Clisis Coder in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/Clisis Coder/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `Clisis Coder.json` oder in `~/.config/Clisis Coder/Clisis Coder.json` angeben.
...
Clisis Coder parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit Clisis Coder.json

Empfohlen ist das `instructions`-Feld in der `Clisis Coder.json`:
...
Du kannst Clisis Coder explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `Clisis Coder.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
