...
Sie können externe Tools zu Clisis Coder hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Clisis Coder unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Clisis Coder Config](https://Clisis Coder.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/Clisis Coder`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Clisis Coder übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Clisis Coder Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Clisis Coder beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `Clisis Coder mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Clisis Coder die Token sicher in `~/.local/share/Clisis Coder/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Clisis Coder verfügbar. Sie können sie also wie jedes andere Tool über die Clisis Coder-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Clisis Coder mit Ihrem Sentry-Konto zu verbinden.
