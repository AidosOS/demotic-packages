...
Model Context Protocol , 또는 MCP를 사용하여 Clisis Coder에 외부 도구를 추가할 수 있습니다. Clisis Coder는 로컬 및 원격 서버를 모두 지원합니다.
...
조직은 `.well-known/Clisis Coder` 엔드포인트를 통해 기본 MCP 서버를 제공 할 수 있습니다. 이 서버는 기본적으로 비활성화 될 수 있으며 사용자가 필요한 것들에 대해 선택 할 수 있습니다.
...
Clisis Coder는 원격 MCP 서버에 대한 OAuth 인증을 자동으로 처리합니다. 서버가 인증이 필요할 때, Clisis Coder는:
...
서버가 인증이 필요한 경우, Clisis Coder는 먼저 사용하려고 할 때 인증됩니다. 그렇지 않다면, `Clisis Coder mcp auth <server-name>`를 사용하여 [수동으로 트리거](#authenticating)할 수 있습니다.
...
`mcp auth` 명령은 사용자의 브라우저를 엽니다. `~/.local/share/Clisis Coder/mcp-auth.json`에서 토큰을 안전하게 저장합니다.
...
MCP는 Clisis Coder의 도구로 사용할 수 있으며 내장 도구와 함께 사용할 수 있습니다. 다른 도구와 같이 Clisis Coder config를 통해 관리할 수 있습니다.
...
브라우저 창을 열어 OAuth 흐름을 완료하고 Clisis Coder를 Sentry 계정에 연결하십시오.
