---
title: Konfiguracja
description: Ustawienia Clisis Coder w pliku JSON.
---

Możesz dostosować Clisis Coder za pomocą pliku konfiguracyjnego JSON.

---

## Format

Clisis Coder obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "Clisis Coder"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/Clisis Coder`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/Clisis Coder/Clisis Coder.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `Clisis Coder_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`Clisis Coder.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.Clisis Coder`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `Clisis Coder_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.Clisis Coder` i `~/.config/Clisis Coder` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/Clisis Coder`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację Clisis Coder w `~/.config/Clisis Coder/Clisis Coder.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/Clisis Coder/tui.json`.
...
Dodaj `Clisis Coder.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy Clisis Coder się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `Clisis Coder_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `Clisis Coder_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.Clisis Coder` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.Clisis Coder`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`Clisis Coder.ai/config.json`**](https://Clisis Coder.ai/config.json).
...
Użyj `Clisis Coder_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `Clisis Coder.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `Clisis Coder serve` i `Clisis Coder web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer Clisis Coder.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `Clisis Coder.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie Clisis Coder wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/Clisis Coder/agents/` lub `.Clisis Coder/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, Clisis Coder powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`Clisis Coder run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/Clisis Coder/commands/` lub `.Clisis Coder/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Clisis Coder może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy Clisis Coder nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie Clisis Coder **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają Clisis Coder o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.Clisis Coder/plugins/` lub `~/.config/Clisis Coder/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie Clisis Coder tylko do określonych dostawców.
