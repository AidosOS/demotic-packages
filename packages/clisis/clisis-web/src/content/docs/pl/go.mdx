...
Clisis Coder Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w Clisis Coder. Subskrybujesz Clisis Coder Go i
...
Clisis Coder Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

Clisis Coder Go działa jak każdy inny dostawca w Clisis Coder.

1. Logujesz się do **<a href={console}>Clisis Coder Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `Clisis Coder Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować Clisis Coder Go.
:::
...
Clisis Coder Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji Clisis Coder
używa formatu `Clisis Coder-go/<model-id>`. Na przykład dla Kimi K2.6 należy użyć
`Clisis Coder-go/kimi-k2.6` w swojej konfiguracji.
...
Stworzyliśmy Clisis Coder Go, aby:
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z Clisis Coder.
