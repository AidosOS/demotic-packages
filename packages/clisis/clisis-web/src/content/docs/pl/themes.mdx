...
Dzięki Clisis Coder możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie Clisis Coder używa naszego własnego motywu `Clisis Coder`.
...
Clisis Coder ma kilka wbudowanych motywów.
...
- Chcą, aby Clisis Coder pasował do wyglądu ich terminala
...
Clisis Coder obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/Clisis Coder/themes/*.json` lub `$XDG_CONFIG_HOME/Clisis Coder/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.Clisis Coder/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.Clisis Coder/themes/*.json`
