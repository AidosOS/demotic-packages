---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w Clisis Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Clisis Coder używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji Clisis Coder.
...
w `~/.local/share/Clisis Coder/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim Clisis Coder
config.
...
Clisis Coder Zen to lista modeli dostarczonych przez zespół Clisis Coder, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z Clisis Coder. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Clisis Coder Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Clisis Coder Zen` i przejdź do [Clisis Coder.ai/auth](https://Clisis Coder.ai/zen).
...
Działa jak każdy inny dostawca w Clisis Coder i jest całkowicie opcjonalny w użyciu.

---

## Clisis Coder Go

Clisis Coder Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół Clisis Coder, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z Clisis Coder.

1. Uruchom polecenie `/connect` w TUI, wybierz `Clisis Coder Go` i przejdź do [Clisis Coder.ai/auth](https://Clisis Coder.ai/zen).
...
Aby używać Amazon Bedrock z Clisis Coder:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania Clisis Coder:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `Clisis Coder.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w Clisis Coder nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować Clisis Coder do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod Clisis Coder działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji Clisis Coder.
...
Clisis Coder używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować Clisis Coder
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`Clisis Coder.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [Clisis Coder-gitlab-auth](https://www.npmjs.com/package/Clisis Coder-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem Clisis Coder:
...
Aby używać Google Vertex AI z Clisis Coder:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania Clisis Coder.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą Clisis Coder, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [Clisis Coder-helicone-session](https://github.com/H2Shami/Clisis Coder-helicone-session), aby automatycznie rejestrować każdą konwersację Clisis Coder jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja Clisis Coder będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować Clisis Coder tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować Clisis Coder tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować Clisis Coder tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem Clisis Coder. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/Clisis Coder).
:::
...
Aby korzystać z Ollama Cloud z Clisis Coder:
...
3. Skopiuj klucz API do użycia w Clisis Coder.
...
6. **Ważne**: Przed użyciem modeli chmurowych w Clisis Coder musisz pobrać informacje o modelu lokalnie:
...
Clisis Coder Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół Clisis Coder. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Clisis Coder Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Clisis Coder Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API Clisis Coder.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji Clisis Coder.
...
5. Możesz je także dostosować za pomocą konfiguracji Clisis Coder. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z Clisis Coder:
...
Możesz także dostosować modele za pomocą konfiguracji Clisis Coder. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z Clisis Coder. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `Clisis Coder.json` w katalogu projektu:
...
Pola `limit` pozwalają Clisis Coder zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `Clisis Coder auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację Clisis Coder i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji Clisis Coder.
