...
W Clisis Coder obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Clisis Coder zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Clisis Coder ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

Clisis Coder ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej Clisis Coder, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `Clisis Coder.json`:
...
- Globalnie: `~/.config/Clisis Coder/agents/`
- W projekcie: `.Clisis Coder/agents/`
...
Jeśli nie zostanie skonfigurowana, Clisis Coder używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Clisis Coder, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Clisis Coder w `provider/model-id`. Na przykład, [Clisis Coder Zen](/docs/zen), użyj `Clisis Coder/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `Clisis Coder models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/AidosOS/demotic-clisis-coder).
:::
