...
Clisis Coder zawiera listę skrótów klawiszowych, które można dostosować za pomocą `tui.json`.
...
Clisis Coder używa klucza `leader` dla głównych skrótów klawiszowych. Rozwiązanie problemu konfliktów w terminalu.
...
Pole sprawdzania aplikacji komputerowej Clisis Coder obsługuje popularne skróty w stylu Readline/Emacs do edycji tekstu. Są one dostępne i obecnie nie można ich skonfigurować za pomocą `Clisis Coder.json`.
