---
title: Serwer
description: Interakcja z serwerem Clisis Coder za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `Clisis Coder serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient Clisis Coder.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `Clisis Coder.local` |
...
Ustaw `Clisis Coder_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `Clisis Coder` lub ustaw `Clisis Coder_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `Clisis Coder serve`, jak i `Clisis Coder web`.
...
Kiedy uruchomisz `Clisis Coder`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera Clisis Coder do programowej interakcji z Clisis Coder.
:::

Ta architektura umożliwia obsługę wielu klientów przez Clisis Coder i programową interakcję z kodem otwartym.

Możesz uruchomić `Clisis Coder serve`, aby uruchomić samodzielny serwer. Jeśli masz
Clisis Coder TUI działa, `Clisis Coder serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki Clisis Coder [IDE](/docs/ide).
...
Serwer Clisis Coder udostępnia następujące interfejsy API.
