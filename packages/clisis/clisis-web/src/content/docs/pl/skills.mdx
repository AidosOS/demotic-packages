...
Umiejętności agenta pozwalają Clisis Coder odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
Clisis Coder przeszukuje te lokalizacje:

- Project config: `.Clisis Coder/skills/<name>/SKILL.md`
- Global config: `~/.config/Clisis Coder/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu Clisis Coder przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.Clisis Coder/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/Clisis Coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.Clisis Coder/skills/git-release/SKILL.md` w ten sposób:
...
Clisis Coder wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `Clisis Coder.json`:
...
**Dla agentów wbudowanych** (w `Clisis Coder.json`):
