...
Możesz dodać dodatkowe informacje do Clisis Coder za pomocą _Model Context Protocol_, w skrócie MCP. Clisis Coder obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [Clisis Coder Config](https://Clisis Coder.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/Clisis Coder`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
Clisis Coder automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, Clisis Coder:
...
Jeśli serwer wymaga uwierzytelnienia, Clisis Coder poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `Clisis Coder mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji Clisis Coder bezpieczne przechowa tokeny w `~/.local/share/Clisis Coder/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w Clisis Coder, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie Clisis Coder, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć Clisis Coder z kontem Sentry.
