---
title: TUI
description: Using the Clisis Coder terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Clisis Coder provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Clisis Coder starts the TUI for the current directory.
...
When using the Clisis Coder TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Clisis Coder. Allows you to select from available providers and add their API keys.
...
Exit Clisis Coder. _Aliases_: `/quit`, `/q`
...
This is separate from `Clisis Coder.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Clisis Coder waits after the leader key. Defaults to `2000`.
...
Use `Clisis Coder_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `Clisis Coder.default`.
