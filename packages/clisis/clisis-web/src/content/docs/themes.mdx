...
With Clisis Coder you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Clisis Coder uses our own `Clisis Coder` theme.
...
Clisis Coder comes with several built-in themes.
...
- Want Clisis Coder to match their terminal's appearance
...
Clisis Coder supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/Clisis Coder/themes/*.json` or `$XDG_CONFIG_HOME/Clisis Coder/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.Clisis Coder/themes/*.json`
4. **Current working directory** - Defined in `./.Clisis Coder/themes/*.json`
