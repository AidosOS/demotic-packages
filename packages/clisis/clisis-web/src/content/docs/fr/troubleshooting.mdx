...
Pour déboguer les problèmes avec Clisis Coder, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\Clisis Coder\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `Clisis Coder --log-level DEBUG`.

---

## Stockage

Clisis Coder stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/Clisis Coder/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\Clisis Coder`
...
Clisis Coder Desktop exécute un serveur Clisis Coder local (le side-car `Clisis Coder-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Clisis Coder` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/Clisis Coder/Clisis Coder.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\Clisis Coder\Clisis Coder.jsonc`
...
Clisis Coder peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/Clisis Coder/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\Clisis Coder\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Clisis Coder puisse le reconstruire.

1. Quittez complètement Clisis Coder Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/Clisis Coder`
- **Linux** : supprimez `~/.cache/Clisis Coder` (ou exécutez `rm -rf ~/.cache/Clisis Coder`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\Clisis Coder`

3. Redémarrez le bureau Clisis Coder.

---

### Résoudre les problèmes de connexion au serveur

Clisis Coder Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `Clisis Coder.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `Clisis Coder_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `Clisis Coder_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Clisis Coder Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Clisis Coder.

---

### Les notifications ne s'affichent pas

Clisis Coder Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Clisis Coder dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Clisis Coder.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Clisis Coder Desktop) :

- `Clisis Coder.settings.dat` (serveur de bureau par défaut URL)
- `Clisis Coder.global.dat` et `Clisis Coder.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Clisis Coder :
...
3. Assurez-vous d'avoir la dernière version avec `Clisis Coder upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `Clisis Coder models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\Clisis Coder`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. Clisis Coder installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\Clisis Coder`

2. Redémarrez Clisis Coder pour réinstaller les derniers packages du fournisseur

Cela forcera Clisis Coder à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
Clisis Coder détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
