...
Les compétences d'agent permettent à Clisis Coder de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Clisis Coder recherche ces emplacements :

- Configuration du projet : `.Clisis Coder/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/Clisis Coder/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Clisis Coder remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.Clisis Coder/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/Clisis Coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.Clisis Coder/skills/git-release/SKILL.md` comme ceci :
...
Clisis Coder répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `Clisis Coder.json` :
...
**Pour les agents intégrés** (dans `Clisis Coder.json`) :
