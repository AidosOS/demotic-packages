---
title: Serveurs LSP
description: Clisis Coder s'intègre à vos serveurs LSP.
---

Clisis Coder peut s'intégrer aux serveurs Language Server Protocol (LSP) pour utiliser les diagnostics comme retour pour l'agent.

---

## Serveurs intégrés

Clisis Coder est livré avec plusieurs serveurs LSP intégrés pour les langages populaires :
...
:::note
Vous pouvez désactiver les téléchargements automatiques du serveur LSP en définissant la variable d'environnement `Clisis Coder_DISABLE_LSP_DOWNLOAD` sur `true`.
:::
...
Lorsque LSP est activé et qu'Clisis Coder ouvre un fichier, il :
...
Vous pouvez activer et personnaliser les serveurs LSP via la section `lsp` de votre configuration Clisis Coder.
...
```json title="Clisis Coder.json" {5-7}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="Clisis Coder.json" {5-9}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "lsp": {
    "typescript": {
