---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Clisis Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Clisis Coder utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Clisis Coder.
...
en `~/.local/share/Clisis Coder/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Clisis Coder
configuration.
...
Clisis Coder Zen est une liste de modèles fournis par l'équipe Clisis Coder qui ont été
testé et vérifié pour fonctionner correctement avec Clisis Coder. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Clisis Coder Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez Clisis Coder et dirigez-vous vers [Clisis Coder.ai/auth](https://Clisis Coder.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Clisis Coder et son utilisation est totalement facultative.

---

## Clisis Coder Go

Clisis Coder Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Clisis Coder qui ont été
testé et vérifié pour fonctionner correctement avec Clisis Coder.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Clisis Coder Go` et rendez-vous sur [Clisis Coder.ai/auth](https://Clisis Coder.ai/zen).
...
Pour utiliser Amazon Bedrock avec Clisis Coder :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de Clisis Coder :
...
Pour une configuration spécifique au projet ou persistante, utilisez `Clisis Coder.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Clisis Coder n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer Clisis Coder pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que Clisis Coder fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration Clisis Coder.
...
3.  Nom : `Clisis Coder`, Portées : `api`
...
pour stocker le jeton dans le stockage d'authentification Clisis Coder.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Clisis Coder utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Clisis Coder
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `Clisis Coder.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [Clisis Coder-gitlab-auth](https://www.npmjs.com/package/Clisis Coder-gitlab-auth).

##### Configuration

Personnalisez via `Clisis Coder.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec Clisis Coder :
...
Pour utiliser Google Vertex AI avec Clisis Coder :
...
Définissez-les lors de l'exécution de Clisis Coder.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via Clisis Coder, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [Clisis Coder-helicone-session](https://github.com/H2Shami/Clisis Coder-helicone-session) pour enregistrer automatiquement chaque conversation Clisis Coder en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Clisis Coder répertoriée comme une session distincte.
...
Vous pouvez configurer Clisis Coder pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer Clisis Coder pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer Clisis Coder pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Clisis Coder. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/Clisis Coder) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec Clisis Coder :
...
3. Copiez la clé API à utiliser dans Clisis Coder.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Clisis Coder, vous devez extraire les informations du modèle localement :
...
Clisis Coder Zen est une liste de modèles testés et vérifiés fournie par l'équipe Clisis Coder. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Clisis Coder Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Clisis Coder Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Clisis Coder API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration Clisis Coder.
...
5. Vous pouvez également les personnaliser via votre configuration Clisis Coder. Voici un exemple de spécification d'un fournisseur
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Clisis Coder :
...
Vous pouvez également personnaliser les modèles via votre configuration Clisis Coder. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec Clisis Coder. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `Clisis Coder.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à Clisis Coder de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `Clisis Coder auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration Clisis Coder et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration Clisis Coder.
