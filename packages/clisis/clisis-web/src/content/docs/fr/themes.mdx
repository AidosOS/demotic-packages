...
Avec Clisis Coder, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Clisis Coder utilise notre propre thème `Clisis Coder`.
...
Clisis Coder est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Clisis Coder corresponde à l'apparence de leur terminal
...
Clisis Coder prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/Clisis Coder/themes/*.json` ou `$XDG_CONFIG_HOME/Clisis Coder/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.Clisis Coder/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.Clisis Coder/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/Clisis Coder/themes
vim ~/.config/Clisis Coder/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .Clisis Coder/themes
vim .Clisis Coder/themes/my-theme.json
```
...
{
  "$schema": "https://Clisis Coder.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
