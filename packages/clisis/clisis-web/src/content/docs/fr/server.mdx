---
title: Serveur
description: Interagissez avec le serveur Clisis Coder via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `Clisis Coder serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client Clisis Coder peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `Clisis Coder.local` |
...
Définissez `Clisis Coder_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `Clisis Coder`, ou définissez `Clisis Coder_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `Clisis Coder serve` et à `Clisis Coder web`.
...
Lorsque vous exécutez `Clisis Coder`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur Clisis Coder pour interagir avec Clisis Coder par programmation.
:::

Cette architecture permet à Clisis Coder de prendre en charge plusieurs clients et vous permet d'interagir avec Clisis Coder par programme.

Vous pouvez exécuter `Clisis Coder serve` pour démarrer un serveur autonome. Si vous avez le
Clisis Coder TUI en cours d'exécution, `Clisis Coder serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Clisis Coder [IDE](/docs/ide).
...
Le serveur Clisis Coder expose les API suivantes.
