...
Vous pouvez ajouter des outils externes à Clisis Coder à l'aide du _Model Context Protocol_ ou MCP. Clisis Coder prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [Clisis Coder Config](https://Clisis Coder.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/Clisis Coder`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
Clisis Coder gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, Clisis Coder :
...
Si le serveur nécessite une authentification, Clisis Coder vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `Clisis Coder mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, Clisis Coder stockera les jetons en toute sécurité dans `~/.local/share/Clisis Coder/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans Clisis Coder, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration Clisis Coder comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter Clisis Coder à votre compte Sentry.
