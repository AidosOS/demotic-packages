---
title: Formateurs
description: Clisis Coder utilise des formateurs spécifiques au langage.
---

Clisis Coder formate automatiquement les fichiers après leur écriture ou leur modification à l'aide de formateurs spécifiques au langage. Cela garantit que le code généré suit les styles de code de votre projet.

---

## Formateurs intégrés

Clisis Coder est livré avec plusieurs formateurs intégrés pour les langages et frameworks populaires. Vous trouverez ci-dessous une liste des formateurs, des extensions de fichiers prises en charge et des commandes ou options de configuration dont il a besoin.
...
Ainsi, si votre projet a `prettier` dans votre `package.json`, Clisis Coder l'utilisera automatiquement.

---

## Comment ça marche

Lorsque Clisis Coder écrit ou modifie un fichier, il :
...
Vous pouvez personnaliser les formateurs via la section `formatter` de votre configuration Clisis Coder.
...
```json title="Clisis Coder.json" {5}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "formatter": {
    "prettier": {
...
```json title="Clisis Coder.json" {4-14}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "formatter": {
    "prettier": {
