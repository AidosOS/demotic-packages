...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Clisis Coder.
...
Voici plusieurs modèles qui fonctionnent bien avec Clisis Coder, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Clisis Coder.

```json title="Clisis Coder.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Clisis Coder Zen](/docs/zen), vous utiliserez `Clisis Coder/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="Clisis Coder.jsonc" {7-12,19-24}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="Clisis Coder.jsonc" {6-21}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "provider": {
    "Clisis Coder": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Clisis Coder est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Clisis Coder est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="Clisis Coder.jsonc" {7-18}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque Clisis Coder démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Clisis Coder.
