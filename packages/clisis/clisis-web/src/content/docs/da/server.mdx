---
title: Server
description: Interager med Clisis Coder-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `Clisis Coder serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en Clisis Coder-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `Clisis Coder.local` |
...
Indstil `Clisis Coder_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `Clisis Coder`, eller indstil `Clisis Coder_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `Clisis Coder serve` og `Clisis Coder web`.
...
Når du kører `Clisis Coder` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug Clisis Coder-serveren til at interagere med Clisis Coder programmatisk.
:::

Denne arkitektur lader Clisis Coder understøtte flere klienter og lader dig interagere med Clisis Coder programmatisk.

Du kan køre `Clisis Coder serve` for at starte en selvstændig server. Hvis du har
Clisis Coder TUI kørende, vil `Clisis Coder serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Clisis Coder [IDE](/docs/ide) plugins.
...
Clisis Coder-serveren viser følgende API'er.
