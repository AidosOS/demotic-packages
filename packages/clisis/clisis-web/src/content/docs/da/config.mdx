---
title: Konfiguration
description: Ved at bruge Clisis Coder JSON konfig.
---

Du kan konfigurere Clisis Coder ved hjælp af en JSON-konfigurationsfil.

---

## Format

Clisis Coder understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/Clisis Coder`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/Clisis Coder/Clisis Coder.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`Clisis Coder_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`Clisis Coder.json` i projekt) - projektspecifikke indstillinger
5. **`.Clisis Coder` mapper** - agent, kommandoer, plugins
6. **Inline config** (`Clisis Coder_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.Clisis Coder` og `~/.config/Clisis Coder` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/Clisis Coder`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale Clisis Coder-konfiguration i `~/.config/Clisis Coder/Clisis Coder.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `Clisis Coder.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når Clisis Coder starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `Clisis Coder_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `Clisis Coder_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.Clisis Coder` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.Clisis Coder` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`Clisis Coder.ai/config.json`**](https://Clisis Coder.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `Clisis Coder serve` og `Clisis Coder web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Clisis Coder-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `Clisis Coder.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Clisis Coder-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Clisis Coder at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Clisis Coder-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/Clisis Coder/agents/` eller `.Clisis Coder/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Clisis Coder falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`Clisis Coder run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/Clisis Coder/commands/` eller `.Clisis Coder/commands/`. [Learn more here](/docs/commands).
...
Clisis Coder vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard Clisis Coder **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide Clisis Coder med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.Clisis Coder/plugins/` eller `~/.config/Clisis Coder/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Clisis Coder til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
