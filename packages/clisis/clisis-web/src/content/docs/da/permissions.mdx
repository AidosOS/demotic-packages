...
Clisis Coder bruger `permission`-konfigurationen til at bestemme, om en given handling skal køre automatisk, bede dig om eller blokeres.
...
Brug `external_directory` til at tillade værktøjsopkald, der berører stier uden for den arbejdsmappe, hvor Clisis Coder blev startet. Dette gælder for ethvert værktøj, der tager en sti som input (for eksempel `read`, `edit`, `glob`, `grep` og mange `bash` kommandoer).
...
Clisis Coder tilladelser indtastes efter værktøjsnavn plus et par sikkerhedsafskærmninger:
...
Hvis du ikke angiver noget, starter Clisis Coder fra tilladelige standardindstillinger:
...
Når Clisis Coder beder om godkendelse, tilbyder brugergrænsefladen tre resultater:

- `once` — godkend netop denne anmodning
- `always` — godkend fremtidige anmodninger, der matcher de foreslåede mønstre (for resten af ​​den nuværende Clisis Coder session)
