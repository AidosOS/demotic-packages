---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Clisis Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Clisis Coder bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Clisis Coder-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/Clisis Coder/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Clisis Coder-konfigurationen.
...
Clisis Coder Zen er en liste over modeller leveret af Clisis Coder-teamet, som er blevet testet og verificeret til at fungere godt med Clisis Coder. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Clisis Coder Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg Clisis Coder og gå til [Clisis Coder.ai/auth](https://Clisis Coder.ai/auth).
...
Det fungerer som alle andre udbydere i Clisis Coder og er helt valgfrit at bruge.

---

## Clisis Coder Go

Clisis Coder Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Clisis Coder-teamet, som er testet og verificeret til at fungere godt med Clisis Coder.

1. Kør kommandoen `/connect` i TUI, vælg `Clisis Coder Go`, og gå til [Clisis Coder.ai/auth](https://Clisis Coder.ai/zen).
...
Sådan bruger du Amazon Bedrock med Clisis Coder:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører Clisis Coder:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `Clisis Coder.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Clisis Coder understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere Clisis Coder til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at Clisis Coder skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem Clisis Coder-konfigurationen.
...
2. Vælg din godkendelsesmetode:
   ...
   3. Navn: `Clisis Coder`, omfang: `api`
...
gemme tokenet i Clisis Coder auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Clisis Coder bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Clisis Coder
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`Clisis Coder.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [Clisis Coder-gitlab-auth](https://www.npmjs.com/package/Clisis Coder-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `Clisis Coder.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Clisis Coder:
...
Sådan bruger du Google Vertex AI med Clisis Coder:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører Clisis Coder.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem Clisis Coder, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [Clisis Coder-helicone-session](https://github.com/H2Shami/Clisis Coder-helicone-session) plugin for automatisk at logge hver Clisis Coder-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Clisis Coder-samtale opført som en separat session.
...
Du kan konfigurere Clisis Coder til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere Clisis Coder til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere Clisis Coder til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Clisis Coder. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/Clisis Coder) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Clisis Coder:
...
3. Kopier API-nøglen til brug i Clisis Coder.
...
6. **Vigtigt**: Før du bruger skymodeller i Clisis Coder, skal du hente modelinformationen lokalt:
...
Clisis Coder Zen er en liste over testede og verificerede modeller leveret af Clisis Coder-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Clisis Coder Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Clisis Coder Zen**.

   ```txt
   /connect
   ```

3. Indtast Clisis Coder API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem Clisis Coder-konfigurationen.
...
5. Du kan også tilpasse dem gennem Clisis Coder-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Clisis Coder:
...
Du kan også tilpasse modeller gennem Clisis Coder-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med Clisis Coder. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `Clisis Coder.json`-filen i projektkataloget:
...
`limit`-felterne lader Clisis Coder forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `Clisis Coder auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek Clisis Coder-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i Clisis Coder-konfigurationen.
