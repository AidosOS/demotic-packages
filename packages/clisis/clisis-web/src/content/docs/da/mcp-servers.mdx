...
Du kan tilføje eksterne værktøjer til Clisis Coder ved hjælp af _Model Context Protocol_ eller MCP. Clisis Coder understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Clisis Coder Config](https://Clisis Coder.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/Clisis Coder`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
Clisis Coder håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Clisis Coder:
...
Hvis serveren kræver godkendelse, vil Clisis Coder bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `Clisis Coder mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Clisis Coder opbevare tokens sikkert i `~/.local/share/Clisis Coder/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Clisis Coder-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Clisis Coder til din Sentry-konto.
