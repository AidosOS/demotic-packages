...
Clisis Coder ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak Clisis Coder kendi `Clisis Coder` temasını kullanır.
...
Clisis Coder birden fazla yerleşik temayla gelir.
...
- Clisis Coder'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
Clisis Coder, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/Clisis Coder/themes/*.json` veya `$XDG_CONFIG_HOME/Clisis Coder/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.Clisis Coder/themes/*.json`
4. **Geçerli çalışma dizini** - `./.Clisis Coder/themes/*.json`
