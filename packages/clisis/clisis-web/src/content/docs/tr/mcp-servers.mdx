...
_Model Bağlam Protokolü_ veya MCP kullanarak Clisis Coder'a harici araçlar ekleyebilirsiniz. Clisis Coder hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [Clisis Coder Config](https://Clisis Coder.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/Clisis Coder` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
Clisis Coder, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde Clisis Coder şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa Clisis Coder, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `Clisis Coder mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından Clisis Coder, belirteçleri `~/.local/share/Clisis Coder/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra Clisis Coder'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi Clisis Coder yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve Clisis Coder'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
