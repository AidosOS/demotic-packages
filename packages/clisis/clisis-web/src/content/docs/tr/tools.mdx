...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. Clisis Coder yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
Clisis Coder içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `Clisis Coder_EXPERIMENTAL_LSP_TOOL=true` (veya `Clisis Coder_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca Clisis Coder provider kullanılırken veya `Clisis Coder_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

Clisis Coder başlatırken etkinleştirmek için:
