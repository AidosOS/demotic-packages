---
title: TUI
description: Clisis Coder terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Clisis Coder, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Clisis Coder'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Clisis Coder TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Clisis Coder'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Clisis Coder'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `Clisis Coder.json` dosyasından ayrıdır.
...
- `leader_timeout` - Clisis Coder'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
...
- `sound_pack` - Kullanılacak sound pack ID. Varsayılan değer `Clisis Coder.default`.
