---
title: Sunucu
description: Clisis Coder sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`Clisis Coder serve` komutu, Clisis Coder istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `Clisis Coder.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `Clisis Coder_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `Clisis Coder` değeridir; değiştirmek isterseniz `Clisis Coder_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `Clisis Coder serve` hem de `Clisis Coder web` için geçerlidir.
...
`Clisis Coder` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
Clisis Coder ile programatik etkileşim için Clisis Coder sunucusunu kullanın.
:::

Bu mimari, Clisis Coder'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`Clisis Coder serve` ile bağımsız bir sunucu başlatabilirsiniz. Clisis Coder TUI açıksa, `Clisis Coder serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum Clisis Coder [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
Clisis Coder sunucusu aşağıdaki API'leri sunar.
