---
title: Yapılandırma
description: Clisis Coder JSON yapılandırması kullanır.
---

Clisis Coder'u JSON yapılandırma dosyası kullanarak yapılandırabilirsiniz.

---

## Format

Clisis Coder hem **JSON** hem de **JSONC** (JSON Yorumlarla birlikte) formatlarını destekler.
...
Örneğin, genel ayarınız `theme: "Clisis Coder"` ve `autoupdate: true`'yi ayarlıyorsa ve proje ayarlarınız `model: "anthropic/claude-sonnet-4-5"`'yi ayarlıyorsa, nihai yapılandırma üç ayarı da içerecektir.
...
1. **Uzak yapılandırma** (`.well-known/Clisis Coder`'dan) - kurumsal varsayılanlar
2. **Genel yapılandırma** (`~/.config/Clisis Coder/Clisis Coder.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`Clisis Coder_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `Clisis Coder.json`) - projeye özgü ayarlar
5. **`.Clisis Coder` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`Clisis Coder_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
:::note
`.Clisis Coder` ve `~/.config/Clisis Coder` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
Kuruluşlar, `.well-known/Clisis Coder` uç noktası aracılığıyla varsayılan yapılandırmayı sağlayabilir. Bu, onu destekleyen bir sağlayıcıyla kimlik doğrulaması yaptığınızda otomatik olarak alınır.
...
Global Clisis Coder yapılandırmanızı `~/.config/Clisis Coder/Clisis Coder.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `Clisis Coder.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.

:::tip
Projeye özel yapılandırmayı projenizin köküne yerleştirin.
:::

Clisis Coder başlatıldığında geçerli dizinde bir yapılandırma dosyası arar veya en yakın Git dizinine gider.
...
`Clisis Coder_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`Clisis Coder_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
ortam değişkeni. Bu dizin agent'lar, komutlar için aranacaktır.
modlar ve eklentiler tıpkı standart `.Clisis Coder` dizini gibi olmalıdır ve
...
Özel dizin, genel yapılandırma ve `.Clisis Coder` dizinlerinden sonra yüklenir, böylece bunların ayarlarını **geçersiz kılabilir**.

---

## Şema

Yapılandırma dosyası [**`Clisis Coder.ai/config.json`**](https://Clisis Coder.ai/config.json)'da tanımlanan bir şemaya sahiptir.
...
`Clisis Coder serve` ve `Clisis Coder web` komutları için sunucu ayarlarını `server` seçeneği aracılığıyla yapılandırabilirsiniz.
...
- `mdns` - mDNS hizmet bulmayı etkinleştirin. Bu, ağdaki diğer cihazların Clisis Coder sunucunuzu keşfetmesine olanak tanır.
- `mdnsDomain` - mDNS hizmeti için özel alan adı. Varsayılan olarak `Clisis Coder.local` şeklindedir. Aynı ağda birden fazla örneği çalıştırmak için kullanışlıdır.
...
Clisis Coder yapılandırmanızda kullanmak istediğiniz sağlayıcıları ve modelleri `provider`, `model` ve `small_model` seçenekleri aracılığıyla yapılandırabilirsiniz.
...
`small_model` seçeneği, başlık oluşturma gibi hafif görevler için ayrı bir model yapılandırır. Varsayılan olarak Clisis Coder, sağlayıcınızda mevcutsa daha ucuz bir model kullanmaya çalışır, aksi takdirde ana modelinize geri döner.
...
Clisis Coder yapılandırmanızda kullanmak istediğiniz temayı `theme` seçeneği aracılığıyla yapılandırabilirsiniz.
...
Agent'ları `~/.config/Clisis Coder/agents/` veya `.Clisis Coder/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Varsayılan agent, birincil agent olmalıdır (alt agent değil). Bu, `"build"` veya `"plan"` gibi yerleşik bir agent ya da tanımladığınız bir [özel agent](/docs/agents) olabilir. Belirtilen agent mevcut değilse veya bir alt agent ise, Clisis Coder bir uyarıyla birlikte `"build"`'ye geri döner.

Bu ayar tüm arayüzler için geçerlidir: TUI, CLI (`Clisis Coder run`), masaüstü uygulaması ve GitHub Action.
...
Komutları `~/.config/Clisis Coder/commands/` veya `.Clisis Coder/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
Clisis Coder başlatıldığında yeni güncellemeleri otomatik olarak indirecektir. Bunu `autoupdate` seçeneğiyle devre dışı bırakabilirsiniz.
...
Varsayılan olarak, Clisis Coder açık bir onay gerektirmeden **tüm işlemlere izin verir**. `permission` seçeneğini kullanarak bunu değiştirebilirsiniz.
...
[Eklentiler](/docs/plugins) Clisis Coder'u özel araçlar, kancalar ve entegrasyonlarla genişletin.

Eklenti dosyalarını `.Clisis Coder/plugins/` veya `~/.config/Clisis Coder/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
...
Bu, Clisis Coder'u tek tek devre dışı bırakmak yerine yalnızca belirli sağlayıcıları kullanacak şekilde kısıtlamak istediğinizde kullanışlıdır.
