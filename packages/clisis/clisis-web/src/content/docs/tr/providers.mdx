---
title: Sağlayıcılar
description: Clisis Coder'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

Clisis Coder, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı Clisis Coder yapılandırmanızda yapılandırın.
...
`~/.local/share/Clisis Coder/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları Clisis Coder'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Clisis Coder Zen, Clisis Coder ekibi tarafından sağlanan modellerin bir listesidir.
Clisis Coder ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Clisis Coder Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, Clisis Coder'u seçin ve [Clisis Coder.ai/auth](https://Clisis Coder.ai/auth)'ye gidin.
...
Clisis Coder'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Clisis Coder Go

Clisis Coder Go, Clisis Coder ile iyi çalıştığı test edilmiş ve doğrulanmış, Clisis Coder ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Clisis Coder Go`'yu seçin ve [Clisis Coder.ai/auth](https://Clisis Coder.ai/zen) adresine gidin.
...
Amazon Bedrock'u Clisis Coder ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Clisis Coder'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `Clisis Coder.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi Clisis Coder'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
Clisis Coder'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   Clisis Coder'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Clisis Coder yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
Clisis Coder, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. Clisis Coder'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`Clisis Coder.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [Clisis Coder-gitlab-auth](https://www.npmjs.com/package/Clisis Coder-gitlab-auth) ana sayfasında.

##### Yapılandırma

`Clisis Coder.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi Clisis Coder'la kullanmak için:
...
Google Vertex AI'yi Clisis Coder ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları Clisis Coder çalıştırılırken ayarlayın.
...
Helicone'un Clisis Coder aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her Clisis Coder görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [Clisis Coder-helicone-session](https://github.com/H2Shami/Clisis Coder-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her Clisis Coder görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
Clisis Coder'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla Clisis Coder'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla Clisis Coder'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini Clisis Coder için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/Clisis Coder)'a bakın.
:::
...
Ollama Cloud'u Clisis Coder ile kullanmak için:
...
3. Clisis Coder'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: Clisis Coder'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Clisis Coder Zen, Clisis Coder ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Clisis Coder Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Clisis Coder Zen**'i arayın.

   ```txt
   /connect
   ```

3. Clisis Coder API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   Clisis Coder yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca Clisis Coder yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Clisis Coder ile kullanmak için:
...
Modelleri Clisis Coder yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı Clisis Coder ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `Clisis Coder.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları Clisis Coder'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `Clisis Coder auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için Clisis Coder yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, Clisis Coder yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
