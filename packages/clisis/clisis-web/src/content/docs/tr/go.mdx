...
Clisis Coder Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Clisis Coder'daki diğer sağlayıcılar gibi çalışır. Clisis Coder Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Clisis Coder'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Clisis Coder Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Clisis Coder Go, Clisis Coder'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Clisis Coder Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Clisis Coder Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Clisis Coder Go'ya abone olabilir.
:::
...
Clisis Coder Go aşağıdaki limitleri içerir:
...
Clisis Coder yapılandırmanızdaki [model id](/docs/config/#models) formatı `Clisis Coder-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `Clisis Coder-go/kimi-k2.6` kullanmalısınız.
...
Clisis Coder Go'yu şu amaçlarla oluşturduk:
...
4. Clisis Coder ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
