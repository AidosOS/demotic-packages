---
title: LSP-серверы
description: Clisis Coder интегрируется с вашими серверами LSP.
---

Clisis Coder может интегрироваться с серверами Language Server Protocol (LSP), чтобы использовать диагностику как обратную связь для агента.

---

## Встроенные

Clisis Coder поставляется с несколькими встроенными LSP-серверами для популярных языков:
...
:::note
Вы можете отключить автоматическую загрузку LSP-сервера, установив для переменной среды `Clisis Coder_DISABLE_LSP_DOWNLOAD` значение `true`.
:::
...
Когда LSP включен и Clisis Coder открывает файл, он:
...
Вы можете включить и настроить серверы LSP через раздел `lsp` в конфигурации Clisis Coder.
