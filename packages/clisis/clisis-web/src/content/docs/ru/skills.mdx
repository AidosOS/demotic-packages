...
Навыки агента позволяют Clisis Coder обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
Clisis Coder выполняет поиск в следующих местах:

- Конфигурация проекта: `.Clisis Coder/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/Clisis Coder/skills/<name>/SKILL.md`.
...
Для локальных путей проекта Clisis Coder переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.Clisis Coder/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/Clisis Coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.Clisis Coder/skills/git-release/SKILL.md` следующим образом:
...
Clisis Coder перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `Clisis Coder.json`:
...
**Для встроенных агентов** (в формате `Clisis Coder.json`):
