---
title: Провайдеры
description: Использование любого провайдера LLM в Clisis Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Clisis Coder использует [AI SDK](https://ai-sdk.dev/) и [Models.dev](https://models.dev) для поддержки **более 75 поставщиков LLM** и поддерживает запуск локальных моделей.

Чтобы добавить провайдера, вам необходимо:

1. Добавьте ключи API для провайдера с помощью команды `/connect`.
2. Настройте провайдера в вашей конфигурации Clisis Coder.
...
в `~/.local/share/Clisis Coder/auth.json`.

---

### Настройка

Вы можете настроить поставщиков через раздел `provider` в вашем Clisis Coder.
конфиг.
...
Clisis Coder Zen — это список моделей, предоставленный командой Clisis Coder, которые были
протестировано и проверено на хорошую работу с Clisis Coder. [Подробнее](/docs/zen).

:::tip
Если вы новичок, мы рекомендуем начать с Clisis Coder Zen.
:::

1. Запустите команду `/connect` в TUI, выберите `Clisis Coder Zen` и перейдите по адресу [Clisis Coder.ai/auth](https://Clisis Coder.ai/zen).
...
Он работает как любой другой поставщик в Clisis Coder и его использование совершенно необязательно.

---

## Clisis Coder Go

Clisis Coder Go — это недорогой план подписки, обеспечивающий надежный доступ к популярным открытым моделям кодирования, предоставляемым командой Clisis Coder, которые были
протестированы и проверены на хорошую работу с Clisis Coder.

1. Запустите команду `/connect` в TUI, выберите `Clisis Coder Go` и перейдите по адресу [Clisis Coder.ai/auth](https://Clisis Coder.ai/zen).
...
Чтобы использовать Amazon Bedrock с Clisis Coder:
...
2. **Настройте аутентификацию** одним из следующих способов:
   ...
   Установите одну из этих переменных среды при запуске Clisis Coder:
   ...
   Для конкретной или постоянной конфигурации проекта используйте `Clisis Coder.json`:
...
:::info
Использование вашей подписки Claude Pro/Max в Clisis Coder официально не поддерживается [Anthropic](https://anthropic.com).
:::
...
Вы можете настроить Clisis Coder для работы с локальными моделями через [Atomic Chat](https://atomic.chat) — десктопное приложение, которое запускает локальные LLM за OpenAI-совместимым API-сервером (конечная точка по умолчанию `http://127.0.0.1:1337/v1`).
...
2. Перейдите в [Azure AI Foundry](https://ai.azure.com/) и разверните модель.
   ...
   :::примечание
   Для правильной работы Clisis Coder имя развертывания должно совпадать с именем модели.
   :::
...
5. Запустите команду `/models`, чтобы выбрать модель.
   ...
   Вы также можете добавлять модели через конфигурацию Clisis Coder.
...
2. Выберите метод аутентификации:
   ...
   3. Имя: `Clisis Coder`, Области применения: `api`
...
для хранения токена в хранилище аутентификации Clisis Coder.
:::

##### Самостоятельная GitLab

:::note[примечание о соответствии]
Clisis Coder использует небольшую модель для некоторых задач ИИ, таких как создание заголовка сеанса.
По умолчанию он настроен на использование gpt-5-nano, размещенного на Zen. Чтобы заблокировать Clisis Coder
чтобы использовать только свой собственный экземпляр, размещенный на GitLab, добавьте следующее в свой
`Clisis Coder.json` файл. Также рекомендуется отключить совместное использование сеансов.
...
Дополнительная документация на домашней странице [Clisis Coder-gitlab-auth](https://www.npmjs.com/package/Clisis Coder-gitlab-auth).

##### Конфигурация

Настройте через `Clisis Coder.json`:
...
Чтобы использовать Google Vertex AI с Clisis Coder:
...
2. Установите необходимые переменные среды:
   ...
   Установите их во время запуска Clisis Coder.
...
Если вы видите функцию или модель от Helicone, которая не настраивается автоматически через Clisis Coder, вы всегда можете настроить ее самостоятельно.
...
Плагин вставляет в ваши запросы заголовки `Helicone-Session-Id` и `Helicone-Session-Name`. На странице «Сеансы» Helicone вы увидите каждый диалог Clisis Coder, указанный как отдельный сеанс.
...
Вы можете настроить Clisis Coder для использования локальных моделей с помощью [утилиты llama-server llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Вы можете настроить Clisis Coder для использования локальных моделей через LM Studio.
...
Вы можете настроить Clisis Coder для использования локальных моделей через Ollama.

:::tip
Ollama может автоматически настроиться для Clisis Coder. Подробности см. в документации по интеграции Ollama](https://docs.ollama.com/integrations/Clisis Coder).
:::
...
Чтобы использовать Ollama Cloud с Clisis Coder:
...
3. Скопируйте ключ API для использования в Clisis Coder.
...
6. **Важно**. Перед использованием облачных моделей в Clisis Coder необходимо получить информацию о модели локально:
...
Clisis Coder Zen — это список протестированных и проверенных моделей, предоставленный командой Clisis Coder. [Подробнее](/docs/zen).

1. Войдите в систему **<a href={console}>Clisis Coder Zen</a>** и нажмите **Создать ключ API**.

2. Запустите команду `/connect` и найдите **Clisis Coder Zen**.

   ```txt
   /connect
   ```

3. Введите свой ключ API Clisis Coder.
...
4. Многие модели OpenRouter предварительно загружены по умолчанию. Запустите команду `/models`, чтобы выбрать нужную.
   ...
   Вы также можете добавить дополнительные модели через конфигурацию Clisis Coder.
...
5. Вы также можете настроить их через конфигурацию Clisis Coder. Вот пример указания провайдера
...
Чтобы использовать [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) с Clisis Coder:
...
Вы также можете настраивать модели через конфигурацию Clisis Coder. Ниже приведен пример указания порядка маршрутизации поставщика.
...
4. Создайте или обновите файл `Clisis Coder.json` в каталоге вашего проекта:
...
Поля `limit` позволяют Clisis Coder понять, сколько контекста у вас осталось. Стандартные поставщики автоматически извлекают их из models.dev.
...
1. **Проверьте настройку аутентификации**: запустите `Clisis Coder auth list`, чтобы проверить, верны ли учетные данные.
...
2. Для пользовательских поставщиков проверьте конфигурацию Clisis Coder и:
   - Убедитесь, что идентификатор провайдера, используемый в команде `/connect`, соответствует идентификатору в вашей конфигурации Clisis Coder.
