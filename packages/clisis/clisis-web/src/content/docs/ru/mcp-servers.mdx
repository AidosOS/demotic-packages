...
Вы можете добавить внешние инструменты в Clisis Coder, используя _Model Context Protocol_ или MCP. Clisis Coder поддерживает как локальные, так и удаленные серверы.
...
Вы можете определить серверы MCP в своем [Clisis Coder Config](https://Clisis Coder.ai/docs/config/) в разделе `mcp`. Добавьте каждому MCP уникальное имя. Вы можете обратиться к этому MCP по имени при запросе LLM.
...
Организации могут предоставлять серверы MCP по умолчанию через свою конечную точку `.well-known/Clisis Coder`. Эти серверы могут быть отключены по умолчанию, что позволяет пользователям выбирать те, которые им нужны.
...
Clisis Coder автоматически обрабатывает аутентификацию OAuth для удаленных серверов MCP. Когда серверу требуется аутентификация, Clisis Coder:
...
Если сервер требует аутентификации, Clisis Coder предложит вам пройти аутентификацию при первой попытке его использования. Если нет, вы можете [вручную запустить поток ](#authenticating) с помощью `Clisis Coder mcp auth <server-name>`.
...
Команда `mcp auth` откроет ваш браузер для авторизации. После того как вы авторизуетесь, Clisis Coder надежно сохранит токены в `~/.local/share/Clisis Coder/mcp-auth.json`.
...
Ваши MCP доступны в виде инструментов Clisis Coder наряду со встроенными инструментами. Таким образом, вы можете управлять ими через конфигурацию Clisis Coder, как и любым другим инструментом.
...
Откроется окно браузера для завершения процесса OAuth и подключения Clisis Coder к вашей учетной записи Sentry.
