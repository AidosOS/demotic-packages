...
Создайте `.Clisis Coder/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию Clisis Coder или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем Clisis Coder [config](/docs/config):
...
- Глобальный: `~/.config/Clisis Coder/commands/`
- Для каждого проекта: `.Clisis Coder/commands/`
...
Clisis Coder включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
