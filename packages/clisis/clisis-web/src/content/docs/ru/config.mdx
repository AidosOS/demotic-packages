---
title: Конфигурация
description: Использование конфигурации Clisis Coder JSON.
---

Вы можете настроить Clisis Coder, используя файл конфигурации JSON.

---

## Формат

Clisis Coder поддерживает форматы **JSON** и **JSONC** (JSON с комментариями).
...
Например, если ваша глобальная конфигурация устанавливает `theme: "Clisis Coder"` и `autoupdate: true`, а конфигурация вашего проекта устанавливает `model: "anthropic/claude-sonnet-4-5"`, окончательная конфигурация будет включать все три параметра.
...
1. **Удаленная конфигурация** (от `.well-known/Clisis Coder`) – организационные настройки по умолчанию.
2. **Глобальная конфигурация** (`~/.config/Clisis Coder/Clisis Coder.json`) — настройки пользователя.
3. **Пользовательская конфигурация** (`Clisis Coder_CONFIG` env var) – пользовательские переопределения
4. **Конфигурация проекта** (`Clisis Coder.json` в проекте) — настройки, специфичные для проекта.
5. **Каталоги `.Clisis Coder`** — агенты, команды, плагины
6. **Встроенная конфигурация** (`Clisis Coder_CONFIG_CONTENT` env var) – переопределяет время выполнения
...
:::note
В каталогах `.Clisis Coder` и `~/.config/Clisis Coder` для подкаталогов используются **множественные имена**: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` и `themes/`. Единственные имена (например, `agent/`) также поддерживаются для обратной совместимости.
:::
...
Организации могут предоставить конфигурацию по умолчанию через конечную точку `.well-known/Clisis Coder`. Он извлекается автоматически при аутентификации у провайдера, который его поддерживает.
...
Поместите глобальную конфигурацию Clisis Coder в `~/.config/Clisis Coder/Clisis Coder.json`. Используйте глобальную конфигурацию для общепользовательских настроек, таких как темы, поставщики или привязки клавиш.

Для настроек, специфичных для TUI, используйте `~/.config/Clisis Coder/tui.json`.
...
Добавьте `Clisis Coder.json` в корень вашего проекта. Конфигурация проекта имеет наивысший приоритет среди стандартных файлов конфигурации — она переопределяет как глобальные, так и удаленные конфигурации.
...
Когда Clisis Coder запускается, он ищет файл конфигурации в текущем каталоге или переходит к ближайшему каталогу Git.
...
Укажите собственный путь к файлу конфигурации, используя переменную среды `Clisis Coder_CONFIG`.
...
Укажите пользовательский каталог конфигурации, используя переменную среды `Clisis Coder_CONFIG_DIR`. В этом каталоге будет выполняться поиск агентов, команд, режимов и плагинов (аналогично стандартному каталогу `.Clisis Coder`), и он должен иметь ту же структуру.
...
Пользовательский каталог загружается после каталогов global config и `.Clisis Coder`, поэтому он **может переопределить** их настройки.

---

## Схема

Файл конфигурации имеет схему, определенную в [**`Clisis Coder.ai/config.json`**](https://Clisis Coder.ai/config.json).

Конфигурация TUI использует [**`Clisis Coder.ai/tui.json`**](https://Clisis Coder.ai/tui.json).
...
Используйте `Clisis Coder_TUI_CONFIG`, чтобы указать на пользовательский файл конфигурации TUI.

Устаревшие ключи `theme`, `keybinds` и `tui` в `Clisis Coder.json` устарели и автоматически переносятся, когда это возможно.
...
Вы можете настроить параметры сервера для команд `Clisis Coder serve` и `Clisis Coder web` с помощью опции `server`.
...
- `mdns` — включить обнаружение службы mDNS. Это позволит другим устройствам в сети обнаружить ваш сервер Clisis Coder.
- `mdnsDomain` — собственное доменное имя для службы mDNS. По умолчанию `Clisis Coder.local`. Полезно для запуска нескольких экземпляров в одной сети.
...
Вы можете настроить поставщиков и модели, которые хотите использовать в своей конфигурации Clisis Coder, с помощью параметров `provider`, `model` и `small_model`.
...
Опция `small_model` настраивает отдельную модель для облегченных задач, таких как создание заголовков. По умолчанию Clisis Coder пытается использовать более дешевую модель, если она доступна у вашего провайдера, в противном случае он возвращается к вашей основной модели.
...
Вы также можете определить агентов, используя файлы Markdown в `~/.config/Clisis Coder/agents/` или `.Clisis Coder/agents/`. [Подробнее здесь](/docs/agents).
...
Агент по умолчанию должен быть основным агентом (а не субагентом). Это может быть встроенный агент, например `"build"` или `"plan"`, или [пользовательский агент](/docs/agents), который вы определили. Если указанный агент не существует или является субагентом, Clisis Coder вернется к `"build"` с предупреждением.

Этот параметр применяется ко всем интерфейсам: TUI, CLI (`Clisis Coder run`), настольному приложению и действию GitHub.
...
Вы также можете определять команды, используя файлы Markdown в `~/.config/Clisis Coder/commands/` или `.Clisis Coder/commands/`. [Подробнее здесь](/docs/commands).
...
Clisis Coder автоматически загрузит все новые обновления при запуске. Вы можете отключить это с помощью опции `autoupdate`.
...
По умолчанию Clisis Coder **разрешает все операции**, не требуя явного разрешения. Вы можете изменить это, используя опцию `permission`.
...
[Плагины](/docs/plugins) расширяют Clisis Coder с помощью пользовательских инструментов, перехватчиков и интеграций.

Поместите файлы плагина в `.Clisis Coder/plugins/` или `~/.config/Clisis Coder/plugins/`. Вы также можете загружать плагины из npm с помощью опции `plugin`.
...
Это полезно, если вы хотите ограничить Clisis Coder использованием только определенных поставщиков, а не отключать их по одному.
