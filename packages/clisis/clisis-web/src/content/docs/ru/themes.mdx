...
С помощью Clisis Coder вы можете выбрать одну из нескольких встроенных тем, использовать тему, которая адаптируется к теме вашего терминала, или определить свою собственную тему.

По умолчанию Clisis Coder использует нашу собственную тему `Clisis Coder`.
...
Clisis Coder поставляется с несколькими встроенными темами.
...
- Хотите, чтобы Clisis Coder соответствовал внешнему виду их терминала
...
Clisis Coder поддерживает гибкую систему тем на основе JSON, которая позволяет пользователям легко создавать и настраивать темы.
...
2. **Каталог конфигурации пользователя** – определяется в `~/.config/Clisis Coder/themes/*.json` или `$XDG_CONFIG_HOME/Clisis Coder/themes/*.json`.
3. **Корневой каталог проекта** – определено в `<project-root>/.Clisis Coder/themes/*.json`.
4. **Текущий рабочий каталог** – определено в `./.Clisis Coder/themes/*.json`.
