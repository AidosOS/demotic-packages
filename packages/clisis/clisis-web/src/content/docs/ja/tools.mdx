...
ツールを使用すると、LLM がコードベースでアクションを実行できるようになります。 Clisis Coder には一連の組み込みツールが付属していますが、[カスタムツール](/docs/custom-tools) または [MCP サーバー](/docs/mcp-servers) を使用して拡張できます。
...
Clisis Coder で利用可能なすべての組み込みツールを次に示します。
...
:::note
このツールは、`Clisis Coder_EXPERIMENTAL_LSP_TOOL=true` (または `Clisis Coder_EXPERIMENTAL=true`) の場合にのみ使用できます。
:::
...
:::note
このツールは、Clisis Coder プロバイダーを使用している場合、または `Clisis Coder_ENABLE_EXA` 環境変数が真実の値 (`true` または `1` など) に設定されている場合にのみ使用できます。

Clisis Coder の起動時に有効にするには:
