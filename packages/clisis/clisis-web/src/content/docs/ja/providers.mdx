---
title: プロバイダー
description: Clisis Coder で LLM プロバイダーを使用する。
---

import config from "../../../../config.mjs"
export const console = config.console

Clisis Coder は [AI SDK](https://ai-sdk.dev/) および [Models.dev](https://models.dev) を使用して **75 以上の LLM プロバイダー** をサポートし、ローカルモデルの実行をサポートします。

プロバイダーを追加するには、次のことを行う必要があります。

1. `/connect` コマンドを使用して、プロバイダーの API キーを追加します。
2. Clisis Coder 設定でプロバイダーを構成します。
...
`~/.local/share/Clisis Coder/auth.json`で。

---

### 設定

Clisis Coder の `provider` セクションを通じてプロバイダーをカスタマイズできます。
config.
...
Clisis Coder Zen は、Clisis Coder チームが提供するモデルのリストです。
Clisis Coder で適切に動作することがテストおよび検証されています。 [詳細はこちら](/docs/zen)。

:::tip
初めての方は、Clisis Coder Zen から始めることをお勧めします。
:::

1. TUI で `/connect` コマンドを実行し、`Clisis Coder Zen` を選択して、[Clisis Coder.ai/zen](https://Clisis Coder.ai/zen) にアクセスします。
...
これは Clisis Coder の他のプロバイダーと同様に機能し、使用は完全にオプションです。

---

## Clisis Coder Go

Clisis Coder Go は、Clisis Coder チームによって提供される、人気のあるオープンコーディングモデルへの信頼性の高いアクセスを提供する低コストのサブスクリプションプランです。これらは Clisis Coder でうまく機能することがテストおよび検証されています。

1. TUI で `/connect` コマンドを実行し、`Clisis Coder Go` を選択して、[Clisis Coder.ai/zen](https://Clisis Coder.ai/zen) にアクセスします。
...
Clisis Coder で Amazon Bedrock を使用するには:
...
2. **次のいずれかの方法を使用して認証を構成します**:
   ...
   ***
   ...
   Clisis Coder の実行中に次の環境変数のいずれかを設定します。
   ...
   ***
   ...
   プロジェクト固有の設定または永続的な設定の場合は、`Clisis Coder.json` を使用します。
...
:::info
Clisis Coder での Claude Pro/Max サブスクリプションの使用は、[Anthropic](https://anthropic.com) によって正式に許可されていませんが、一般的に機能します。
:::
...
Clisis Coder は、OpenAI 互換の API サーバーの背後でローカル LLM を実行するデスクトップアプリケーション [Atomic Chat](https://atomic.chat) 経由でローカルモデルを使うように設定できます（デフォルトのエンドポイントは `http://127.0.0.1:1337/v1`）。
...
:::note
Clisis Coder が正しく動作するには、デプロイメント名がモデル名と一致する必要があります。
:::
...
Clisis Coder 設定を通じてモデルを追加することもできます。
...
トークンを Clisis Coder 認証ストレージに保存します。
:::

##### 自己ホスト型 GitLab

:::note[コンプライアンスメモ]
Clisis Coder は、セッションタイトルの生成などの一部の AI タスクに小規模なモデルを使用します。
デフォルトでは、Zen によってホストされる gpt-5-nano を使用するように構成されています。 Clisis Coder をロックするには
自分の GitLab でホストされているインスタンスのみを使用するには、次の行を
`Clisis Coder.json` ファイル。セッション共有を無効にすることもお勧めします。
...
詳細については、[Clisis Coder-gitlab-auth](https://www.npmjs.com/package/Clisis Coder-gitlab-auth) ホームページ。

##### 設定

`Clisis Coder.json` を通じてカスタマイズします。
...
GitHub Copilot サブスクリプションを Clisis Coder で使用するには:
...
Clisis Coder で Google Vertex AI を使用するには:
...
Clisis Coder を通じて自動的に構成されていない Helicone の機能またはモデルを見つけた場合は、いつでも自分で構成できます。
...
Helicone の [Sessions](https://docs.helicone.ai/features/sessions) 機能を使用すると、関連する LLM リクエストをグループ化できます。 [Clisis Coder-helicone-session](https://github.com/H2Shami/Clisis Coder-helicone-session) プラグインを使用して、各 Clisis Coder 会話を Helicone のセッションとして自動的に記録します。
...
プラグインは、リクエストに `Helicone-Session-Id` ヘッダーと `Helicone-Session-Name` ヘッダーを挿入します。 Helicone のセッションページでは、Clisis Coder の各会話が個別のセッションとしてリストされています。
...
[llama.cpp の s](https://github.com/ggml-org/llama.cpp) llama-server ユーティリティ] を通じて、ローカルモデルを使用するように Clisis Coder を構成できます。
...
LM Studio を通じてローカルモデルを使用するように Clisis Coder を構成できます。
...
Ollama を通じてローカルモデルを使用するように Clisis Coder を構成できます。

:::tip
Ollama は Clisis Coder 用に自動的に構成できます。詳細については、[Ollama 統合 docs](https://docs.ollama.com/integrations/Clisis Coder)」を参照してください。
:::
...
6. **重要**: Clisis Coder でクラウドモデルを使用する前に、モデル情報をローカルに取得する必要があります。
...
Clisis Coder Zen は、Clisis Coder チームによって提供される、テストおよび検証されたモデルのリストです。 [詳細はこちら](/docs/zen)。

1. **<a href={console}>Clisis Coder Zen</a>** にサインインし、**API キーの作成** をクリックします。

2. `/connect` コマンドを実行し、**Clisis Coder Zen** を検索します。

   ```txt
   /connect
   ```

3. Clisis Coder API キーを入力します。
...
Clisis Coder 設定を通じて追加のモデルを追加することもできます。
...
5. Clisis Coder 設定を通じてカスタマイズすることもできます。プロバイダーを指定する例を次に示します。
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) を Clisis Coder で使用するには:
...
Clisis Coder 設定を通じてモデルをカスタマイズすることもできます。プロバイダーのルーティング順序を指定する例を次に示します。
...
:::tip
OpenAI と互換性のある任意のプロバイダーを Clisis Coder で使用できます。最新の AI プロバイダーのほとんどは、OpenAI 互換の API を提供しています。
:::
...
4. プロジェクトディレクトリで `Clisis Coder.json` ファイルを作成または更新します。
...
`limit` フィールドを使用すると、Clisis Coder はコンテキストがどのくらい残っているかを理解できます。標準プロバイダーは、これらを models.dev から自動的に取得します。
...
1. **認証設定を確認します**: `Clisis Coder auth list` を実行して、資格情報が正しいかどうかを確認します。
   プロバイダー用のものが設定に追加されます。

これは、認証に環境変数に依存する Amazon Bedrock のようなプロバイダーには当てはまりません。

2. カスタムプロバイダーの場合は、Clisis Coder 設定を確認し、次のことを行います。
   - `/connect` コマンドで使用されるプロバイダー ID が、Clisis Coder 設定内の ID と一致することを確認してください。
