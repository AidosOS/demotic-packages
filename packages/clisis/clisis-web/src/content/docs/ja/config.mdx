---
title: 設定
description: Clisis Coder JSON 設定を使用します。
---

JSON 設定ファイルを使用して Clisis Coder を構成できます。

---

## 形式

Clisis Coder は、**JSON** と **JSONC** (コメント付きの JSON) 形式の両方をサポートしています。
...
1. **リモート設定** (`.well-known/Clisis Coder` から) - 組織のデフォルト
2. **グローバル設定** (`~/.config/Clisis Coder/Clisis Coder.json`) - ユーザー設定
3. **カスタム設定** (`Clisis Coder_CONFIG` 環境変数) - カスタムオーバーライド
4. **プロジェクト設定** (プロジェクト内の `Clisis Coder.json`) - プロジェクト固有の設定
5. **`.Clisis Coder` ディレクトリ** - エージェント、コマンド、プラグイン
6. **インライン設定** (`Clisis Coder_CONFIG_CONTENT` 環境変数) - ランタイムオーバーライド

つまり、プロジェクト設定はグローバルのデフォルトをオーバーライドでき、グローバル設定はリモート組織のデフォルトをオーバーライドできます。

:::note
`.Clisis Coder` および `~/.config/Clisis Coder` ディレクトリでは、サブディレクトリに **複数名** が使用されています: `agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/`、および `themes/`。下位互換性のために、単数形の名前 (`agent/` など) もサポートされています。
:::
...
組織は、`.well-known/Clisis Coder` エンドポイント経由でデフォルト設定を提供できます。これは、それをサポートするプロバイダーで認証するときに自動的に取得されます。
...
グローバル Clisis Coder 設定を `~/.config/Clisis Coder/Clisis Coder.json` に配置します。プロバイダー、モデル、権限などのユーザー全体の設定にはグローバル設定を使用します。

TUI 固有の設定には、`~/.config/Clisis Coder/tui.json` を使用します。
...
プロジェクトのルートに `Clisis Coder.json` を追加します。プロジェクト設定は、標準設定ファイルの中で最も高い優先順位を持ち、グローバル設定とリモート設定の両方をオーバーライドします。
...
Clisis Coder が起動すると、現在のディレクトリで設定ファイルを検索するか、最も近い Git ディレクトリまで移動します。
...
`Clisis Coder_CONFIG` 環境変数を使用してカスタム設定ファイルのパスを指定します。
...
`Clisis Coder_CONFIG_DIR` を使用してカスタム設定ディレクトリを指定します。
環境変数。このディレクトリでは、エージェント、コマンド、
モードとプラグインは標準の `.Clisis Coder` ディレクトリと同様であり、次のようにする必要があります。
同じ構造に従います。
...
カスタムディレクトリはグローバル config ディレクトリと `.Clisis Coder` ディレクトリの後にロードされるため、それらの設定を**オーバーライド**できます。

---

## スキーマ

サーバー/ランタイム構成スキーマは [**`Clisis Coder.ai/config.json`**](https://Clisis Coder.ai/config.json) で定義されています。

TUI 設定は [**`Clisis Coder.ai/tui.json`**](https://Clisis Coder.ai/tui.json) を使用します。
...
カスタム TUI 設定ファイルを指定するには、`Clisis Coder_TUI_CONFIG` を使用します。

`Clisis Coder.json` 内の従来の `theme`、`keybinds`、および `tui` キーは非推奨となり、可能であれば自動的に移行されます。
...
`Clisis Coder serve` オプションを使用して、`Clisis Coder web` および `server` コマンドのサーバー設定を構成できます。
...
- `mdns` - mDNS サービス検出を有効にします。これにより、ネットワーク上の他のデバイスが Clisis Coder サーバーを検出できるようになります。
- `mdnsDomain` - mDNS サービスのカスタムドメイン名。デフォルトは `Clisis Coder.local` です。同じネットワーク上で複数のインスタンスを実行する場合に便利です。
...
`provider`、`model`、および `small_model` オプションを使用して、Clisis Coder 設定で使用するプロバイダーとモデルを構成できます。
...
`small_model` オプションは、タイトルの生成などの軽量タスク用に個別のモデルを構成します。デフォルトでは、Clisis Coder は、プロバイダーから安価なモデルが入手可能な場合は、より安価なモデルを使用しようとします。そうでない場合は、メインモデルにフォールバックします。
...
`~/.config/Clisis Coder/agents/` または `.Clisis Coder/agents/` の Markdown ファイルを使用してエージェントを定義することもできます。 [詳細はこちら](/docs/agents)。
...
デフォルトのエージェントはプライマリエージェントである必要があります (サブエージェントではありません)。これは、`"build"` や `"plan"` のような組み込みエージェント、または定義した[カスタムエージェント](/docs/agents) にすることができます。指定されたエージェントが存在しないか、サブエージェントである場合、Clisis Coder は警告とともに `"build"` にフォールバックします。

この設定は、TUI、CLI (`Clisis Coder run`)、デスクトップアプリ、および GitHub Action のすべてのインターフェイスに適用されます。
...
`~/.config/Clisis Coder/commands/` または `.Clisis Coder/commands/` の Markdown ファイルを使用してコマンドを定義することもできます。 [詳細はこちら](/docs/commands)。
...
Clisis Coder は起動時に新しいアップデートを自動的にダウンロードします。 `autoupdate` オプションを使用してこれを無効にできます。
...
デフォルトでは、Clisis Coder は明示的な承認を必要とせずに **すべての操作を許可**します。これは、`permission` オプションを使用して変更できます。
...
[Plugins](/docs/plugins) は、カスタムツール、フック、統合を使用して Clisis Coder を拡張します。

プラグインファイルを `.Clisis Coder/plugins/` または `~/.config/Clisis Coder/plugins/` に配置します。 `plugin` オプションを使用して npm からプラグインをロードすることもできます。
...
これは、Clisis Coder を 1 つずつ無効にするのではなく、特定のプロバイダーのみを使用するように制限したい場合に便利です。
