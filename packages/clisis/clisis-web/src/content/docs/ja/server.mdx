---
title: サーバー
description: HTTP 経由で Clisis Coder サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`Clisis Coder serve` コマンドは、Clisis Coder クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `Clisis Coder.local` |
...
HTTP 基本認証でサーバーを保護するには、`Clisis Coder_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `Clisis Coder` になるか、`Clisis Coder_SERVER_USERNAME` を設定してオーバーライドします。これは、`Clisis Coder serve` と `Clisis Coder web` の両方に当てはまります。
...
`Clisis Coder` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Clisis Coder サーバーを使用して、プログラムで Clisis Coder と対話します。
:::

`Clisis Coder serve` を実行してスタンドアロンサーバーを起動できます。すでに Clisis Coder TUI を実行している場合でも、`Clisis Coder serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Clisis Coder [IDE](/docs/ide) プラグインによって使用されます。
...
Clisis Coder サーバーは次の API を公開します。
