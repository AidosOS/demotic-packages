...
Clisis Coder には 2 種類のエージェントがあります。プライマリエージェントとサブエージェント。
...
Clisis Coder には、**Build** と **Plan** という 2 つの組み込みプライマリエージェントが付属しています。これらについて以下で説明します。
...
Clisis Coder には、**General**、**Explore**、**Scout** という 3 つの組み込みサブエージェントが付属しています。これについては以下で見ていきます。

---

## 組み込み

Clisis Coder には、2 つの組み込みプライマリエージェントと 3 つの組み込みサブエージェントが付属しています。
...
外部ドキュメントや依存関係の調査を行うための読み取り専用エージェントです。依存関係のリポジトリを Clisis Coder の管理キャッシュにクローンしたいとき、ライブラリのソースコードを調べたいとき、あるいはワークスペースを変更せずにローカルコードを upstream の実装と突き合わせたいときに使用します。
...
`Clisis Coder.json` 設定ファイルでエージェントを構成します。
...
- グローバル: `~/.config/Clisis Coder/agents/`
- プロジェクトごと: `.Clisis Coder/agents/`
...
温度が指定されていない場合、Clisis Coder はモデル固有のデフォルトを使用します。通常、ほとんどのモデルでは 0、Qwen モデルでは 0.55 です。
...
このパスは、設定ファイルが配置されている場所に対する相対パスです。したがって、これはグローバルな Clisis Coder 設定とプロジェクト固有の設定の両方で機能します。
...
Clisis Coder 設定内のモデル ID は、`provider/model-id` という形式を使用します。たとえば、[Clisis Coder Zen](/docs/zen) を使用している場合、GPT 5.1 Codex には `Clisis Coder/gpt-5.1-codex` を使用します。
...
:::tip
`Clisis Coder models` を実行して、利用可能なモデルのリストを表示します。
:::
...
:::tip
共有したいエージェントはいますか? [PR](https://github.com/AidosOS/demotic-clisis-coder) を送信します。
:::
