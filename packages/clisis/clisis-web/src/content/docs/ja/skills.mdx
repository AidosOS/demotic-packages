...
エージェントスキルにより、Clisis Coder はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
Clisis Coder は次の場所を検索します。

- プロジェクト設定: `.Clisis Coder/skills/<name>/SKILL.md`
- グローバル設定: `~/.config/Clisis Coder/skills/<name>/SKILL.md`
...
プロジェクトのローカルパスの場合、Clisis Coder は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
途中で、一致する `skills/*/SKILL.md` を `.Clisis Coder/` に読み込み、一致する `.claude/skills/*/SKILL.md` または `.agents/skills/*/SKILL.md` を読み込みます。

グローバル定義は、`~/.config/Clisis Coder/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md`、および `~/.agents/skills/*/SKILL.md` からもロードされます。
...
次のように `.Clisis Coder/skills/git-release/SKILL.md` を作成します。
...
Clisis Coder では、`skill` ツールの説明に利用可能なスキルがリストされています。
...
`Clisis Coder.json` のパターンベースの権限を使用して、エージェントがアクセスできるスキルを制御します。
...
**組み込みエージェントの場合** (`Clisis Coder.json` 内):
