---
title: الخادم
description: تفاعل مع خادم Clisis Coder عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `Clisis Coder serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل Clisis Coder استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `Clisis Coder.local` |
...
عيّن `Clisis Coder_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `Clisis Coder`، أو عيّن `Clisis Coder_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `Clisis Coder serve` و `Clisis Coder web`.
...
عند تشغيل `Clisis Coder` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم Clisis Coder للتفاعل مع Clisis Coder برمجيا.
:::

تتيح هذه البنية لـ Clisis Coder دعم عدة عملاء وتمكّنك من التفاعل مع Clisis Coder برمجيا.

يمكنك تشغيل `Clisis Coder serve` لبدء خادم مستقل. إذا كانت واجهة Clisis Coder في terminal (TUI)
قيد التشغيل، فسيبدأ `Clisis Coder serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Clisis Coder لـ [IDE](/docs/ide).
...
يعرض خادم Clisis Coder واجهات API التالية.
