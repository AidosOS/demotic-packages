---
title: المزوّدون
description: استخدام أي موفّر LLM في Clisis Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Clisis Coder ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Clisis Coder.
...
في `~/.local/share/Clisis Coder/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Clisis Coder.
...
Clisis Coder Zen هي قائمة نماذج يوفّرها فريق Clisis Coder وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Clisis Coder. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Clisis Coder Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر Clisis Coder، ثم انتقل إلى [Clisis Coder.ai/auth](https://Clisis Coder.ai/auth).
...
يعمل مثل أي مزوّد آخر في Clisis Coder واستخدامه اختياري بالكامل.

---

## Clisis Coder Go

Clisis Coder Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Clisis Coder، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Clisis Coder.

1. شغّل الأمر `/connect` في TUI، واختر `Clisis Coder Go`، ثم انتقل إلى [Clisis Coder.ai/auth](https://Clisis Coder.ai/zen).
...
لاستخدام Amazon Bedrock مع Clisis Coder:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل Clisis Coder:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `Clisis Coder.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Clisis Coder غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين Clisis Coder لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل Clisis Coder بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات Clisis Coder.
...
تخزين الرمز في مساحة تخزين مصادقة Clisis Coder.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Clisis Coder نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Clisis Coder
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`Clisis Coder.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [Clisis Coder-gitlab-auth](https://www.npmjs.com/package/Clisis Coder-gitlab-auth).

##### التهيئة

خصّص عبر `Clisis Coder.json`:
...
لاستخدام اشتراك GitHub Copilot مع Clisis Coder:
...
لاستخدام Google Vertex AI مع Clisis Coder:
...
2. عيّن متغيرات البيئة المطلوبة:
   ...
   عيّنها أثناء تشغيل Clisis Coder.
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر Clisis Coder، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [Clisis Coder-helicone-session](https://github.com/H2Shami/Clisis Coder-helicone-session) لتسجيل كل محادثة Clisis Coder تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Clisis Coder مدرجة كجلسة مستقلة.
...
يمكنك ضبط Clisis Coder لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط Clisis Coder لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط Clisis Coder لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Clisis Coder. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/Clisis Coder) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Clisis Coder:
...
3. انسخ مفتاح API لاستخدامه في Clisis Coder.
...
6. **مهم**: قبل استخدام نماذج السحابة في Clisis Coder، يجب سحب معلومات النموذج محليا:
...
Clisis Coder Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Clisis Coder. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Clisis Coder Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Clisis Coder Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Clisis Coder.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات Clisis Coder.
...
5. يمكنك أيضا تخصيصها عبر إعدادات Clisis Coder. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Clisis Coder:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات Clisis Coder. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع Clisis Coder. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `Clisis Coder.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Clisis Coder بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `Clisis Coder auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات Clisis Coder وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات Clisis Coder.
