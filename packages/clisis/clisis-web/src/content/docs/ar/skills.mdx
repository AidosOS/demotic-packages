...
تتيح مهارات الوكيل لـ Clisis Coder اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Clisis Coder في هذه المواقع:

- إعدادات المشروع: `.Clisis Coder/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/Clisis Coder/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Clisis Coder صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.Clisis Coder/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/Clisis Coder/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.Clisis Coder/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Clisis Coder المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `Clisis Coder.json`:
...
**للوكلاء المدمجين** (في `Clisis Coder.json`):
