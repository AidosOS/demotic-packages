...
يمكنك إضافة أدوات خارجية إلى Clisis Coder باستخدام _Model Context Protocol_، أو MCP. يدعم Clisis Coder خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Clisis Coder](https://Clisis Coder.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/Clisis Coder` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Clisis Coder تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Clisis Coder بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Clisis Coder المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `Clisis Coder mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Clisis Coder الرموز بشكل آمن في `~/.local/share/Clisis Coder/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Clisis Coder إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Clisis Coder مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Clisis Coder بحساب Sentry الخاص بك.
