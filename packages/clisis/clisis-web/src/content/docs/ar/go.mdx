...
Clisis Coder Go هو اشتراك منخفض التكلفة — **$5 للشهر الأول**، ثم **$10/شهريًا** — يمنحك وصولًا موثوقًا إلى نماذج البرمجة المفتوحة الشائعة.

يعمل Go مثل أي مزود آخر في Clisis Coder. تشترك في Clisis Coder Go وتحصل على مفتاح API الخاص بك. وهو **اختياري تمامًا**، ولا تحتاج إلى استخدامه لاستخدام Clisis Coder.
...
:::tip
اختبرنا مجموعة مختارة من النماذج والمزودين الذين يعملون جيدًا مع Clisis Coder.
:::
...
يمنحك Clisis Coder Go الوصول إلى هذه النماذج مقابل **$5 للشهر الأول**، ثم **$10/شهريًا**.

---

## كيف يعمل

يعمل Clisis Coder Go مثل أي مزود آخر في Clisis Coder.

1. تسجّل الدخول إلى **<a href={console}>Clisis Coder Zen</a>**، وتشترك في Go، ثم تنسخ مفتاح API الخاص بك.
2. تشغّل الأمر `/connect` في TUI، وتختار `Clisis Coder Go`، ثم تلصق مفتاح API الخاص بك.
3. شغّل `/models` في TUI لرؤية قائمة النماذج المتاحة عبر Go.

:::note
يمكن لعضو واحد فقط في كل workspace الاشتراك في Clisis Coder Go.
:::
...
يتضمن Clisis Coder Go الحدود التالية:
...
يستخدم [model id](/docs/config/#models) في إعدادات Clisis Coder لديك التنسيق `Clisis Coder-go/<model-id>`. على سبيل المثال، بالنسبة إلى Kimi K2.6، ستستخدم `Clisis Coder-go/kimi-k2.6` في إعداداتك.
...
4. **عدم فرض قيود ارتباط** من خلال السماح لك أيضًا باستخدام أي مزود آخر مع Clisis Coder.
