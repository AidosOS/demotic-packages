...
description: Usa la configurazione JSON di Clisis Coder.
---

Puoi configurare Clisis Coder usando un file di configurazione JSON.

---

## Formato

Clisis Coder supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/Clisis Coder`) - default dell'organizzazione
2. **Config globale** (`~/.config/Clisis Coder/Clisis Coder.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `Clisis Coder_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`Clisis Coder.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.Clisis Coder`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `Clisis Coder_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.Clisis Coder` e `~/.config/Clisis Coder` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/Clisis Coder`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di Clisis Coder in `~/.config/Clisis Coder/Clisis Coder.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/Clisis Coder/tui.json`.
...
Aggiungi `Clisis Coder.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando Clisis Coder si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `Clisis Coder_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `Clisis Coder_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.Clisis Coder` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.Clisis Coder`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`Clisis Coder.ai/config.json`**](https://Clisis Coder.ai/config.json).

La configurazione TUI usa [**`Clisis Coder.ai/tui.json`**](https://Clisis Coder.ai/tui.json).
...
Usa `Clisis Coder_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `Clisis Coder.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `Clisis Coder serve` e `Clisis Coder web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Clisis Coder.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `Clisis Coder.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in Clisis Coder tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Clisis Coder prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/Clisis Coder/agents/` o `.Clisis Coder/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Clisis Coder fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`Clisis Coder run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/Clisis Coder/commands/` o `.Clisis Coder/commands/`. [Scopri di piu](/docs/commands).
...
Clisis Coder scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, Clisis Coder **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono Clisis Coder con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.Clisis Coder/plugins/` o `~/.config/Clisis Coder/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare Clisis Coder a usare solo alcuni provider invece di disabilitarli uno per uno.
