...
In Clisis Coder esistono due tipi di agenti: agenti primari e subagenti.
...
Clisis Coder include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Clisis Coder include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

Clisis Coder include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di Clisis Coder, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `Clisis Coder.json`:
...
- Globale: `~/.config/Clisis Coder/agents/`
- Per progetto: `.Clisis Coder/agents/`
...
Se non specifichi una temperature, Clisis Coder usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Clisis Coder sia per la config specifica del progetto.
...
L'ID modello nella configurazione Clisis Coder usa il formato `provider/model-id`. Per esempio, se stai usando [Clisis Coder Zen](/docs/zen), useresti `Clisis Coder/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `Clisis Coder models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/AidosOS/demotic-clisis-coder).
:::
