---
title: Provider
description: Usa qualsiasi provider LLM in Clisis Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Clisis Coder usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Clisis Coder.
...
configurazione di Clisis Coder.
...
Clisis Coder Zen e una lista di modelli forniti dal team Clisis Coder, testati e
verificati per funzionare bene con Clisis Coder. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Clisis Coder Zen.
:::

1. Esegui `/connect` nella TUI, seleziona Clisis Coder e vai su [Clisis Coder.ai/auth](https://Clisis Coder.ai/auth).
...
Funziona come qualsiasi altro provider in Clisis Coder ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Clisis Coder:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui Clisis Coder:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `Clisis Coder.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Clisis Coder non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare Clisis Coder per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché Clisis Coder funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di Clisis Coder.
...
2. Scegli il tuo metodo di autenticazione:
   ...
   3. Nome: `Clisis Coder`, Scopes: `api`
...
salvare il token nello storage di auth di Clisis Coder.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Clisis Coder usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Clisis Coder
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `Clisis Coder.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [Clisis Coder-gitlab-auth](https://www.npmjs.com/package/Clisis Coder-gitlab-auth).

##### Configurazione

Personalizza tramite `Clisis Coder.json`:
...
Per usare il tuo abbonamento GitHub Copilot con Clisis Coder:
...
Per usare Google Vertex AI con Clisis Coder:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui Clisis Coder.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da Clisis Coder, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [Clisis Coder-helicone-session](https://github.com/H2Shami/Clisis Coder-helicone-session) per loggare automaticamente ogni conversazione di Clisis Coder come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Clisis Coder elencata come sessione separata.
...
Puoi configurare Clisis Coder per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare Clisis Coder per usare modelli locali tramite LM Studio.
...
Puoi configurare Clisis Coder per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Clisis Coder. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/Clisis Coder) per dettagli.
:::
...
Per usare Ollama Cloud con Clisis Coder:
...
3. Copia la chiave API da usare in Clisis Coder.
...
6. **Importante**: Prima di usare modelli cloud in Clisis Coder, devi scaricare le informazioni del modello localmente:
...
Clisis Coder Zen è una lista di modelli testati e verificati forniti dal team Clisis Coder. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Clisis Coder Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Clisis Coder Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Clisis Coder.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di Clisis Coder.
...
5. Puoi anche personalizzarli tramite la configurazione di Clisis Coder. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Clisis Coder:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di Clisis Coder. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con Clisis Coder. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `Clisis Coder.json` nella directory del progetto:
...
I campi `limit` permettono a Clisis Coder di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `Clisis Coder auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di Clisis Coder e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config Clisis Coder.
