---
title: Server
description: Interagisci con il server Clisis Coder via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `Clisis Coder serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client Clisis Coder.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `Clisis Coder.local` |
...
Imposta `Clisis Coder_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `Clisis Coder`, oppure imposta `Clisis Coder_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `Clisis Coder serve` sia per `Clisis Coder web`.
...
Quando esegui `Clisis Coder` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server Clisis Coder per interagire con Clisis Coder in modo programmatico.
:::

Questa architettura permette a Clisis Coder di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `Clisis Coder serve` per avviare un server standalone. Se la TUI di Clisis Coder e gia in esecuzione, `Clisis Coder serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Clisis Coder.
...
Il server Clisis Coder espone le seguenti API.
