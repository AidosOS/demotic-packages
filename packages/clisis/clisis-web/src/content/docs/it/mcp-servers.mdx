...
Puoi aggiungere strumenti esterni a Clisis Coder usando il _Model Context Protocol_, o MCP. Clisis Coder supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Clisis Coder Config](https://Clisis Coder.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/Clisis Coder`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Clisis Coder gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Clisis Coder:
...
Se il server richiede autenticazione, Clisis Coder ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `Clisis Coder mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Clisis Coder memorizzera' i token in modo sicuro in `~/.local/share/Clisis Coder/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Clisis Coder insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Clisis Coder come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Clisis Coder al tuo account Sentry.
