...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. Clisis Coder include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in Clisis Coder.
...
:::note
Questo strumento e' disponibile solo quando `Clisis Coder_EXPERIMENTAL_LSP_TOOL=true` (oppure `Clisis Coder_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider Clisis Coder o quando la variabile d'ambiente `Clisis Coder_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di Clisis Coder:
