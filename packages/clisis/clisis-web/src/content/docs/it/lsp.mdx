---
title: Server LSP
description: Clisis Coder si integra con i tuoi server LSP.
---

Clisis Coder può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

Clisis Coder include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `Clisis Coder_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e Clisis Coder apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di Clisis Coder.
