...
Clisis Coder Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Clisis Coder. Ti abboni a Clisis Coder Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare Clisis Coder.
...
Clisis Coder Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Clisis Coder Go funziona come qualsiasi altro provider in Clisis Coder.

1. Accedi a **<a href={console}>Clisis Coder Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Clisis Coder Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Clisis Coder Go.
:::
...
Clisis Coder Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Clisis Coder config
utilizza il formato `Clisis Coder-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`Clisis Coder-go/kimi-k2.6` nella tua configurazione.
...
Abbiamo creato Clisis Coder Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Clisis Coder.
