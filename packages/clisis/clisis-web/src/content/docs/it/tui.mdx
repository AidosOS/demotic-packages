---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Clisis Coder.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Clisis Coder fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Clisis Coder avvia la TUI per la directory corrente.
...
Quando usi la TUI di Clisis Coder, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Clisis Coder. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Clisis Coder. _Alias_: `/quit`, `/q`
...
Questo è separato da `Clisis Coder.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Clisis Coder attende dopo il leader key. Default: `2000`.
...
Usa `Clisis Coder_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
...
- `sound_pack` - ID del sound pack da usare. Default: `Clisis Coder.default`.
