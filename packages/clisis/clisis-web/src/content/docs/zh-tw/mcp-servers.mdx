...
您可以透過 _Model Context Protocol_（MCP）為 Clisis Coder 新增外部工具。Clisis Coder 同時支援本地和遠端伺服器。
...
您可以在 [Clisis Coder 設定](https://Clisis Coder.ai/docs/config/)的 `mcp` 欄位下定義 MCP 伺服器。為每個 MCP 指定一個唯一的名稱，在提示詞中可以透過該名稱來參照對應的 MCP。
...
組織可以透過其 `.well-known/Clisis Coder` 端點提供預設的 MCP 伺服器。這些伺服器可能預設處於停用狀態，允許使用者按需啟用。
...
Clisis Coder 會自動處理遠端 MCP 伺服器的 OAuth 身分驗證。當伺服器需要身分驗證時，Clisis Coder 將：
...
如果伺服器需要身分驗證，Clisis Coder 會在您首次使用時提示您進行認證。您也可以使用 `Clisis Coder mcp auth <server-name>` [手動觸發認證流程](#authenticating)。
...
`mcp auth` 指令會開啟瀏覽器進行授權。授權完成後，Clisis Coder 會將 Token 安全地儲存在 `~/.local/share/Clisis Coder/mcp-auth.json` 中。
...
您的 MCP 在 Clisis Coder 中作為工具使用，與內建工具並列。因此，您可以像管理其他工具一樣，透過 Clisis Coder 設定來管理它們。
...
這會開啟瀏覽器視窗完成 OAuth 流程，將 Clisis Coder 連結到您的 Sentry 帳號。
