---
title: LSP Servers
description: Clisis Coder integrates with your LSP servers.
---

Clisis Coder can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

Clisis Coder comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `Clisis Coder_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and Clisis Coder opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your Clisis Coder config.
...
```json title="Clisis Coder.json" {5-8}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="Clisis Coder.json" {5-13}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "lsp": {
    "custom-lsp": {
