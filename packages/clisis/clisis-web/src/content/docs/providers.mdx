---
title: Providers
description: Using any LLM provider in Clisis Coder.
---

import config from "../../../config.mjs"
export const console = config.console

Clisis Coder uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Clisis Coder config.
...
in `~/.local/share/Clisis Coder/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Clisis Coder
config.
...
```json title="Clisis Coder.json" {6}
{
...
Clisis Coder Zen is a list of models provided by the Clisis Coder team that have been
tested and verified to work well with Clisis Coder. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Clisis Coder Zen.
:::

1. Run the `/connect` command in the TUI, select `Clisis Coder Zen`, and head to [Clisis Coder.ai/auth](https://Clisis Coder.ai/zen).
...
It works like any other provider in Clisis Coder and is completely optional to use.

---

## Clisis Coder Go

Clisis Coder Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Clisis Coder team that have been
tested and verified to work well with Clisis Coder.

1. Run the `/connect` command in the TUI, select `Clisis Coder Go`, and head to [Clisis Coder.ai/auth](https://Clisis Coder.ai/zen).
...
To use Amazon Bedrock with Clisis Coder:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running Clisis Coder:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY Clisis Coder

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile Clisis Coder

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX Clisis Coder
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `Clisis Coder.json`:
...
Clisis Coder. Anthropic explicitly prohibits this.

Previous versions of Clisis Coder came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Clisis Coder with zero setup:
...
You can configure Clisis Coder to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="Clisis Coder.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for Clisis Coder to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX Clisis Coder
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX Clisis Coder
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your Clisis Coder config.
...
Clisis Coder supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Clisis Coder uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Clisis Coder. Sign in and approve.

   :::note
   Clisis Coder requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Clisis Coder surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Clisis Coder forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Clisis Coder is experimental. Features, configuration, and
behavior may change in future releases.
:::

Clisis Coder integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
2. Choose your authentication method:
   ...
   3. Name: `Clisis Coder`, Scopes: `api`
...
to store token in Clisis Coder auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Clisis Coder uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Clisis Coder
to only use your own GitLab-hosted instance, add the following to your
`Clisis Coder.json` file. It is also recommended to disable session sharing.
...
More documentation on [Clisis Coder-gitlab-auth](https://www.npmjs.com/package/Clisis Coder-gitlab-auth) homepage.

##### Configuration

Customize through `Clisis Coder.json`:
...
When a `duo-workflow-*` model is selected, Clisis Coder will:
...
4. Route tool execution requests through Clisis Coder's permission-gated tool system
...
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "plugin": ["Clisis Coder-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with Clisis Coder:
...
To use Google Vertex AI with Clisis Coder:
...
2. Set the required environment variables:
   ...
   Set them while running Clisis Coder.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id Clisis Coder
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through Clisis Coder, you can always configure it yourself.
...
```jsonc title="~/.config/Clisis Coder/Clisis Coder.jsonc"
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [Clisis Coder-helicone-session](https://github.com/H2Shami/Clisis Coder-helicone-session) plugin to automatically log each Clisis Coder conversation as a session in Helicone.

```bash
npm install -g Clisis Coder-helicone-session
```
...
```json title="Clisis Coder.json"
{
  "plugin": ["Clisis Coder-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Clisis Coder conversation listed as a separate session.
...
You can configure Clisis Coder to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="Clisis Coder.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure Clisis Coder to use local models through LM Studio.

```json title="Clisis Coder.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure Clisis Coder to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Clisis Coder. See the [Ollama integration docs](https://docs.ollama.com/integrations/Clisis Coder) for details.
:::

```json title="Clisis Coder.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Clisis Coder:
...
3. Copy the API key for use in Clisis Coder.
...
6. **Important**: Before using cloud models in Clisis Coder, you must pull the model information locally:
...
Clisis Coder Zen is a list of tested and verified models provided by the Clisis Coder team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Clisis Coder Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Clisis Coder Zen**.

   ```txt
   /connect
   ```

3. Enter your Clisis Coder API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your Clisis Coder config.
...
5. You can also customize them through your Clisis Coder config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' Clisis Coder
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group Clisis Coder
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Clisis Coder:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=Clisis Coder&utm_medium=docs&utm_campaign=cortex-provider).

Clisis Coder's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
Alternatively, set environment variables before starting Clisis Coder:
...
The model catalog is provided automatically. A minimal `Clisis Coder.json` is all that's needed:
...
You can also customize models through your Clisis Coder config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. Clisis Coder opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
Clisis Coder refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when Clisis Coder is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running Clisis Coder — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. Clisis Coder prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. Clisis Coder polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with Clisis Coder. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in Clisis Coder.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `Clisis Coder.json` file in your project directory:

   ```json title="Clisis Coder.json" ""myprovider"" {5-15}
   {
     "$schema": "https://Clisis Coder.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="Clisis Coder.json" {9,11,17-20}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Clisis Coder to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `Clisis Coder auth list` to see if the credentials
...
2. For custom providers, check the Clisis Coder config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your Clisis Coder config.
