...
Los proveedores más populares están precargados de forma predeterminada. Si agregó las credenciales de un proveedor mediante el comando `/connect`, estarán disponibles cuando inicie Clisis Coder.
...
Aqui tienes varios modelos que funcionan bien con Clisis Coder, sin orden particular. (Esta no es una lista exhaustiva ni necesariamente actualizada):
...
Aquí el ID completo es `provider_id/model_id`. Por ejemplo, si usa [Clisis Coder Zen](/docs/zen), usaría `Clisis Coder/gpt-5.1-codex` para GPT 5.1 Codex.
...
Muchos modelos admiten múltiples variantes con diferentes configuraciones. Clisis Coder se envía con variantes predeterminadas integradas para proveedores populares.

### Variantes integradas

Clisis Coder se envía con variantes predeterminadas para muchos proveedores:
...
Cuando se inicia Clisis Coder, busca modelos en el siguiente orden de prioridad:
...
2. La lista de modelos en la configuración Clisis Coder.
