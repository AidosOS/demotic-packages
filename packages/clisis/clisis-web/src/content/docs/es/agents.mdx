...
Hay dos tipos de agentes en Clisis Coder; agentes primarios y subagentes.
...
Clisis Coder viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
Clisis Coder viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

Clisis Coder viene con dos agentes primarios integrados y tres subagentes integrados.
...
Configure agentes en su archivo de configuración `Clisis Coder.json`:
...
- Por proyecto: `.Clisis Coder/agents/`
...
Si no se especifica ninguna temperatura, Clisis Coder utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global Clisis Coder como para la configuración específica del proyecto.
...
El ID del modelo en su configuración Clisis Coder usa el formato `provider/model-id`. Por ejemplo, si usa [Clisis Coder Zen](/docs/zen), usaría `Clisis Coder/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `Clisis Coder models` para ver una lista de los modelos disponibles.
:::
...
:::tip
¿Tiene un agente que le gustaría compartir? [Enviar un PR](https://github.com/AidosOS/demotic-clisis-coder).
:::
