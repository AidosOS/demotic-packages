...
Las herramientas permiten que LLM realice acciones en su código base. Clisis Coder viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Clisis Coder.
...
```json title="Clisis Coder.json" {4}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "permission": {
    "bash": "allow"
...
:::note
Esta herramienta solo está disponible cuando `Clisis Coder_EXPERIMENTAL_LSP_TOOL=true` (o `Clisis Coder_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Clisis Coder o cuando la variable de entorno `Clisis Coder_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Clisis Coder:

```bash
Clisis Coder_ENABLE_EXA=1 Clisis Coder
```
