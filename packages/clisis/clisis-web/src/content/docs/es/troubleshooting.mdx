...
Para depurar problemas con Clisis Coder, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\Clisis Coder\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `Clisis Coder --log-level DEBUG`.

---

## Almacenamiento

Clisis Coder almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/Clisis Coder/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\Clisis Coder`
...
Clisis Coder Desktop ejecuta un servidor Clisis Coder local (el sidecar `Clisis Coder-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Clisis Coder` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/Clisis Coder/Clisis Coder.jsonc` (o `~/.config/Clisis Coder/Clisis Coder.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/Clisis Coder/Clisis Coder.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\Clisis Coder\Clisis Coder.jsonc`
...
Clisis Coder también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/Clisis Coder/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\Clisis Coder\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Clisis Coder pueda reconstruirlo.

1. Salga de Clisis Coder Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/Clisis Coder`
- **Linux**: eliminar `~/.cache/Clisis Coder` (o ejecutar `rm -rf ~/.cache/Clisis Coder`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\Clisis Coder`

3. Reinicie el escritorio Clisis Coder.

---

### Solucionar problemas de conexión del servidor

Clisis Coder Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `Clisis Coder.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `Clisis Coder_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `Clisis Coder_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Clisis Coder requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Clisis Coder.

---

### Notificaciones que no se muestran

Clisis Coder Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Clisis Coder en la configuración de su sistema operativo, y
...
1. Salga del escritorio Clisis Coder.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Clisis Coder):

- `Clisis Coder.settings.dat` (URL del servidor predeterminado de escritorio)
- `Clisis Coder.global.dat` y `Clisis Coder.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Clisis Coder:
...
3. Asegúrese de tener la última versión con `Clisis Coder upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `Clisis Coder models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\Clisis Coder`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. Clisis Coder instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\Clisis Coder`

2. Reinicie Clisis Coder para reinstalar los paquetes del proveedor más recientes.

Esto obligará a Clisis Coder a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
Clisis Coder detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
