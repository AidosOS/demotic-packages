...
Puede agregar herramientas externas a Clisis Coder utilizando el _Model Context Protocol_ o MCP. Clisis Coder admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Clisis Coder Config](https://Clisis Coder.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/Clisis Coder`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Clisis Coder maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Clisis Coder:
...
Si el servidor requiere autenticación, Clisis Coder le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `Clisis Coder mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Clisis Coder almacenará los tokens de forma segura en `~/.local/share/Clisis Coder/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Clisis Coder, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Clisis Coder como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Clisis Coder a su cuenta Sentry.
