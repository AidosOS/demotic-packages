---
title: Proveedores
description: Usando cualquier proveedor LLM en Clisis Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Clisis Coder usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Clisis Coder.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Clisis Coder
configuración.
...
Clisis Coder Zen es una lista de modelos proporcionados por el equipo Clisis Coder que han sido
probado y verificado para funcionar bien con Clisis Coder. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Clisis Coder Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione Clisis Coder y diríjase a [Clisis Coder.ai/auth](https://Clisis Coder.ai/auth).
...
Funciona como cualquier otro proveedor en Clisis Coder y su uso es completamente opcional.

---

## Clisis Coder Go

Clisis Coder Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Clisis Coder que han sido
probado y verificado para funcionar bien con Clisis Coder.

1. Ejecute el comando `/connect` en TUI, seleccione `Clisis Coder Go` y diríjase a [Clisis Coder.ai/auth](https://Clisis Coder.ai/zen).
...
Para usar Amazon Bedrock con Clisis Coder:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta Clisis Coder:
   ...
   Para una configuración persistente o específica del proyecto, utilice `Clisis Coder.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Clisis Coder no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar Clisis Coder para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que Clisis Coder funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración Clisis Coder.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `Clisis Coder`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación Clisis Coder.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Clisis Coder utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Clisis Coder
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `Clisis Coder.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [Clisis Coder-gitlab-auth](https://www.npmjs.com/package/Clisis Coder-gitlab-auth).

##### Configuración

Personalizar a través de `Clisis Coder.json`:
...
Para utilizar su suscripción GitHub Copilot con Clisis Coder:
...
Para utilizar Google Vertex AI con Clisis Coder:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta Clisis Coder.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de Clisis Coder, siempre podrá configurarlo usted mismo.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Clisis Coder enumerada como una sesión separada.
...
Puede configurar Clisis Coder para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar Clisis Coder para usar modelos locales a través de LM Studio.
...
Puedes configurar Clisis Coder para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Clisis Coder. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/Clisis Coder) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Clisis Coder:
...
3. Copie la clave API para usarla en Clisis Coder.
...
6. **Importante**: Antes de usar modelos en la nube en Clisis Coder, debe obtener la información del modelo localmente:
...
Clisis Coder Zen es una lista de modelos probados y verificados proporcionada por el equipo Clisis Coder. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Clisis Coder Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Clisis Coder Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Clisis Coder API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración Clisis Coder.
...
5. También puedes personalizarlos a través de tu configuración Clisis Coder. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Clisis Coder:
...
También puede personalizar modelos a través de su configuración Clisis Coder. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con Clisis Coder. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `Clisis Coder.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Clisis Coder comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `Clisis Coder auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración Clisis Coder y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración Clisis Coder.
