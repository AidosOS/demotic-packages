...
Clisis Coder Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

Go funciona como cualquier otro proveedor en Clisis Coder. Te suscribes a Clisis Coder Go y
...
Clisis Coder Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

Clisis Coder Go funciona como cualquier otro proveedor en Clisis Coder.

1. Inicias sesión en **<a href={console}>Clisis Coder Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `Clisis Coder Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a Clisis Coder Go.
:::
...
Clisis Coder Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de Clisis Coder
usa el formato `Clisis Coder-go/<model-id>`. Por ejemplo, para Kimi K2.6, usarías
`Clisis Coder-go/kimi-k2.6` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con Clisis Coder.
