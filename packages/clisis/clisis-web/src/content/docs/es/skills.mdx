...
Las habilidades del agente permiten a Clisis Coder descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Clisis Coder busca estas ubicaciones:

- Configuración del proyecto: `.Clisis Coder/skills/<name>/SKILL.md`
- Configuración global: `~/.config/Clisis Coder/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Clisis Coder sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.Clisis Coder/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/Clisis Coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.Clisis Coder/skills/git-release/SKILL.md` así:
...
Clisis Coder enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `Clisis Coder.json`:
...
**Para agentes integrados** (en `Clisis Coder.json`):
