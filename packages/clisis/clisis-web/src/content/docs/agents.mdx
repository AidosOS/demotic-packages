...
There are two types of agents in Clisis Coder; primary agents and subagents.
...
Clisis Coder comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Clisis Coder comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

Clisis Coder comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into Clisis Coder's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `Clisis Coder.json` config file:
...
- Per-project: `.Clisis Coder/agents/`
...
If no temperature is specified, Clisis Coder uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Clisis Coder config and the project specific config.
...
The model ID in your Clisis Coder config uses the format `provider/model-id`. For example, if you're using [Clisis Coder Zen](/docs/zen), you would use `Clisis Coder/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `Clisis Coder models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/AidosOS/demotic-clisis-coder).
:::
