---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Clisis Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Clisis Coder bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Clisis Coder-konfigurasjonen.
...
i `~/.local/share/Clisis Coder/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Clisis Coder
konfig.
...
Clisis Coder Zen er en liste over modeller levert av Clisis Coder-teamet som har vært
testet og verifisert for å fungere godt med Clisis Coder. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Clisis Coder Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Clisis Coder Zen` og gå til [Clisis Coder.ai/auth](https://Clisis Coder.ai/zen).
...
Det fungerer som alle andre leverandører i Clisis Coder og er helt valgfritt å bruke.

---

## Clisis Coder Go

Clisis Coder Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Clisis Coder-teamet som har vært
testet og verifisert for å fungere godt med Clisis Coder.

1. Kjør kommandoen `/connect` i TUI, velg `Clisis Coder Go`, og gå til [Clisis Coder.ai/auth](https://Clisis Coder.ai/zen).
...
Slik bruker du Amazon Bedrock med Clisis Coder:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Clisis Coder:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `Clisis Coder.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i Clisis Coder støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere Clisis Coder til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Clisis Coder skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Clisis Coder-konfigurasjonen.
...
for å lagre token i Clisis Coder auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Clisis Coder bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Clisis Coder
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`Clisis Coder.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [Clisis Coder-gitlab-auth](https://www.npmjs.com/package/Clisis Coder-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `Clisis Coder.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Clisis Coder:
...
Slik bruker du Google Vertex AI med Clisis Coder:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Clisis Coder.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Clisis Coder, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [Clisis Coder-helicone-session](https://github.com/H2Shami/Clisis Coder-helicone-session) plugin for automatisk å logge hver Clisis Coder-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Clisis Coder-samtale oppført som en separat økt.
...
Du kan konfigurere Clisis Coder for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Clisis Coder for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Clisis Coder for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Clisis Coder. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/Clisis Coder) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Clisis Coder:
...
3. Kopier API-nøkkelen for bruk i Clisis Coder.
...
6. **Viktig**: Før du bruker skymodeller i Clisis Coder, må du hente modellinformasjonen lokalt:
...
Clisis Coder Zen er en liste over testede og verifiserte modeller levert av Clisis Coder-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Clisis Coder Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Clisis Coder Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Clisis Coder API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Clisis Coder-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Clisis Coder-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Clisis Coder:
...
Du kan også tilpasse modeller gjennom Clisis Coder-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Clisis Coder. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `Clisis Coder.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Clisis Coder forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `Clisis Coder auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Clisis Coder-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Clisis Coder-konfigurasjonen.
