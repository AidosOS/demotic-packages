...
Du kan legge til eksterne verktøy i Clisis Coder ved å bruke _Model Context Protocol_ eller MCP. Clisis Coder støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Clisis Coder Config](https://Clisis Coder.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="Clisis Coder.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/Clisis Coder`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="Clisis Coder.jsonc" {15}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
Clisis Coder håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Clisis Coder:
...
Hvis serveren krever autentisering, vil Clisis Coder be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `Clisis Coder mcp auth <server-name>`.
...
```json title="Clisis Coder.json" {7-11}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
Clisis Coder mcp auth my-oauth-server
```
...
```bash
Clisis Coder mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Clisis Coder lagre tokens sikkert i `~/.local/share/Clisis Coder/mcp-auth.json`.
...
```json title="Clisis Coder.json" {7}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "mcp": {
    "my-api-key-server": {
...
Clisis Coder mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Clisis Coder, sammen med innebygde verktøy. Så du kan administrere dem gjennom Clisis Coder-konfigurasjonen som alle andre verktøy.
...
```json title="Clisis Coder.json" {14}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "mcp": {
    "my-mcp-foo": {
...
```json title="Clisis Coder.json" {11, 14-18}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="Clisis Coder.json" {4-8}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
Clisis Coder mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Clisis Coder til Sentry-kontoen din.
...
```json title="Clisis Coder.json" {4-7}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="Clisis Coder.json" {7-9}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Clisis Coder-samtale oppført som en separat økt.
