---
title: Tjener
description: Samhandle med Clisis Coder-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `Clisis Coder serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Clisis Coder-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `Clisis Coder.local` |
...
Sett `Clisis Coder_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `Clisis Coder` som standard, eller sett `Clisis Coder_SERVER_USERNAME` for å overstyre det. Dette gjelder både `Clisis Coder serve` og `Clisis Coder web`.
...
Når du kjører `Clisis Coder` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Clisis Coder-serveren til å samhandle med Clisis Coder programmatisk.
:::

Denne arkitekturen lar Clisis Coder støtte flere klienter og lar deg samhandle med Clisis Coder programmatisk.

Du kan kjøre `Clisis Coder serve` for å starte en frittstående server. Hvis du har
Clisis Coder TUI kjører, vil `Clisis Coder serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Clisis Coder [IDE](/docs/ide) plugins.
...
Clisis Coder-serveren viser følgende APIer.
