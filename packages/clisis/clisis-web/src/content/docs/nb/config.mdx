---
title: Konfigurasjon
description: Ved å bruke Clisis Coder JSON-konfigurasjonen.
---

Du kan konfigurere Clisis Coder ved å bruke en JSON konfigurasjonsfil.

---

## Format

Clisis Coder støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "Clisis Coder"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/Clisis Coder`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/Clisis Coder/Clisis Coder.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`Clisis Coder_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`Clisis Coder.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.Clisis Coder` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`Clisis Coder_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.Clisis Coder` og `~/.config/Clisis Coder` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/Clisis Coder`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale Clisis Coder-konfigurasjon i `~/.config/Clisis Coder/Clisis Coder.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `Clisis Coder.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Clisis Coder starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `Clisis Coder_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `Clisis Coder_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.Clisis Coder` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.Clisis Coder`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`Clisis Coder.ai/config.json`**](https://Clisis Coder.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `Clisis Coder serve` og `Clisis Coder web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Clisis Coder-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `Clisis Coder.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Clisis Coder-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Clisis Coder å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/Clisis Coder/agents/` eller `.Clisis Coder/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Clisis Coder falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`Clisis Coder run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/Clisis Coder/commands/` eller `.Clisis Coder/commands/`. [Les mer her](/docs/commands).
...
Clisis Coder vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Clisis Coder alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Clisis Coder med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.Clisis Coder/plugins/` eller `~/.config/Clisis Coder/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Clisis Coder til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
