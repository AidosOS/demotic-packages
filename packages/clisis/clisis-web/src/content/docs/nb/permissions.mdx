...
Clisis Coder bruker `permission`-konfigurasjonen for å bestemme om en gitt handling skal kjøres automatisk, spørre deg eller blokkeres.
...
Bruk `external_directory` for å tillate verktøyanrop som berører stier utenfor arbeidskatalogen der Clisis Coder ble startet. Dette gjelder alle verktøy som tar en bane som input (for eksempel `read`, `edit`, `glob`, `grep` og mange `bash`-kommandoer).
...
Clisis Coder-tillatelser tastes inn etter verktøynavn, pluss et par sikkerhetsvakter:
...
Hvis du ikke spesifiserer noe, starter Clisis Coder fra tillatte standardinnstillinger:
...
Når Clisis Coder ber om godkjenning, gir brukergrensesnittet tre alternativer:

- `once` — godkjenne nettopp denne forespørselen
- `always` — godkjenn fremtidige forespørsler som samsvarer med de foreslåtte mønstrene (for resten av den nåværende Clisis Coder-økten)
