...
De fleste populære leverandørene er forhåndslastet som standard. Hvis du har lagt til legitimasjonen for en leverandør gjennom kommandoen `/connect`, vil den være tilgjengelig når du starter Clisis Coder.
...
Her er flere modeller som fungerer godt med Clisis Coder, uten spesiell rekkefølge. (Dette er ikke en uttømmende liste og er heller ikke nødvendigvis oppdatert):
...
Her er hele ID `provider_id/model_id`. For eksempel, hvis du bruker [Clisis Coder Zen](/docs/zen), vil du bruke `Clisis Coder/gpt-5.1-codex` for GPT 5.1 Codex.
...
Mange modeller støtter flere varianter med forskjellige konfigurasjoner. Clisis Coder leveres med innebygde standardvarianter for populære leverandører.

### Innebygde varianter

Clisis Coder leveres med standardvarianter for mange leverandører:
...
Når Clisis Coder starter opp, ser den etter modeller i følgende prioriterte rekkefølge:

1. Kommandolinjeflagget `--model` eller `-m`. Formatet er det samme som i konfigurasjonsfilen: `provider_id/model_id`.

2. Modelllisten i Clisis Coder-konfigurasjonen.
