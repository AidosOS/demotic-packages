...
Možete dodati vanjske alate u Clisis Coder koristeći _Model Context Protocol_, ili MCP. Clisis Coder podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Clisis Coder Config](https://Clisis Coder.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/Clisis Coder`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Clisis Coder automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Clisis Coder će:
...
Ako server zahtijeva autentifikaciju, Clisis Coder će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `Clisis Coder mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Clisis Coder sigurno čuva tokene u `~/.local/share/Clisis Coder/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Clisis Coder, zajedno s ugrađenim alatima. Možete njima upravljati kroz Clisis Coder konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Clisis Coder sa vašim Sentry nalogom.
