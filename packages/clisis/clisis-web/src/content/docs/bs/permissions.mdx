...
Clisis Coder koristi `permission` konfiguraciju da odluči da li će se određena radnja pokrenuti automatski, zatražiti od vas ili biti blokirana.
...
Koristite `external_directory` da dozvolite pozive alata koji dodiruju putanje izvan radnog direktorija gdje je Clisis Coder pokrenut. Ovo se odnosi na bilo koji alat koji uzima putanju kao ulaz (na primjer `read`, `edit`, `glob`, `grep` i mnoge `bash` komande).
...
Dozvole Clisis Coder su označene imenom alata, plus nekoliko sigurnosnih mjera:
...
Ako ništa ne navedete, Clisis Coder počinje od dozvoljenih zadanih vrijednosti:
...
Kada Clisis Coder zatraži odobrenje, korisničko sučelje nudi tri ishoda:

- `once` — odobri samo ovaj zahtjev
- `always` — odobri buduće zahtjeve koji odgovaraju predloženim obrascima (za ostatak trenutne Clisis Coder sesije)
