...
Postoje dvije vrste agenata u Clisis Coder: primarni agenti i podagenti.
...
Clisis Coder dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Clisis Coder dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Clisis Coder dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u Clisis Coder-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `Clisis Coder.json`:
...
- Globalno: `~/.config/Clisis Coder/agents/`
- Po projektu: `.Clisis Coder/agents/`

```markdown title="~/.config/Clisis Coder/agents/review.md"
---
...
Ako temperatura nije navedena, Clisis Coder koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Clisis Coder konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Clisis Coder konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Clisis Coder Zen](/docs/zen), koristili biste `Clisis Coder/gpt-5.1-codex` za GPT 5.1 Codex.
...
```json title="Clisis Coder.json" {3-6,9-12}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "tools": {
    "write": true,
...
```json title="Clisis Coder.json" {3-5,8-10}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="Clisis Coder.json" {8}
{
  "$schema": "https://Clisis Coder.ai/config.json",
  "agent": {
    "build": {
...
```json title="Clisis Coder.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Pokrenite `Clisis Coder models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/AidosOS/demotic-clisis-coder).
:::
...
```markdown title="~/.config/Clisis Coder/agents/docs-writer.md"
---
...
```markdown title="~/.config/Clisis Coder/agents/security-auditor.md"
---
