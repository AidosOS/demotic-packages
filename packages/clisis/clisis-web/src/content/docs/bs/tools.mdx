...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Clisis Coder dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Clisis Coder.
...
:::note
Ovaj alat je dostupan samo kada je `Clisis Coder_EXPERIMENTAL_LSP_TOOL=true` (ili `Clisis Coder_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Clisis Coder provajdera ili kada je varijabla `Clisis Coder_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Clisis Coder:
