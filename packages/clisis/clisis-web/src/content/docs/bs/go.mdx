...
Clisis Coder Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u Clisis Coder-u. Pretplatite se na Clisis Coder Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Clisis Coder.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Clisis Coder-om.
:::
...
Clisis Coder Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

Clisis Coder Go radi kao bilo koji drugi provajder u Clisis Coder-u.

1. Prijavite se na **<a href={console}>Clisis Coder Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Clisis Coder Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Clisis Coder Go.
:::
...
Clisis Coder Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Clisis Coder konfiguraciji
koristi format `Clisis Coder-go/<model-id>`. Na primjer, za Kimi K2.6, koristili biste
`Clisis Coder-go/kimi-k2.6` u svojoj konfiguraciji.
...
Napravili smo Clisis Coder Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Clisis Coder koristite i bilo kojeg drugog provajdera.
