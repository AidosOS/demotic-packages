---
title: Server
description: Komunicirajte s Clisis Coder serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `Clisis Coder serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji Clisis Coder klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `Clisis Coder.local` |
...
Postavite `Clisis Coder_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `Clisis Coder`, ili postavite `Clisis Coder_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `Clisis Coder serve` i za `Clisis Coder web`.
...
Kada pokrenete `Clisis Coder`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite Clisis Coder server za programsku interakciju sa Clisis Coder-om.
:::

Ova arhitektura omogucava Clisis Coder podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `Clisis Coder serve` da startate standalone server. Ako je Clisis Coder TUI vec pokrenut, `Clisis Coder serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Clisis Coder [IDE](/docs/ide) pluginovi.
...
Clisis Coder server izlaže sljedece API-je.
