---
title: SDK
description: Type-safe JS klijent za Clisis Coder server.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Clisis Coder JS/TS SDK pruza type-safe klijent za interakciju sa serverom.
Koristite ga za izradu integracija i programsko upravljanje Clisis Coder-om.
...
Kreirajte instancu Clisis Coder:
...
Mozete proslijediti konfiguracijski objekat za prilagodavanje ponasanja. Instanca i dalje ucitava `Clisis Coder.json`, ali konfiguraciju mozete nadjacati ili dodati inline:
...
Ako vec imate pokrenutu Clisis Coder instancu, mozete napraviti klijentsku instancu i povezati se na nju:
...
MoÅ¾ete zatraÅ¾iti strukturirani JSON izlaz od modela specificiranjem `format` sa JSON Å¡emom. Model Ä‡e koristiti `StructuredOutput` alat da vrati validirani JSON koji odgovara vaÅ¡oj Å¡emi.
...
| `json_schema` | VraÄ‡a validirani JSON koji odgovara pruÅ¾enoj Å¡emi                   |
...
| Polje        | Tip             | Opis                                                        |
| ------------ | --------------- | ----------------------------------------------------------- |
| `type`       | `'json_schema'` | Obavezno. OdreÄ‘uje JSON schema naÄin rada                   |
| `schema`     | `object`        | Obavezno. JSON Schema objekt koji definira strukturu izlaza |
| `retryCount` | `number`        | Opcionalno. Broj ponovnih pokuÅ¡aja validacije (default: 2)  |

### Rukovanje greÅ¡kama

Ako model ne uspije proizvesti validan strukturirani izlaz nakon svih ponovnih pokuÅ¡aja, odgovor Ä‡e ukljuÄivati `StructuredOutputError`:
...
1. **Navedite jasne opise** u svojstvima vaÅ¡e Å¡eme kako biste pomogli modelu da razumije koje podatke treba izdvojiti
2. **Koristite `required`** da odredite koja polja moraju biti prisutna
3. **DrÅ¾ite Å¡eme fokusiranim** - sloÅ¾ene ugnijeÅ¾Ä‘ene Å¡eme modelu mogu biti teÅ¾e za ispravno popunjavanje
4. **Postavite odgovarajuÄ‡i `retryCount`** - poveÄ‡ajte za sloÅ¾ene Å¡eme, smanjite za jednostavne

---

## API-ji

SDK izlaÅ¾e sve server API-je kroz type-safe klijent.
