---
title: Konfiguracija
description: Korištenje Clisis Coder JSON konfiguracije.
---

Možete konfigurirati Clisis Coder koristeći JSON konfiguracijski fajl.

---

## Format

Clisis Coder podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/Clisis Coder`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/Clisis Coder/Clisis Coder.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`Clisis Coder_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`Clisis Coder.json` u projektu) - postavke specifične za projekat
5. **`.Clisis Coder` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`Clisis Coder_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.Clisis Coder` i `~/.config/Clisis Coder` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/Clisis Coder` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Clisis Coder konfiguraciju u `~/.config/Clisis Coder/Clisis Coder.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/Clisis Coder/tui.json`.
...
Dodajte `Clisis Coder.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Clisis Coder pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `Clisis Coder_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `Clisis Coder_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.Clisis Coder` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.Clisis Coder`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`Clisis Coder.ai/config.json`**](https://Clisis Coder.ai/config.json).

TUI konfiguracija koristi [**`Clisis Coder.ai/tui.json`**](https://Clisis Coder.ai/tui.json).
...
Koristite `Clisis Coder_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `Clisis Coder.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `Clisis Coder serve` i `Clisis Coder web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Clisis Coder server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `Clisis Coder.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Clisis Coder konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Clisis Coder pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/Clisis Coder/agents/` ili `.Clisis Coder/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Clisis Coder će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`Clisis Coder run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/Clisis Coder/commands/` ili `.Clisis Coder/commands/`. [Saznajte više ovdje](/docs/commands).
...
Clisis Coder će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Clisis Coder **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Clisis Coder sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.Clisis Coder/plugins/` ili `~/.config/Clisis Coder/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Clisis Coder da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
