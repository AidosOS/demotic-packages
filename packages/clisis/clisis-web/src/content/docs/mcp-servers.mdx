...
You can add external tools to Clisis Coder using the _Model Context Protocol_, or MCP. Clisis Coder supports both local and remote servers.
...
You can define MCP servers in your [Clisis Coder Config](https://Clisis Coder.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/Clisis Coder` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Clisis Coder automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Clisis Coder will:
...
If the server requires authentication, Clisis Coder will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `Clisis Coder mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Clisis Coder will store the tokens securely in `~/.local/share/Clisis Coder/mcp-auth.json`.
...
Your MCPs are available as tools in Clisis Coder, alongside built-in tools. So you can manage them through the Clisis Coder config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Clisis Coder to your Sentry account.
