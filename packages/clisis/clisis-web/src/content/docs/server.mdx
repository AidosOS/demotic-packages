---
title: Server
description: Interact with Clisis Coder server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `Clisis Coder serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an Clisis Coder client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `Clisis Coder.local` |
...
Set `Clisis Coder_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `Clisis Coder`, or set `Clisis Coder_SERVER_USERNAME` to override it. This applies to both `Clisis Coder serve` and `Clisis Coder web`.
...
When you run `Clisis Coder` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the Clisis Coder server to interact with Clisis Coder programmatically.
:::

This architecture lets Clisis Coder support multiple clients and allows you to interact with Clisis Coder programmatically.

You can run `Clisis Coder serve` to start a standalone server. If you have the
Clisis Coder TUI running, `Clisis Coder serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Clisis Coder [IDE](/docs/ide) plugins.
...
The Clisis Coder server exposes the following APIs.
