...
通过 Clisis Coder，您可以从多个内置主题中进行选择，使用能自动适配终端主题的主题，或者定义您自己的自定义主题。

默认情况下，Clisis Coder 使用我们自己的 `Clisis Coder` 主题。
...
Clisis Coder 支持灵活的基于 JSON 的主题系统，让用户可以轻松创建和自定义主题。
...
1. **内置主题** — 嵌入在二进制文件中
2. **用户配置目录** — 定义在 `~/.config/Clisis Coder/themes/*.json` 或 `$XDG_CONFIG_HOME/Clisis Coder/themes/*.json`
3. **项目根目录** — 定义在 `<project-root>/.Clisis Coder/themes/*.json`
4. **当前工作目录** — 定义在 `./.Clisis Coder/themes/*.json`
