...
代理技能让 Clisis Coder 能够从你的仓库或主目录中发现可复用的指令。
...
Clisis Coder 会搜索以下位置：

- 项目配置：`.Clisis Coder/skills/<name>/SKILL.md`
- 全局配置：`~/.config/Clisis Coder/skills/<name>/SKILL.md`
...
对于项目本地路径，Clisis Coder 会从当前工作目录向上遍历，直到到达 git 工作树根目录。
在此过程中，它会加载 `.Clisis Coder/` 中所有匹配的 `skills/*/SKILL.md`，以及匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。

全局定义也会从 `~/.config/Clisis Coder/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md` 和 `~/.agents/skills/*/SKILL.md` 中加载。
...
创建 `.Clisis Coder/skills/git-release/SKILL.md`，内容如下：
...
Clisis Coder 会在 `skill` 工具描述中列出可用技能。
每个条目包含技能名称和描述：
...
在 `Clisis Coder.json` 中使用基于模式的权限来控制代理可以访问哪些技能：
