...
console.log("ðŸš€ Starting clisisCoder server...")
const clisisCoder = await createClisisCoder({
  port: 0,
})
console.log("âœ… clisisCoder server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await ClisisCoder.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("ðŸ†• Creating new clisisCoder session...")
    const { client, server } = clisisCoder
    ...
    console.log("âœ… Created clisisCoder session:", createResult.data.id)
  ...
  console.log("ðŸ“ Sending to clisisCoder:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("ðŸ“¤ clisisCoder response:", JSON.stringify(result, null, 2))
