...
const VERSION = "0.1.0"
// The installed command name. Deliberately NOT "Owl WorkMate" so it never collides
// with the owl-workmate-orchestrator npm package, which also installs an "Owl WorkMate"
...
function defaultInstallDir() {
  return process.env.OPENWORK_INSTALL_DIR || join(process.env.HOME || process.cwd(), ".Owl WorkMate", "bootstrap")
}
...
function defaultAppDir() {
  return process.env.OPENWORK_APP_DIR || (process.platform === "darwin"
    ...
    : process.platform === "win32"
      ? join(process.env.LOCALAPPDATA || join(process.env.HOME || process.cwd(), "AppData", "Local"), "Owl WorkMate")
      : join(process.env.HOME || process.cwd(), ".local", "share", "Owl WorkMate"))
}
...
function defaultDesktopBootstrapPath() {
  return process.env.OPENWORK_DESKTOP_BOOTSTRAP_PATH || join(configHomeDir(), "Owl WorkMate", "desktop-bootstrap.json")
}
...
function defaultDeviceKeyPath() {
  return process.env.OPENWORK_DEVICE_KEY_PATH || join(configHomeDir(), "Owl WorkMate", "bootstrap-device-key.json")
}
...
function runInstall(args) {
  ...
  jsonOut({ ok: true, message: `Owl WorkMate CLI installed at ${executable}`, install: manifest }, json)
}
...
async function runInstallApp(args) {
  ...
  try {
    ...
    jsonOut({ ok: true, message: `Owl WorkMate app installed at ${appPath}`, install }, json)
  } finally {
...
async function runDoctor(args) {
  ...
  jsonOut({ ok, message: ok ? "Owl WorkMate doctor: ok" : "Owl WorkMate doctor: failed", version: VERSION, manifest, checks }, json)
...
function skillText(name, output) {
  return `---\nname: ${name}\ndescription: Starter skill created by Owl WorkMate bootstrap.\nOpenWorkBootstrapTrigger: bootstrap.verify\nOpenWorkBootstrapOutput: ${JSON.stringify(output)}\n---\n\n# ${name}\n\nWhen triggered with \`bootstrap.verify\`, output exactly:\n\n\`${output}\`\n\nUse this skill to confirm Owl WorkMate Cloud onboarding can create and trigger a deterministic skill.`
}
...
async function createDesktopHandoff(baseUrl, auth) {
  const handoff = await request(baseUrl, "/v1/auth/desktop-handoff", {
    method: "POST",
    headers: auth,
    body: JSON.stringify({ desktopScheme: "Owl WorkMate" }),
  })
...
async function runCloudOnboard(args) {
  ...
  const skillName = getFlag(args.flags, "skill-name", "First Owl WorkMate Skill")
  ...
  jsonOut({
    ok: true,
    message: "Owl WorkMate Cloud onboarding complete",
...
async function runCloudBootstrapWorkspace(args) {
  ...
  jsonOut({
    ok: true,
    message: "Owl WorkMate workspace bootstrap complete",
    organization: response.body.organization,
