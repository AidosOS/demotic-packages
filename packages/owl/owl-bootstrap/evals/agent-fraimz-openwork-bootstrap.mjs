...
const installedOwl WorkMate = join(binDir, process.platform === "win32" ? "openwork-bootstrap.cmd" : "openwork-bootstrap")
...
try {
  ...
  prove("A bootstrap script can install the Owl WorkMate CLI", {
  ...
  }, install.status === 0 && install.json?.ok === true && install.json?.install?.executable === installedOwl WorkMate)

  const doctor = run(installedOwl WorkMate, ["doctor", "--install-dir", installDir, "--bin-dir", binDir, "--base-url", baseUrl, "--json"])
  prove("The installed CLI can doctor itself and the Den API", {
    action: "Owl WorkMate doctor --base-url <live-den-api> --json",
  ...
  const onboard = run(installedOwl WorkMate, [
    "cloud",
    "onboard",
    "--base-url",
    baseUrl,
  ...
  prove("The installed CLI can onboard a user, org, invite, and skill end-to-end", {
    action: "Owl WorkMate cloud onboard --base-url <live-den-api> --owner-email ... --org-name ... --invite-email ... --skill-name ... --json",
...
<title>Owl WorkMate Bootstrap CLI — live e2e fraimz</title>
...
<h1>Owl WorkMate Bootstrap CLI — live e2e fraimz</h1>
