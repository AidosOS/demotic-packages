...
// bin dir, then runs the installed `Owl WorkMate install app` command against the
// manifest and verifies it with `Owl WorkMate doctor --app`.
...
if (process.platform !== "darwin") {
  throw new Error("Owl WorkMate install app DMG fraimz requires macOS")
}
...
const outDir = join(repoRoot, "evals", "results", "owl-workmate-app-install-dmg")
const temp = join(tmpdir(), `owl-workmate-app-install-dmg-${Date.now()}-${Math.random().toString(36).slice(2)}`)
...
const installedOwl WorkMate = join(binDir, "openwork-bootstrap")
...
<key>CFBundleExecutable</key><string>Owl WorkMate</string>
<key>CFBundleIdentifier</key><string>com.openwork.fixture</string>
<key>CFBundleName</key><string>Owl WorkMate</string>
<key>CFBundlePackageType</key><string>APPL</string>
<key>CFBundleShortVersionString</key><string>0.0.0-fixture</string>
</dict></plist>
`)
  writeFileSync(join(appFixture, "Contents", "MacOS", "Owl WorkMate"), "#!/usr/bin/env sh\necho Owl WorkMate fixture\n")
  chmodSync(join(appFixture, "Contents", "MacOS", "Owl WorkMate"), 0o755)

  execFileSync("hdiutil", ["create", "-quiet", "-volname", "Owl WorkMate", "-srcfolder", sourceDir, "-ov", "-format", "UDZO", dmgPath])
  ...
  prove("The bootstrap CLI can be installed from a script", {
    ...
    assert: "exit 0 and an Owl WorkMate executable exists",
    evidence: { status: installCli.status, body: installCli.json },
  }, installCli.status === 0 && existsSync(installedOwl WorkMate))

  const installApp = run(installedOwl WorkMate, ["install", "app", "--manifest", manifestPath, "--app-dir", appDir, "--json"], { timeout: 30_000 })
  ...
  prove("The installed CLI can download, verify, mount, and install openwork.app from a DMG", {
    action: "Owl WorkMate install app --manifest <fixture-manifest> --app-dir <tmp>/Applications --json",
  ...
  const doctor = run(installedOwl WorkMate, ["doctor", "--install-dir", installDir, "--bin-dir", binDir, "--app", "--app-dir", appDir, "--json"])
  prove("doctor verifies the installed desktop app", {
    action: "Owl WorkMate doctor --app --app-dir <tmp>/Applications --json",
...
<title>Owl WorkMate app DMG Install — fraimz</title>
...
</head><body><main><h1>Owl WorkMate app DMG Install — fraimz</h1><div class="meta">Result: <code>${allOk ? "passed" : "failed"}</code> · Frames: ${frames.length}</div>
...
writeFileSync(join(outDir, "report.json"), JSON.stringify({
  flow: "owl-workmate-app-install-dmg",
