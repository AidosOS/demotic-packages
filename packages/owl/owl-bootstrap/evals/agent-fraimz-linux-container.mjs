...
// - `Owl WorkMate install app` installs a Linux tar.gz app artifact from a manifest,
// - `Owl WorkMate doctor --app` verifies the installed app,
// - `Owl WorkMate cloud onboard` can hit a live Den API from inside Linux.
//
// Required host setup:
...
try {
  const appBinary = join(artifactRoot, "Owl WorkMate")
  writeFileSync(appBinary, "#!/usr/bin/env sh\necho Owl WorkMate Linux fixture\n")
  chmodSync(appBinary, 0o755)
  execFileSync("tar", ["-czf", artifactPath, "-C", artifactRoot, "Owl WorkMate"])
  const digest = sha256(artifactPath)
  writeFileSync(manifestPath, JSON.stringify({
    version: "0.0.0-linux-fixture",
    artifacts: {
      linux: {
        x64: { type: "tar.gz", url: "file:///fixture/openwork-linux.tar.gz", sha256: digest, appName: "Owl WorkMate" },
        arm64: { type: "tar.gz", url: "file:///fixture/openwork-linux.tar.gz", sha256: digest, appName: "Owl WorkMate" },
      },
    },
  }, null, 2))
  prove("A Linux app artifact manifest is available without shipping to production", {
    action: "Create local Owl WorkMate tar.gz artifact and manifest with SHA-256",
  ...
  prove("Linux can install the CLI and app artifact, then doctor the app", {
    action: "Run Owl WorkMate install, Owl WorkMate install app, and Owl WorkMate doctor --app inside Linux",
  ...
  prove("Linux installed CLI can complete live cloud onboarding", {
    action: "Run Owl WorkMate cloud onboard from inside Linux against live Den API",
...
<title>Owl WorkMate Linux Bootstrap — fraimz</title>
...
</head><body><main><h1>Owl WorkMate Linux Bootstrap — fraimz</h1><div class="meta">Result: <code>${allOk ? "passed" : "failed"}</code> · Live Den API: <code>${esc(denBaseUrl)}</code> · Frames: ${frames.length}</div>
