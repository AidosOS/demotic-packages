 ...
 * openwork-ui-mcp
 *
 * MCP server that exposes Owl WorkMate's UI control surface as MCP tools.
 * Speaks MCP stdio and proxies to the Owl WorkMate Desktop bridge HTTP API.
 *
 * Requires Owl WorkMate Desktop running with the local UI control bridge active.
 *
 * Usage:
 *   npx openwork-ui-mcp
 *
...
function discoveryPaths() {
  return [
    ...
    join(userAppDataDir(), "com.differentai.Owl WorkMate", DISCOVERY_FILE),
...
async function bridgeRequest(path, options = {}) {
  ...
  if (!bridge) {
    return {
      ok: false,
      error: "Owl WorkMate is not running. Launch the Owl WorkMate Desktop app first.",
      hint: "The MCP server connects to a running Owl WorkMate instance via its local bridge.",
    };
  }
...
server.tool(
  "ui_snapshot",
  "Get a snapshot of the current Owl WorkMate UI state: active route, narration, visible actions, and status. Use this before taking action to understand what the user sees.",
  {},
  async () => {
    ...
    return { content: [{ type: "text", text: lines.join("\n") || "Owl WorkMate is reachable, but it did not return visible UI state." }] };
  }
);
...
server.tool(
  "ui_list_actions",
  "List all UI control actions currently available in Owl WorkMate: session navigation, composer control, transcript access, and more. Each action has an id you can pass to ui_execute_action.",
  {},
  async () => {
    ...
    if (!Array.isArray(result.actions) || result.actions.length === 0) {
      return { content: [{ type: "text", text: "No actions available. Is Owl WorkMate on the main screen?" }] };
    }
...
server.tool(
  "ui_execute_action",
  "Execute an Owl WorkMate UI action by its id. Use ui_list_actions first to see available actions and their required arguments.",
  {
...
server.tool(
  "ui_status",
  "Check if Owl WorkMate is running and the bridge is reachable. Returns connection status and app info.",
  {},
  async () => {
    const bridge = await discoverBridge();
    if (!bridge) {
      return { content: [{ type: "text", text: "Owl WorkMate is not running.\nLaunch the Owl WorkMate Desktop app to enable UI control." }], isError: true };
    }
    try {
      const response = await fetch(`${bridge.baseUrl}/health`, { signal: AbortSignal.timeout(3000) });
      const data = await response.json();
      return { content: [{ type: "text", text: `Connected to ${data.app || "Owl WorkMate"}\nBridge: ${bridge.baseUrl}\nVersion: ${data.version ?? "?"}` }] };
    } catch (error) {
      clearBridgeCache();
      return { content: [{ type: "text", text: `Bridge file found but not reachable: ${error.message}\nOwl WorkMate may have quit. Relaunch it.` }], isError: true };
    }
  }
);
