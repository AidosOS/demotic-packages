...
export const emailSubjects: { [Template in EmailTemplate]: (props: EmailTemplateProps[Template]) => string } = {
  verification: ({ verificationCode }) => `Your Owl WorkMate verification code is ${verificationCode}`,
  passwordReset: () => "Reset your Owl WorkMate password",
  organizationInvite: ({ organizationName }) => `You're invited to join ${organizationName} on Owl WorkMate`,
  downloadLink: () => "Your Owl WorkMate download link",
  feedback: ({ name, source }) => `Owl WorkMate feedback from ${name}${source ? ` (${source})` : ""}`,
}
