...
describe('social-share contract', () => {
  it('builds Mona Design repository share targets', () => {
    const payload = buildSocialSharePayload({
      kind: 'mona-design-repo',
      locale: 'zh-CN',
      title: 'Mona Design GitHub',
      text: '推荐 Mona Design',
    });
  ...
  it('keeps deployed project links and the repo recommendation together', () => {
    const payload = buildSocialSharePayload({
      kind: 'project-html',
      locale: 'en',
      url: 'https://example.com/mona-design-demo',
      title: 'Demo',
      text: `Built with Mona Design. Repo: ${OPEN_DESIGN_GITHUB_REPO_URL}`,
      copyText: `Demo\nhttps://example.com/mona-design-demo\n${OPEN_DESIGN_GITHUB_REPO_URL}`,
    });

    expect(payload.url).toBe('https://example.com/mona-design-demo');
    ...
    expect(payload.platforms.find((target) => target.platform === 'telegram')?.shareUrl)
      .toContain('https%3A%2F%2Fexample.com%2Fmona-design-demo');
  });
});
