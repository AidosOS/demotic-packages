...
describe('pluginDetailSlug (single segment = last id segment)', () => {
  it('takes the slugified last segment, dropping any namespace', () => {
    expect(pluginDetailSlug('mona-design/Hero Deck')).toBe('hero-deck');
...
describe('pluginSlug (multi-segment, namespace preserved)', () => {
  it('slugifies each segment and keeps / as a separator', () => {
    expect(pluginSlug('mona-design/Hero Deck')).toBe('mona-design/hero-deck');
  });
});

describe('pluginDetailPath / pluginPreviewPath', () => {
  it('detail path is single-segment with trailing slash', () => {
    expect(pluginDetailPath('mona-design/Hero Deck')).toBe('/plugins/hero-deck/');
    expect(pluginDetailPath('live-dashboard')).toBe('/plugins/live-dashboard/');
  });
  it('preview path keeps the namespace', () => {
    expect(pluginPreviewPath('mona-design/Hero Deck')).toBe(
      '/plugins/previews/mona-design/hero-deck/',
    );
  });
});

describe('pluginShareUrl', () => {
  it('defaults to the public mona-design.ai origin, single-segment path', () => {
    expect(OPEN_DESIGN_SITE_ORIGIN).toBe('https://mona-design.ai');
    expect(pluginShareUrl('mona-design/live-dashboard')).toBe(
      'https://mona-design.ai/plugins/live-dashboard/',
    );
  });
