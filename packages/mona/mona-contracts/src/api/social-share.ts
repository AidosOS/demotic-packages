export const OPEN_DESIGN_GITHUB_REPO_URL = 'https://github.com/nexu-io/mona-design';

export type SocialShareTargetKind = 'mona-design-repo' | 'project-html';
...
export function buildSocialSharePayload(input: SocialShareRequest): SocialShareResponse {
  const kind = input.kind === 'project-html' ? 'project-html' : 'mona-design-repo';
  const url = normalizeSocialShareUrl(input.url)
    ?? (kind === 'mona-design-repo' ? OPEN_DESIGN_GITHUB_REPO_URL : '');
  const fallbackTitle = kind === 'project-html' ? 'Mona Design project' : 'Mona Design';
  const title = cleanText(input.title, fallbackTitle);
  const fallbackText = kind === 'project-html'
    ? `Built with Mona Design: ${title}. Mona Design repo: ${OPEN_DESIGN_GITHUB_REPO_URL}`
    : 'Mona Design is an open-source workspace for creating, editing, deploying, and handing off design artifacts.';
