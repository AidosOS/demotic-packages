...
// Read raw `mona-design.json` text into a typed PluginManifest. The Zod
...
export function parseManifest(raw: string): ManifestParseResult {
  ...
  try {
    ...
    return {
      ok: false,
      warnings: [],
      errors: [`mona-design.json is not valid JSON: ${(err as Error).message}`],
    };
  }
