import { MarketplaceManifestSchema, type MarketplaceManifest } from '@demotic/mona-contracts';
...
export function parseMarketplace(raw: string): MarketplaceParseResult {
  ...
  try {
    ...
    return { ok: false, errors: [`mona-design-marketplace.json is not valid JSON: ${(err as Error).message}`] };
  }
