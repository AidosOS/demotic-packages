...
describe("@demotic/mona-release", () => {
  ...
  it("centralizes release identity and namespace derivation", () => {
    expect(releaseChannelDescriptor("prerelease").productName).toBe("Mona Design Prerelease");
    expect(releaseChannelDescriptor("betas").productName).toBe("Mona Design Betas");
    expect(releaseInstallIdentity("prerelease")).toEqual({
      appId: "io.mona-design.desktop.prerelease",
      executableName: "Mona Design Prerelease",
      productName: "Mona Design Prerelease",
    });
  ...
  it("infers release channels from versions and namespaces", () => {
    ...
    expect(releaseChannelFromNamespace("mona-design")).toBe("stable");
