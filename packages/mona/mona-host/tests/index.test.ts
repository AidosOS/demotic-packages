...
describe("mona-design host contract", () => {
  it("stays independent from daemon/web contracts", () => {
    ...
    }).not.toHaveProperty("@demotic/mona-contracts");

    const offenders = filesUnder(join(hostRoot, "src")).filter((path) =>
      readFileSync(path, "utf8").includes("@demotic/mona-contracts"),
    );
  ...
  it("routes updater status, actions, and subscriptions through package-owned helpers", async () => {
    const status = {
      ...
      downloadPath: "/tmp/Mona Design Beta.dmg",
      enabled: true,
