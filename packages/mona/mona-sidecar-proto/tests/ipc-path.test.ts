...
describe("normalizeIpcPath", () => {
  it("returns absolute POSIX paths unchanged", () => {
    expect(normalizeIpcPath("/tmp/mona-design/ipc/ns/web.sock")).toBe("/tmp/mona-design/ipc/ns/web.sock");
  });

  it("returns Windows named-pipe paths unchanged without absolute-path checking", () => {
    expect(normalizeIpcPath("\\\\.\\pipe\\mona-design-ns-web")).toBe("\\\\.\\pipe\\mona-design-ns-web");
  });
  ...
  it("throws when the path has leading or trailing whitespace", () => {
    expect(() => normalizeIpcPath(" /tmp/mona-design/ipc/ns/web.sock")).toThrow(/whitespace/);
    expect(() => normalizeIpcPath("/tmp/mona-design/ipc/ns/web.sock\n")).toThrow(/whitespace/);
  });

  it("throws when the path contains a null byte", () => {
    expect(() => normalizeIpcPath("/tmp/mona-design/ipc/ns/web.sock\0extra")).toThrow(/null bytes/);
  });
...
describe("isWindowsNamedPipePath", () => {
  it("returns true for paths starting with \\\\.\\pipe\\", () => {
    expect(isWindowsNamedPipePath("\\\\.\\pipe\\mona-design-ns-web")).toBe(true);
    expect(isWindowsNamedPipePath("\\\\.\\pipe\\")).toBe(true);
  });

  it("returns false for POSIX, drive-letter, or non-string inputs", () => {
    expect(isWindowsNamedPipePath("/tmp/mona-design/ipc/ns/web.sock")).toBe(false);
    expect(isWindowsNamedPipePath("C:\\Users\\ipc\\web.sock")).toBe(false);
    expect(isWindowsNamedPipePath("\\\\.\\Pipe\\mona-design")).toBe(false);
